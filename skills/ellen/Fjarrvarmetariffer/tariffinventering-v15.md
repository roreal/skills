...
**Rättat P1 (granskning `2026-09-09-005`) — kroppen var bara en fyrpunkts-sammanfattning,
utan den avsmalning `Tariffberakningsunderlag`s VALFRIA fält kräver innan
`byggKontraktIndata`s OBLIGATORISKA `kapacitetKw: number`/`policyFalt: Record<...>` kan
anropas, och utan uttrycklig ordning för energi-/kapacitetskontrollerna.** Löst genom att
skriva ut hela kroppen, i SAMMA ordning och med SAMMA orsaker som den befintliga
...
ändlig/heltal/≥golv), plus policyns fältnära förkontroll och till sist fasaden:

```ts
export function beraknaArsprodukt(underlag: Tariffberakningsunderlag): ArsprodukResultat {
  const { prisar } = valjLeverantorOchPrisar(underlag.leverantorId);
  const tariffId = prisar.id;
  ...
  const gated = kontraktsgatadPolicy(prisar)!; // icke-null: stodjerAktuellArskostnad === true
  // garanterar gated !== undefined (samma reglerorsak, ingen ny risk).
  ...
  // 6. Policyns fältnära förkontroll (§6a.2, punkt 6) — en tom karta representeras
  // EXPLICIT (inte en non-null-cast) om formuläret inte skickat några policyfält.
  const policyFalt = underlag.policyFalt ?? {};
  const { saknade, ogiltiga } = forkontrolleraPolicyIndata(gated.policy, prisar, policyFalt, 'annual');
  if (saknade.length > 0 || ogiltiga.length > 0) {
    throw new KontraktBlockerat(tariffId, 'invalid_capacity', { saknade, ogiltiga } as any);
    // (exakt felform/konstruktoralternativ specificeras av §6a.2 punkt 6, ingen ny orsak
    // uppfinns här — se KontraktBlockeratOrsak-unionen ovan.)
  }

  // 7. Bygg indata och anropa fasaden — enda anropet till motorn.
  const indata = byggKontraktIndata(gated.policy, prisar, underlag.kapacitetKw, policyFalt);
  const resultat = beraknaArskostnadMedKontrakt(indata, prisar, underlag.kapacitetKw);
  return {
    typ: 'aktuell_arskostnad',
    kostnad: resultat.kostnad.summaInkl,
    leverantor: prisar.leverantor,
    prisar: prisar.ar,
    kapacitetKw: underlag.kapacitetKw,
...
Steg 6:s exakta `KontraktBlockerat`-konstruktoranrop (vilken orsak, vilket tredje argument)
följer §6a.2 punkt 6:s redan beskrivna felform — koden ovan visar ORDNINGEN och att kartan
avsmalnas explicit, inte en ny orsakskod.
...
`falt`, `energyProvenance`) — `{ ...bas, paverkbarMwh, besparingsgrad }` är därför ett
typkorrekt `BesparingsvardeArgs`-objekt, inte ett separat, inkompatibelt schema.
`policyFalt` på `bas` ignoreras tyst av `beraknaBesparingsvarde` (legacyvägen känner inte
till fältet) — exakt samma bakåtkompatibla mönster som `energyProvenance` redan har.
...
interface. `calcResult` självt ändras därför INTE. I stället en NY, tunn wrapper-funktion:
