# Tariffinventering v6.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-08 av Claude. Ersätter `tariffinventering-v5.md` i sin helhet (v5 ändras
...
(`status: changes-required`, supersedes `2026-09-08-004`) av v5. Ingen produktkod, tariffdata
eller genererad fil är ändrad av detta dokument. Alla kodpåståenden nedan är verifierade
genom att läsa/köra den verkliga koden i `enkey-agents@fd8f8da` (skrivskyddat, ingen fil
ändrad) — inte bara omskrivna från granskningen.

**Vad som är nytt i v6, i korthet** (se granskning `2026-09-08-005` för fullständig
motivering till varje punkt):

1. **Grindpreflight ersatt av en SAMMANSATT aktiveringspreflight** (§6): v5:s `45/45`
   provade bara `grind()`, som varken känner till policyregistret, generatorns kontraktsgrind,
   leverantörsvalt band eller Umeås framtida multiplikatorbindning. §6 beskriver nu fyra
   verifierbara steg — (1) verklig utrednings-/request-status, (2) `grind()`, (3) generatorns
   `kontrollera_aktiveringsgrind()` mot ett verkligt (om ännu opopulerat) policyregister,
   (4) ett minimalt `annual_forward`-anrop — och vilket av de fyra stegen som i dag är sista
   verifierade hindret för varje rad. Ingen rad påstås passera alla fyra utan att varje steg
   är läst mot verklig kod.
2. **Tre kontraktstillägg specificerade explicit** (nytt §6a), inget av dem fanns i v5:
   `KravPost.maxvarde` (mirror av `minvarde`, samma mönster som Sandvikens golv), ett
   `supplier_confirmed_band_id`-fält skilt från det numeriska debiteringsunderlaget för
   Borlänge/C4, och en `tariff_ids`-representation på `remaining_information_requests` som
   ersätter den nuvarande `member_ids`-bara datastrukturen (§7 omskriven till denna
   representation, "SPLITTA"/"eller" borttaget helt).
3. **Sundsvall Indal flyttad från "legacy utan kontrakt" till en riktig `Tariffpolicy`**:
   v5:s batch 2 skulle ha kastat i `bygg_ts_fran_katalog()` eftersom tariff-ID:t inte står i
   `LEGACY_UNDANTAGNA_TARIFF_ID` (verifierat genom att läsa `policyregister.py`). Kräver
   `contract_required: true` plus en minimal policy (`capacity.type: not_applicable`, inga
   kapacitetsbundna krav) — samma mönster Sandviken redan bevisat, inte ny mekanism.
4. **Umeås `B`-bindning fick ett namngivet fält med intervallkrav**: `Tariffpolicy` får ett
   nytt `kapacitet_multiplikator_bindning`-fält (mirror i TS), validerat mot det publicerade
   möjliga intervallet `[0,93; 1,4]` (katalogens egna `pieces`-brytpunkter) — inte ett
   generellt undantag för varje `post_multiplier`, och testet heter `B`, inte "U-intervall",
   eftersom motorn aldrig räknar `U`.
5. **Stockholm Exergi: en produktionskälla, strukturellt bunden.** Rekommenderad väg
   specificerad konkret: utöka leverantörsfilens `stockholm-exergi-2026` med en separat,
   typad `annual_forward`-adapter i ett allowlistat adapterregister (`ADAPTERREGISTER` i
   `policyregister.py`, ny konstant) som BÅDE `grind()`-motsvarigheten och generatorn läser —
   inget rått ID-undantag i `katalog.py`. Katalogdubbletten
   (`stockholm-exergi-stockholm-exergi-normal-2026`) förblir explicit exkluderad av samma
   dedupliceringsregel som redan gäller (verifierat: `_stabilt_tariff_id()` ger
   `stockholm-exergi-stockholm-exergi-normal`, skilt från leverantörsfilens
   `stockholm-exergi` — en riktig ID-kollision om båda aktiverades).
6. **P2 rättat:** en enda livscykel per request (R10/R15 hade två motstridiga i v5); samtliga
   batchers fillistor räknade om mot §7; de påhittade filnamnen `katalog.ts`/`adjustments.ts`
   ersatta med de verkliga: TypeScript-data GENERERAS av `generera.py` ur
   Python-katalogen, motorn är `fjarrvarme.ts`, kontraktet är `resultatkontrakt.ts`, och
   `JUSTERINGSTYPER` speglas INLINE i `fjarrvarme.ts` — det finns ingen fristående
   `adjustments.ts`-fil (verifierat: `find`/`grep` mot repot). Leveranstiden i v5:s logg
   (`15:40:00`) var omöjlig (committen är tidsstämplad `14:58:36`) — rättad med en daterad
   korrigering, den gamla raden lämnad orörd.
7. **Oförändrat från v5 (redan korrekt):** dispositionsräkning 7+45+26 bas / 9+5 variant = 92,
   Borlänge/C4/Falus exakta gränsfallsbugg (verifierad på nytt med Python 3.13 mot den
   verkliga katalogen — samma resultat som Codex: Borlänge 501→band 5, C4 500→band 6, Falu
   ytterorter 501→`ValueError`), Lidköpings kända metodavvikelse, källänkarna på de 14
   varianterna.
...
## 6. Sammansatt aktiveringspreflight — samtliga 45 `ready`-ID:n (granskning 2026-09-08-005, P1)
...
1. **Utrednings-/request-status** — `investigation.status != "utreds"` OCH tariffens
   medlem/tariff-ID inte längre täcks av en öppen `remaining_information_requests`-post
   (§7, ny `tariff_ids`-representation).
2. **`grind(tariff, utredda)` → `None`** — den befintliga strukturella kontrollen
   (`katalog.py`), oförändrad kod.
...
- **Steg 3 (generatorns kontraktsgrind):** samtliga 45 saknar I DAG en registrerad
  `Tariffpolicy` (`POLICYREGISTER` innehåller bara de 7 redan implementerade + Sandviken).
  Samtliga 45 kräver alltså EN NY policyregisterpost vardera innan generatorn accepterar
  dem — detta gäller ÄVEN de 31 raderna märkta "leverantörsvärde-mönstret räcker" i v5,
  som förväxlade "inget nytt MOTORARBETE" med "ingen ny POLICY". `sundsvall-energi-indal-
...
|---|---|---|
| `borlange-energi-borlange-2026`, `c4-energi-kristianstad-2026` | `supplier_confirmed_band_id`-bindning (idag: `_niva()` väljer band automatiskt, `band_selection` når aldrig motorn) | 6a.2 |
| `falu-energi-vatten-bjursas-grycksbo-sundborn-svardsjo-2026` | `KravPost.maxvarde` (500 kW-tak; dagens `KravPost` har bara `minvarde`/`heltal`) | 6a.1 |
| `umea-energi-umea-enkel-2026` | Namngiven `kapacitet_multiplikator_bindning` för `B`, min/max `[0,93; 1,4]` | 6a.3 |
| `stockholm-exergi-stockholm-exergi-normal-2026` (katalogdubblett) | EXKLUDERAS explicit — se 6a.4 för vald väg (utökad leverantörsfilsadapter, inte katalogaktivering) | 6a.4 |
| Övriga 40 rader | Endast en ny `Tariffpolicy`-post (steg 3) — inget nytt kontraktsfält | — |
...
`harledResultatstatus`. `policyFranGenererad`/`_policy_till_json` behöver ingen ändring —
`dataclasses.asdict` serialiserar det nya fältet automatiskt, precis som `minvarde`/`heltal`
...
### 6a.2 `supplier_confirmed_band_id`-bindning

Ett NYTT, separat `KravPost`/`IndataPost`-fält (`nyckel="effektband_id"` eller likvärdigt),
skilt från det numeriska kapacitetsvärdet som redan binds via `kapacitet_bindning`. Motorn
validerar att det angivna band-ID:t finns i `prisar.kapacitet.nivaer[].id` och att det
numeriska värdet FAKTISKT ligger inom det bandets `min`/`max` — annars blockeras (inte
gissas). `_niva()` i `faktura.py`/`fjarrvarme.ts` rörs INTE (ändras aldrig för
auto-härledning); i stället läggs en kontroll FÖRE anropet till `_niva()` i
kontraktsfasaden (`resultatkontrakt.py`/`.ts`) som jämför det bekräftade band-ID:t mot vad
`_niva()} skulle ha valt automatiskt, och kastar om de skiljer sig åt.

**Används av:** `borlange-energi-borlange-2026` (band 5, `>501`) och
`c4-energi-kristianstad-2026` (band 6, `>500`) — de två rader där ett exakt gränsvärde
(501 respektive 500 kW) är den uttryckligen osäkra punkten en leverantör måste bekräfta.

### 6a.3 Umeås `kapacitet_multiplikator_bindning`

Nytt, namngivet fält på `Tariffpolicy` (INTE en generisk `post_multiplier`-vitlistning i
`grind()` — grinden ska fortsatt avvisa `post_multiplier` för alla andra tariffer). Python:
...
`kapacitetMultiplikatorBindning?: string`. Motorn (`faktura.py`/`fjarrvarme.ts`) läser det
bundna värdet som `B` och multiplicerar in det i kapacitetsformeln — motorn räknar ALDRIG
`U` (normalårskorrigerat förhållande), bara `B` som ett direkt leverantörsvärde.

**Intervallkrav:** `minvarde=0.93`, `maxvarde=1.4` på kravposten för `B` (katalogens egna
`pieces`-brytpunkter: `<0.300→0.93`, `0.800+→1.4`, med de två mellanliggande styckena som
gränser däremellan) — samma `minvarde`/`maxvarde`-mekanism som §6a.1, inget nytt
valideringskoncept. Test: golden-värden vid `B=0.93`, `B=1.4` och en punkt mellan
brytpunkterna; ett negativt test för `B=14` (utanför intervallet) ska blockera, inte räkna
en 10× för hög kapacitetskostnad. Testet kallas `B-intervall`, inte "U-intervall" — motorn
räknar aldrig `U`.

**Används av:** `umea-energi-umea-enkel-2026` — enda tariffen med `post_multiplier` i dagens
kontrollmängd.

### 6a.4 Stockholm Exergi — vald väg: utökad leverantörsfilsadapter, ingen katalogaktivering

Katalograden `stockholm-exergi-stockholm-exergi-normal-2026` AKTIVERAS INTE i denna plan.
I stället:

1. Leverantörsfilens redan verifierade `stockholm-exergi-2026` (tariff-ID `stockholm-
   exergi-2026`, skilt från katalogens `stockholm-exergi-stockholm-exergi-normal-2026`) får
   en NY, separat `Tariffpolicy` med `tackning: {"annual_forward"}` (utöver den befintliga
   `{"monthly_invoice"}`-policyn — två policyer för samma tariff-ID, en per täckningsområde,
   samma mönster som redan finns dokumenterat i `resultatkontrakt.py`s modulkommentar för
   flera täckningsområden).
2. Ett nytt `ADAPTERREGISTER`-register i `policyregister.py`: `dict[str, str]` som mappar
   ett katalog-tariff-ID (`stockholm-exergi-stockholm-exergi-normal-2026`) till det
   leverantörsfils-ID som faktiskt ska användas (`stockholm-exergi-2026`). Generatorn
   (`bygg_ts_fran_katalog`) läser detta register och HOPPAR ÖVER katalograden helt när ett
   register-ID matchar — explicit dedup, inte ett implicit ID-undantag i `katalog.py`.
3. `grind()` självt rörs INTE — katalograden fortsätter korrekt visa `energiform` som
   ...
   produkten ändå syns i kalkylatorn — under leverantörsfilens ID.
...
produkt-ID skild från leverantörsfilens `stockholm-exergi`. Utan `ADAPTERREGISTER`-hoppet
skulle en katalogaktivering skapa TVÅ val i UI för samma underliggande normalprodukt,
vilket bryter inventeringens egen dedupliceringsregel (§1).

## 7. Livscykel för `remaining_information_requests` som berör `ready`-raderna (granskning 2026-09-08-005, P1)

**Vald representation (ersätter v5:s "SPLITTA"/"eller"):** `remaining_information_requests`
i katalog-JSON:et får ett NYTT, valfritt fält `tariff_ids: list[str] | null` VID SIDAN AV
det befintliga `member_ids`. `utredda_medlemmar()` i `katalog.py` byggs om till en ny
funktion `oppna_tariff_ider(katalog) -> set[str]` som, för varje request: om `tariff_ids`
är satt, lägger till EXAKT de angivna tariff-ID:na (inte hela medlemmen); annars (fältet
`null`/saknas, bakåtkompatibelt för de requests som verkligen gäller alla en medlems
tariffer) faller tillbaka på nuvarande `member_ids`-beteende. `grind()` byts till att
kontrollera `tariff.get("id") in oppna_tariff_ider(...)` FÖRST, och faller bara tillbaka på
`tariff.get("member_id") in utredda_medlemmar(...)` för requests utan `tariff_ids`. Samma
representation används för SAMTLIGA 14 requests nedan — inget "eller" kvar i arbetsordern.
...
**Oförändrat i v6 (granskning `2026-09-08-005`):** dispositionsräkningen ovan står ÄVEN nu kvar oförändrad. v6 bytte grind-only-preflighten mot en fyrstegs sammansatt aktiveringspreflight, specificerade fyra konkreta kontraktstillägg (§6a) och bytte requestens datastruktur till `tariff_ids` (§7) — allt planeringsdjup som INTE flyttar någon post mellan `ready_to_implement`/`blocked_external_info`, eftersom samtliga 45 rader fortfarande bedöms genomförbara med källverifierat underlag, bara med mer specificerat kontraktsarbete kvar än v5 visade.
