# Batchplan v20.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v19.md` i sin helhet (v19 ändras INTE i
...
(`status: changes-required`, supersedes `2026-09-09-012`) av v19/`tariffinventering-v19.md`.
**Rättat P1 (granskning `2026-09-09-013`, fynd 3) — v19:s batchplansfil hade fortfarande
rubrik, föregångare, scope och innehåll från v18/v17; detta dokument är en genuint fräsch
v20-fil, byggd om från v19:s ändringar och v20:s nya fynd, inte en ytterligare selektiv
patch ovanpå v18-texten.** Bygger på dispositionerna i
[`tariffinventering-v20.md`](tariffinventering-v20.md) §4.1 (47 bastariffer) och §5 (10
...
blockerade / 92 totalt** — v20 gör bara Lidköpings implementationsplan körbar, flyttar ingen
post.

**Vad som är nytt i v20** (se granskning `2026-09-09-013` för fullständig motivering):

1. **`skapaTariffpolicy()`s options-typ OCH returvärde utökas med ALLA åtta planerade
   `Tariffpolicy`-fält i EN ändring** (Batch 0 punkt 5, se `tariffinventering-v20.md` §6a.1):
   `kapacitetBandBindning`, `kapacitetMultiplikatorBindning`, `flodeskorrigeringVariant`,
   `kallenergiArsserieBindning`, `stodjerAktuellArskostnad`,
   `returtemperaturArsserieBindning`, `ersatterKatalograd`, `stodjerBesparing` — inte bara
   det sista fältet som v19 begränsade sig till. v19:s felaktiga påstående om TypeScripts
   excess-property-check är rättat och ersatt med ett reproducerat `TS2353`-fel.
2. **Attesteringskanalen är nu fullständigt typad genom hela produktkedjan** (Batch 0 punkt
   2, se `tariffinventering-v20.md` §6a.2/§6a.7.6): ny `policyFaltAttestering: Record<string,
   boolean>`, trådad `KalkylatorPage.tsx` → `KalkylatorInputs`/`Tariffberakningsunderlag`/
   `BesparingsvardeArgs` → `argsFranInputs` → `byggKontraktIndata`/`byggIndataFranPolicy` →
   `IndataPost.attesterad`. `forkontrolleraPolicyIndata` ger nu `'ej_attesterat'` fältnära.
3. **Denna fil är ett genuint fräscht v20-dokument** (rättar P1 #3): korrekt rubrik,
   föregångare, inventeringslänk, dokumentationsscope; den fiktiva
   `besparingsvarde.ts`/"motsvarande Pythonfil"-referensen i Batch 5d borttagen.
4. **`PolicyFaltMetadata`s objektform och Sandvikens sidtest rättade** (rättar P1 #4):
   `kravAttestering: boolean` i den utskrivna objektformen; Sandvikens test motiverar sin
   gröna väg med `stodjerBesparing === true`, inte det tidigare `!== false`-språket.
5. Samtliga v17–v19-fynd (t.o.m. granskning `2026-09-09-012`) som INTE fick ett nytt fynd i
   granskning `2026-09-09-013` är BEVARADE oförändrade — femparametersguarden, fail-closed
...
- **Innehåll:**
  ...
  5. **Rättat P1 (granskning `2026-09-08-009`) — egen, typad produkt-DTO i stället för det
     ...
     **Rättat P1 (granskning `2026-09-09-007`) — v15:s inventeringsdokument motsade denna
     ...
     tyst av `beraknaBesparingsvarde`.** V16 gör DENNA rads kontrakt (`policyFalt` som ett
     additivt, valfritt fält på `BesparingsvardeArgs`) till det enda, entydiga beslutet —
     speglat i `tariffinventering-v18.md` §6a.4. `beraknaBesparingsvarde(args)` skickar redan
     (verifierat, `besparingsvarde.ts` rad 361) hela `args` vidare till
     `beraknaBesparingsvardeKontrakt`, som byter sitt ensamma
     `kapacitetBindning`-kartinlägg mot `byggKontraktIndata(policy, prisar, kapacitetKw,
     args.policyFalt ?? {})` och kör `forkontrolleraPolicyIndata` på den byggda kartan —
     samma helper och sekvens som Lidköpings batch 5d (nedan) och Stockholms
     `beraknaArsprodukt` (Batch 7) delar. Legacyvägen fortsätter ignorera fältet, inte hela
     funktionen.
  ...
  10. `KravPost.tillatna_varden` (§6a.6) och `KravPost.maxvarde`/`Tariffpolicy.kapacitet_
      ...
      föregående punkt.
  11. **NYTT (rättat P1, granskning `2026-09-09-013`) — `skapaTariffpolicy()`s options-typ
      OCH returvärde utökas med ALLA ÅTTA planerade `Tariffpolicy`-fält i SAMMA ändring**
      (§6a.1): punkt 10:s sex bindningsfält plus `stodjerAktuellArskostnad`/
      `stodjerBesparing` läggs samtliga till i `opts`-parametern och returobjektet, i BÅDA
      språken, i EN commit — inte punktvis över flera versioner som v19 gjorde med bara
      `stodjerBesparing`. `policyFranGenererad()` använder exakt samma åttafältiga
      `opts`-objekt. Testat direkt konstruktion OCH genererad policy för samtliga åtta
      fält, samt det reproducerade `TS2353`-felet som bevisar varför denna utökning är
      obligatorisk, inte valfri.
  12. **NYTT (rättat P1, granskning `2026-09-09-013`) — attesteringstransport genom hela
      produkt-DTO-kedjan** (§6a.2): ny parallell karta `policyFaltAttestering:
      Record<string, boolean>` (Python: `policy_falt_attestering: Mapping[str, bool] |
      None`), trådad `KalkylatorPage.tsx` (ny checkbox-state) →
      `KalkylatorInputs.policyFaltAttestering` → `Tariffberakningsunderlag`/
      `BesparingsvardeArgs.policyFaltAttestering` → `argsFranInputs`s passthrough →
      `byggKontraktIndata`/`byggIndataFranPolicy` → `IndataPost.attesterad`. Ny
      `PolicyValideringsOrsak`-medlem `'ej_attesterat'`, given av
      `forkontrolleraPolicyIndata` för ett närvarande men icke attesterat
      `kravAttestering`-krav, oberoende av `harledResultatstatus`s auktoritativa kontroll
      (oförändrad sedan v19, §6a.7.6).
...
`tariffinventering-v18.md` §6a.7 för fullständig design, formel, fail-closed-kontrakt och
...
- **Tariffer:** `lidkoping-energi-lidkoping-041-kw-2026`, `lidkoping-energi-lidkoping-42-kw-2026`
  ...
  `tariffinventering-v18.md` §4.2 och §8).
...
adderas: 55 + 2 = **57**, exakt matchande `tariffinventering-v17.md` §8: 47 bastariffer +
