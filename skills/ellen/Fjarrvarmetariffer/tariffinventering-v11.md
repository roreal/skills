# Tariffinventering v11.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v10.md` i sin helhet (v10 ändras
...
(`status: changes-required`, supersedes `2026-09-08-009`) av v10. Ingen produktkod, tariffdata
...
**Vad som är nytt i v11, i korthet** (se granskning `2026-09-09-001` för fullständig
...
1. **Rättat P1 (granskning `2026-09-08-009`) — separat, typad produkt-DTO i stället för att
   ...
   **Löst med två separata typer:**
   ...
   - `PolicyInputValue = number | string | readonly number[]` (oförändrad typ, men nu ENDAST
     ...
     PolicyInputValue | PolicyValideringsFel` (`resultatkontrakt.ts`), anropad först vid
     submit (samma tidpunkt som `faltForBerakning` redan byggs i dag):
...
2. `KalkylatorPage.tsx` bygger `policyFalt: Record<string, PolicyInputValue>` VID SUBMIT
   genom att köra `parsaPolicyIndata` (punkt 1) över varje nyckel i `policyFaltRavarden` mot
   dess `policyFaltMetadata`-post (punkt 5) — precis samma tidpunkt som `faltForBerakning`
   redan byggs i dag. Ett `PolicyValideringsFel` härifrån stoppar submit direkt och mappas
   till `KontraktBlockerat.ogiltigaFalt` (punkt 6) UTAN att ens nå `besparingsvarde.ts` —
   `byggIndataFranPolicy` (nästa) tar därför bara emot REDAN validerade `PolicyInputValue`.
...
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel, genom en
   ...
   **Löst med en separat valideringsfunktion FÖRE fasadanropet (Codex alternativ A),** i
   ...
   icke-tom talserie för `number_series`, icke-tom sträng för `band_id`), `antalVarden` för
   seriekrav, `tillatnaVarden`-medlemskap för enum, och — för `band_id_val` specifikt —
   slår upp värdet mot `prisar.kapacitet.nivaer[].id` och returnerar `{nyckel, orsak:
   'okant_val'}` om ingen träff finns. Returnerar en TOM lista om allt är giltigt (den
   vanliga, gröna vägen fortsätter oförändrat till `harledResultatstatus`); en ICKE-TOM
   lista `{nyckel: string; orsak: 'typ' | 'numerik' | 'kardinalitet' | 'okant_val'}[]`
   STOPPAR anropet direkt.
   ...
   det generiska felmeddelandet. Testat identiskt i BÅDA språken: tom/delvis ifylld serie,
...
def bygg_ts(filer, ..., policyregister=POLICYREGISTER, adapterregister=ADAPTERREGISTER):
    ...
    kontrollera_adapterpreflight({"tariffs": []}, leverantorer, policyregister, adapterregister)
    # ... resten av funktionen oförändrad.
```

**Rättat P1 (granskning `2026-09-09-001`) — ETT anropskontrakt för `bygg_ts()`, ingen låtsad
tom kontext.** v10 sa att `bygg_ts()` skulle köra preflighten med `adapterregister={}` "om
inga adapterposter är relevanta i det anropet". Verifierat att det INTE stämmer i praktiken:
`_bearbeta_leverantorsfil()` bygger Stockholms `ersatter_katalograd`-markör OVILLKORAT in i
leverantörsfilens policy, oavsett vilket adapterregister som injiceras — markören är en
egenskap hos POLICYN (satt i `policyregister.py`), inte hos anropet. Ett tomt
`adapterregister={}` skulle därför få riktning 2 att se markören men ett tomt `adapterade_mal`
och kasta, trots att `bygg_ts()` bygger EXAKT samma leverantörsfil som
`bygg_ts_fran_katalog()`. Den "tomma kontext"-vägen kan alltså aldrig lyckas mot det verkliga
produktionsregistret — den bevisade ingenting.

**Löst genom att ta bort undantaget helt:** `bygg_ts()` anropas ALLTID med det verkliga,
injicerade `adapterregister=ADAPTERREGISTER` (produktionsdefault) precis som
`bygg_ts_fran_katalog()` — den enda skillnaden mellan de två anropsvägarna är att `bygg_ts()`
skickar en TOM katalog-`tariffs`-lista (`{"tariffs": []}`), inte ett tomt adapterregister.
Riktning 1 (varje `ADAPTERREGISTER`-post pekar på en verklig kedja) itererar då noll
katalograder att slå upp `rad = next(...)` mot — men Stockholms enda adapterpost pekar på ett
KATALOG-ID, inte en leverantörsfilspost, så riktning 1 för just den posten SKULLE kasta
`"finns inte i katalogen"` om `bygg_ts()` någonsin faktiskt behövde validera den fullt ut.
Det är korrekt och avsiktligt: `bygg_ts()` (leverantörsfiler utan katalog) kan inte
självständigt bevisa att en `ADAPTERREGISTER`-post pekar på en verklig katalograd, eftersom
den katalograden per definition inte finns i dess anrop — så `bygg_ts()` används INTE för
leverans av en katalogberoende adapter som Stockholms; den vägen kräver alltid
`bygg_ts_fran_katalog()`. `bygg_ts()` förblir korrekt för leverantörsfiler UTAN
`ersatter_katalograd` (riktning 2 fortsätter fungera identiskt eftersom den bara läser
leverantörernas egna policyer) — vilket är den legitima, tydligt avgränsade standalone-vägen.
...
**Löst med en delad indatabyggare och tre explicita grenar:**

```ts
export type ArsprodukResultat =
  | { typ: 'besparing'; besparingsvarde: Besparingsvarde }
  | { typ: 'aktuell_arskostnad'; kostnad: number; leverantor: string; prisar: number;
      kapacitetKw: number; status: Resultatstatus }; // Stockholm-fallet
```
...
`beraknaBesparingsvardeKontrakt` redan hade), så BÅDA de nedanstående grenarna får en
komplett indatakarta — kapaciteten läggs inte längre in bara inuti EN av de tre grenarna.
...
produktentrypunkten `calcResult`/den nya sidnivå-wrappern (nedan) anropar för en
kontraktsgated tariff. Dispatch-logik, TRE grenar:

1. **`onskadTyp === 'besparing'` OCH `policy.kallenergiArsserieBindning` är satt** (Stockholms
   fall, den enda tariffen i denna kontrollmängd med bindningen): kastar det typade
   produktbegränsningsfelet (INTE `KontraktBlockerat` — det här är ingen kunddatalucka, det
   är att den efterfrågade PRODUKTFÖRMÅGAN saknar en källverifierad regel).
2. **`onskadTyp === 'besparing'` OCH policyn saknar `kallenergiArsserieBindning`** (Sandviken
   och alla framtida kontraktstariffer utan Stockholms serie): anropar `byggKontraktIndata`
   en gång, kör den BEFINTLIGA kontraktsgated före-/efterfunktionen (samma
   `beraknaBesparingsvardeKontrakt`-kärna som redan finns för Sandviken, orörd av detta
   tillägg) och returnerar `{ typ: 'besparing', besparingsvarde }` där `besparingsvarde` är
   HELA det befintliga `Besparingsvarde`-kontraktet (leverantör, prisår, kapacitet, andel av
   notan, status) — inte bara tre kostnadstal som skulle tvinga anroparen att återskapa
   resten av informationen.
3. **`onskadTyp === 'aktuell_arskostnad'`** (oavsett `kallenergiArsserieBindning` — gäller
   Stockholm i denna kontrollmängd, och framtida tariffer som bara stödjer denna gren):
   anropar `byggKontraktIndata` en gång, ETT anrop till `beraknaArskostnadMedKontrakt`,
   returnerar `{ typ: 'aktuell_arskostnad', kostnad: resultat.kostnad.summaInkl, leverantor,
   prisar: prisar.ar, kapacitetKw, status: resultat.status }` — `status.noggrannhet` bär
   `'snapshot'`-proveniensen (§6a.2s `harled_resultatstatus`-regler, ingen ny logik).
...
export function calcResultForOnskadTyp(
  inputs: KalkylatorInputs, onskadTyp: 'besparing' | 'aktuell_arskostnad'
): KalkylatorResultUnion {
  if (onskadTyp === 'aktuell_arskostnad') {
    const { prisar } = valjLeverantorOchPrisar(inputs.leverantorId);
    const resultat = beraknaArsprodukt({ ...argsFranInputs(inputs), onskadTyp });
    if (resultat.typ !== 'aktuell_arskostnad') throw new Error('oväntad gren');
    return { typ: 'aktuell_arskostnad', ...resultat };
  }
  return { typ: 'fullstandig', resultat: calcResult(inputs) };
}
```

Ett nytt, deterministiskt fält `onskadTyp: 'besparing' | 'aktuell_arskostnad'` läggs till i
`KalkylatorInputs` — KÄLLAN är ett explicit kundval i formuläret (en radioknapp/växel bredvid
leverantörsvalet, synlig bara när den valda tariffen faktiskt stödjer BÅDA grenarna, annars
låst till den enda stödda), inte en implicit produktförmåga — så samma tariff kan efterfrågas
på olika sätt av olika anrop utan att UI:t behöver gissa.

`KalkylatorPage.tsx` anropar `calcResultForOnskadTyp` (inte `calcResult` direkt) och grenar
PÅ RESULTATETS `.typ` INNAN några besparings-/paybackfält läses: `typ: 'fullstandig'` renderar
den befintliga, oförändrade resultatsidan; `typ: 'aktuell_arskostnad'` renderar en EGEN
resultatsektion (kostnad + "uppskattning" per verifieringsspråket i `PROJECT_CHARTER.md` §3)
UTAN besparingsfält — ingen tom eller påhittad `besparingKr` visas någonsin, eftersom den
grenens typ strukturellt SAKNAR fältet. Kr- och schablonlägena förblir blockerade som redan
...
**Sidtest (utökat, granskning `2026-09-09-001`):** väljer Stockholm i den RIKTIGA
...
besparingssiffra synlig (via `typ: 'aktuell_arskostnad'`-grenen). Ett separat test begär
`onskadTyp: 'besparing'` för Stockholm och verifierar produktbegränsningsfelet (sidnivå-notis,
inte ett fältnära formulärfel). Ett TREDJE test (nytt, stänger regressionsrisken) begär
`onskadTyp: 'besparing'` för Sandviken och verifierar att `{typ:'besparing',
besparingsvarde}`-grenen fortfarande ger en komplett `Besparingsvarde` (regression mot att
v10:s enda `annars`-gren skulle ha gjort denna väg onåbar).
