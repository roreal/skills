# Tariffinventering v7.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-08 av Claude. Ersätter `tariffinventering-v6.md` i sin helhet (v6 ändras
INTE i efterhand — kvar som historik), som svar på Codex omgranskning
[2026-09-08-006](../conversations/reviews/2026/09/2026-09-08-omgranskning-tariffinventering-v6.md)
(`status: changes-required`, supersedes `2026-09-08-005`) av v6. Ingen produktkod, tariffdata
eller genererad fil är ändrad av detta dokument. Alla kodpåståenden nedan är verifierade
genom att läsa och köra den verkliga koden i `enkey-agents@fd8f8da` (skrivskyddat, ingen fil
ändrad — bl.a. en fristående Python-kontroll av `capacity.band_selection` för samtliga 45
`ready`-rader mot den checkade-in katalogen) — inte bara omskrivna från granskningen.

**Vad som är nytt i v7, i korthet** (se granskning `2026-09-08-006` för fullständig
motivering till varje punkt):

1. **Band-ID-kontraktet omfattar nu alla 42 berörda `ready`-rader, inte två.** Egen
   maskinkontroll mot katalogen bekräftar Codex tal exakt: 42 av de 45 `ready`-raderna bär
   `capacity.band_selection: supplier_confirmed_band_id_required`; enda undantagen är
   Finspång, Mälarenergi 2–4 lägenheter och Sundsvall Indal/Liden/Lucksta (ingen
   kapacitetsdel eller redan täckt av en annan mekanism). §6a.2 är omskriven med en komplett
   42-radstabell (tariff-ID, antal band, källans band-ID:n) i stället för att bara nämna
   Borlänge/C4. Mekanismen är också omkonstruerad: bandvalet SKA styra prisraden direkt
   (`till_prisar` bevarar `band["id"]` in i `nivaer`, en ny `Tariffpolicy.kapacitet_band_bindning`
   pekar ut vilket krävt fält som bär det bekräftade band-ID:t, och en ny
   fasadparameter `vald_niva_id` i motorn hoppar över `_niva()`:s automatiska
   intervalltolkning helt när ett band-ID är bekräftat) — INTE, som v6 föreslog, att först
   bekräfta ett band-ID och sedan kasta om `_niva()` skulle valt något annat, vilket hade
   gjort leverantörens besked verkningslöst i precis de tvetydiga fallen det finns till för.
2. **Stockholm Exergis policymodell är nu representerbar.** `POLICYREGISTER` är
   `dict[str, Tariffpolicy]` — v6:s förslag om en andra, separat policy för samma
   `stockholm-exergi-2026`-nyckel hade tyst skrivit över den befintliga `monthly_invoice`-
   policyn. §6a.4 specificerar i stället EN konsoliderad `Tariffpolicy` för
   `stockholm-exergi-2026` med båda täckningsändamålen (`monthly_invoice` OCH
   `annual_forward`) och ändamålsspecifika krav (`kravs_for=("monthly",)` respektive
   `("annual",)` per fält). `ADAPTERREGISTER` typas nu med `provider_id`, `tariff_id` och
   `kravd_tackning: "annual_forward"` — generatorn kastar om målpolicyn saknar den täckningen,
   i stället för att bara hoppa över katalograden blint. Preflighten (§6) redovisar nu
   uttryckligen 44 katalogaktiveringar + 1 leverantörsfilsaktivering, inte 45 identiska
   `grind()`-vägar.
3. **Verklig utrednings-/request-mutation och ett körbart request-API.** Egen kontroll
   bekräftar Codex siffror exakt: 45/45 har `investigation.status: utreds`,
   `production_ready: false` och saknar `contract_required`. §6 anger nu den konkreta
   statusövergången (`investigation.status: null`, `contract_required: true`) för var och en
   av de 44 katalograder som ska aktiveras (Stockholms dubblett hålls separat, förblir
   `utreds`/blockerad). §7 väljer EN request-representation rakt igenom: varje öppen
   `remaining_information_requests`-post får ett explicit `tariff_ids`-fält (vid sidan av
   `member_ids`, för bakåtkompatibilitet med redan medlemsomfattande poster som inte delats
   upp); en ny `blockerade_tariff_ider(katalog)` löser upp BÅDA fälten till en enda mängd
   tariff-ID:n, och `grind(tariff, blockerade_tariff_ider)` byter jämförelse från
   `tariff.member_id in utredda` till `tariff.get("id") in blockerade` — en syskontariff hos
   samma leverantör kan då vara `ready` medan en namngiven blockerad syskontariff förblir det.
4. **Umeås aktiveringsgrind är arkitektoniskt separerad från den strukturella kataloggrinden.**
   v6 lät `grind()` självt läsa `kapacitet_multiplikator_bindning` trots att `grind()` varken
   tar en policyparameter eller har registeråtkomst — olöst i v6. §6a.3 inför i stället en
   ny funktion `kontrollera_kompositgrind(tariff, policy)` som körs EFTER `grind()`/generatorns
   kontraktsgrind, tar policyn som explicit argument, och bara då tillåter
   `capacity.post_multiplier` när `policy.kapacitet_multiplikator_bindning` är satt — den
   nakna `grind()` fortsätter avvisa varje `post_multiplier` som `"kapacitetsformel med
   multiplikator"`, som i dag. Intervallet är omräknat: `1,34×U+0,330` ger `1,40066` vid
   `U=0,799`, så ett absolut tak `maxvarde=1,4` hade kunnat avvisa ett giltigt, bara olika
   avrundat leverantörsvärde — `maxvarde` är därför satt till `1,401` (fyra värdesiffror,
   samma precision som leverantörens publicerade formel) med den avrundningsregeln
   dokumenterad explicit i stället för ett gissat exakt tak.
5. **TypeScript-deserialiseringen är nu fullständigt specificerad.** `dataclasses.asdict`
   serialiserar `maxvarde`/`kapacitet_band_bindning`/`kapacitet_multiplikator_bindning`
   automatiskt på Python-JSON-sidan, men TypeScripts `policyFranGenererad()`
   (`resultatkontrakt.ts`) mappar fält FÖR HAND och läser i dag bara `minvarde`/`heltal` —
   verifierat genom att läsa funktionen. §6a listar nu varje nytt fälts Python-namn,
   JSON-namn och den saknade TypeScript-mappningen (`maxVarde: k.maxvarde`,
   `kapacitetBandBindning: k.kapacitet_band_bindning`,
   `kapacitetMultiplikatorBindning: k.kapacitet_multiplikator_bindning`) som en explicit
   ändring i planen, inte en implicit automatik.
6. **Per-produktmatrisen synkroniserad mot besluten.** Stockholm-, Sundsvall Indal- och
   Umeå-raderna (de tre v6 lämnade omskrivna från v5) är rättade till exakt §6/§6a/§7:s
   modell. Ingen av de 42 bandraderna beskriver längre automatisk effektbandstolkning som
   tillräcklig — varje sådan rad hänvisar nu till den konsoliderade 42-radstabellen i §6a.2 i
   stället för att upprepa kontraktstexten 42 gånger.
7. **Jönköpings accessavgift — Codex/Roberts beslut tillämpat:** variantens disposition
   flyttad till `ready_to_implement` som ett synligt, obligatoriskt kundval (0/10/25/50
   kr/månad, inget standardvärde, okänt val blockerar), ingen dubblettprodukt. Ny
   variantfördelning: 10 redo, 4 blockerade av 14. **Ny totalsumma: 7 implementerade + 55
   redo + 30 blockerade = 92** (upp från 7+54+31 i v6).
8. **Övriga Codex-beslut på v6:s öppna frågor tillämpade utan omfrågan:** E.ON/Navirums
   batch 3b-ordning godkänd oförändrad; batch 5:s 5a/5b/5c-uppdelning godkänd men görs nu
   beroende av det gemensamma bandkontraktet (§6a.2) i stället för att vara fristående;
   Kraftringens parametriserade motortyp godkänd med kravet att regelvarianten är en
   explicit, typad diskriminator (inte härledd implicit från leverantörs-ID) — specificerat i
   §6a.5.
9. **P2 rättat:** `POLICYREGISTER`s nulägesbeskrivning korrigerad (tre poster i dag —
   Stockholm 2025, Stockholm 2026, Sandviken — inte "de 7 + Sandviken"; de sex andra
   legacy-produkterna har ingen policy). Samtliga batchers fillistor i `batchplan-v7.md`
   räknade om mot det nya §7-kontraktet så att varje borttagen/omskopad request har sin
   requestfil med. v6-korrigeringscommitten `058ffb4` (som saknades i sessionsloggens
   ändringshistorik) loggad som en egen daterad rad utan att skriva om den äldre v6-raden.
10. **Oförändrat från v6 (redan korrekt och återverifierat):** 78 katalog-ID:n, 14
    variant-ID:n (nu 10/4 efter Jönköping), Borlänge/C4/Falus exakta gränsfallsbugg
    (återverifierad med Python 3.13 mot den verkliga katalogen: Borlänge 501→band 5 idag,
    C4 500→band 6 idag, Falu ytterorter 501→`ValueError`), Lidköpings kända metodavvikelse,
    källänkarna på varianterna.
...
- **Inmatningslägen:** mwh (obligatorisk indata krävs); kr och schablon BLOCKERAS (ingen verifierad invers/schablonmodell)
- **Tariffamilj/adapter:** Kraftringen — samma parametriserade motortyp som E.ON/Navirum, med `kapacitet_bindning_variant: "golvbegransad"` som explicit, typad diskriminator (§6a.5) — INTE härledd implicit från leverantörs-ID
...
## 6. Sammansatt aktiveringspreflight — samtliga 45 `ready`-ID:n (granskning 2026-09-08-005/-006, P1)
...
- **Steg 1+2 (grind-nivå):** FÖRE alla rättningar: 21/45 passerar. EFTER v5/v6:s
  katalog-/motorrättningar (simulerade i minnet): 45/45 passerar `grind()`. Detta är
  ENDAST steg 2 av 4 — samma resultat v5 redan visade, nu uttryckligt namngivet som
  ofullständigt.
- **Steg 3 (generatorns kontraktsgrind):** samtliga 45 saknar I DAG en registrerad
  `Tariffpolicy`. Rättat P2 (granskning `2026-09-08-006`): `POLICYREGISTER` innehåller i
  verkligheten TRE poster — Stockholm Exergi 2025, Stockholm Exergi 2026 och Sandviken; de
  sex ÖVRIGA redan implementerade legacy-produkterna (Göteborg, Gotland ×2, Halmstad,
  Mölndal, Norrenergi) har INGEN policy alls, eftersom de aktiverades före
  resultatkontraktet fanns och står i `LEGACY_UNDANTAGNA_TARIFF_ID`. Slutsatsen är ändå
  oförändrad: samtliga 45 `ready`-rader kräver EN NY policyregisterpost vardera innan
  generatorn accepterar dem — detta gäller ÄVEN de 39 av de 42 bandraderna (§6a.2) som inte
  behöver ytterligare motorarbete, vilket tidigare rundor förväxlade med "ingen ny policy
  krävs". `sundsvall-energi-indal-
  liden-och-lucksta-2026` är INTE i `LEGACY_UNDANTAGNA_TARIFF_ID` (verifierat: bara de sex
  ursprungliga uppgift-7-tarifferna finns i den frozensetten) — v5:s batch 2 hade fått
  `bygg_ts_fran_katalog()` att kasta om den byggts. Sundsvall Indal behöver samma minimala
  policymekanism som Sandviken (`contract_required: true` + en `Tariffpolicy` med
  `capacity.type: not_applicable`, inga kapacitetsbundna krav) — inte legacy-vägen.
- **Steg 4 (minimalt annual_forward-anrop):** kräver att varje rads `Tariffpolicy` faktiskt
  ...
  explicit som ÅTERSTÅENDE arbete i V7/implementationsfasen, inte påstått klart.
...
## 6a. Kontraktstillägg som krävs (granskning 2026-09-08-005, P1)
...
Negativa test krävs för samtliga tre: en genererad policy med fältet satt som TypeScript
...
2. Nytt `Tariffpolicy`-fält `kapacitet_band_bindning: str | None = None` (mirror
   ...
   `KravPost`, t.ex. `nyckel="effektband_id"`, `Varde` utökat till
   `float | Sequence[float] | str`) som bär det leverantörsbekräftade band-ID:t. Skilt från
   det befintliga numeriska fältet bakom `kapacitet_bindning` — bandvalet och det numeriska
   debiteringsunderlaget är två oberoende krävda fält.
...
### 6a.3 Umeås `kapacitet_multiplikator_bindning` — arkitektoniskt separerad grind (rättat P1)

**Rättat designfel (P1, granskning `2026-09-08-006`):** v6 lät `grind()` självt läsa
`kapacitet_multiplikator_bindning`, men `grind(tariff, blockerade_tariff_ider)` (§7) tar
varken en policyparameter eller har registeråtkomst — ingen arkitektur fanns för HUR
kontrollen skulle göras, vilket lämnade valet mellan fortsatt blockering och ett
hårdkodat/allmänt undantag för varje `post_multiplier`. Löst genom att skilja den
STRUKTURELLA kataloggrinden från den SAMMANSATTA aktiveringsgrinden helt:
...
1. Den nakna `grind()` (`katalog.py`) ändras INTE — den fortsätter avvisa varje
   ...
   alla tariffer inklusive Umeå. Umeå passerar alltså ALDRIG steg 2 (§6) ensamt.
2. Ny funktion `kontrollera_kompositgrind(tariff: dict, policy: Tariffpolicy) -> str | None`
   (ny fil eller i `policyregister.py`, körs EFTER steg 2+3 i §6, aldrig i stället för dem):
   tar policyn som explicit argument. Om `tariff["capacity"].get("post_multiplier")
   is not None`, kräver funktionen att `policy.kapacitet_multiplikator_bindning is not
   None` OCH att den bundna `KravPost` finns i `policy.kravda_falt` — annars avslagsorsak.
   Detta är den EXPLICITA, typade capability-kontrollen som saknades i v6; den nakna
   grinden godkänner aldrig en okänd multiplikator på egen hand.
3. Motorn (`faktura.py`/`fjarrvarme.ts`) läser det bundna värdet som `B` och multiplicerar
   in det i kapacitetsformeln, precis som v6 beskrev — motorn räknar ALDRIG `U`
   (normalårskorrigerat förhållande), bara `B` som ett direkt leverantörsvärde. Python:
   `kapacitet_multiplikator_bindning: str | None = None` på `Tariffpolicy`, validerat mot
   `kravda_falt` precis som `kapacitet_bindning`/`kallenergi_bindning`. TypeScript-spegel:
   `kapacitetMultiplikatorBindning?: string` — mappningsraden i §6a.1s tabell.

**Intervallkrav, omräknat (rättat P1):** den publicerade mellanformeln `1,34×U+0,330` ger
`1,40066` vid `U=0,799` (katalogens övre brytpunkt) — ett absolut, avrundat tak
`maxvarde=1,4` hade därför kunnat avvisa ett giltigt leverantörsvärde beroende på HUR
leverantören själv avrundar sin egen publicerade formel. Kravet sätts i stället till
`minvarde=0.93`, `maxvarde=1.401` — fyra värdesiffror, en (1) enhet extra marginal på sista
decimalen mot den exakta formelutledda gränsen `1,40066`, med regeln dokumenterad explicit
i policykommentaren i stället för att gissa ett exakt tak. Om leverantören i framtiden
publicerar `B` med fler decimaler ska gränsen omprövas mot den nya precisionen, inte antas
vara ett runt tal. Test: golden-värden vid `B=0.93`, `B=1.401` och en punkt mellan
brytpunkterna; ett negativt test för `B=14` (uppenbart utanför intervallet, en
decimalkommafel-klass av fel) ska blockera via `kontrollera_kompositgrind`, inte räkna en
10× för hög kapacitetskostnad. Testet kallas `B-intervall`, inte "U-intervall" — motorn
räknar aldrig `U`.
...
### 6a.4 Stockholm Exergi — EN konsoliderad policy, inte två poster med samma nyckel (rättat P1)

**Rättat designfel (P1, granskning `2026-09-08-006`):** v6 föreslog en ANDRA, separat
`Tariffpolicy`-post för samma tariff-ID `stockholm-exergi-2026`. `POLICYREGISTER` är
verifierat `dict[str, Tariffpolicy]` (läst i `policyregister.py`) — två poster med samma
nyckel kan inte samexistera i en dict, den senare skriver tyst över den tidigare. v6:s väg
hade alltså antingen förlorat den redan godkända `monthly_invoice`-policyn eller aldrig
registrerat den nya `annual_forward`-policyn, beroende på registreringsordning.

Katalograden `stockholm-exergi-stockholm-exergi-normal-2026` AKTIVERAS INTE i denna plan.
I stället:

1. Leverantörsfilens redan verifierade tariff-ID `stockholm-exergi-2026` (leverantörs-ID
   `stockholm-exergi` — RÄTTAT P1: v6 kallade felaktigt `stockholm-exergi-2026` själva
   leverantörsfils-ID:t; det ÄR tariff-ID:t, leverantörs-ID:t är `stockholm-exergi`) får
   sin BEFINTLIGA `Tariffpolicy` UTÖKAD — inte duplicerad — med en andra täckning:
   `tackning: {"monthly_invoice", "annual_forward"}` (i dag bara `{"monthly_invoice"}`).
   Kraven i `kravda_falt` blir ÄNDAMÅLSSPECIFIKA: varje `KravPost` som redan gäller
   `kravs_for=("monthly",)` behåller det, och nya/kompletterande krav med
   `kravs_for=("annual",)` läggs till för de fält årsvägen behöver (samma mönster som redan
   finns för att skilja `monthly`/`annual` på fältnivå inom EN policy, inget nytt
   registerkoncept).
2. Nytt `ADAPTERREGISTER`-register i `policyregister.py`: `dict[str, AdapterEntry]` (inte
   en bar `dict[str, str]`) där `AdapterEntry` typat bär `provider_id: str` (`"stockholm-
   exergi"`), `tariff_id: str` (`"stockholm-exergi-2026"`) och `kravd_tackning:
   Tackningsomrade` (`"annual_forward"`). Nyckeln är katalog-tariff-ID:t
   (`stockholm-exergi-stockholm-exergi-normal-2026`). Generatorn (`bygg_ts_fran_katalog`)
   läser registret för varje katalogtariff: om ett register-ID matchar, hoppar den över
   katalograden EFTER att ha verifierat att målpolicyn (`POLICYREGISTER[tariff_id]`)
   FAKTISKT finns OCH att `kravd_tackning in målpolicy.tackning` — saknas endera KASTAR
   generatorn (`raise`, aldrig en tyst tom rad), i stället för v6:s obevisade "hoppa över"
   utan verifiering. Explicit dedup med ett fail-closed kontrakt, inte ett implicit
   ID-undantag i `katalog.py`.
3. `grind()` självt rörs INTE — katalograden fortsätter korrekt visa `energiform` som
   avslagsorsak (`monthly_with_peak_volume_replacement` står inte i `FAS1_ENERGIFORMER`).
   Det är `ADAPTERREGISTER`-hoppet i generatorn, inte ett grindundantag, som gör att
   produkten ändå syns i kalkylatorn — under leverantörsfilens ID `stockholm-exergi-2026`,
   inte under katalogradens ID.

**Preflighten (§6) rapporterar därför 44 katalogaktiveringar + 1 leverantörsfils-
utökning** — inte 45 identiska `grind()`-vägar. Stockholms rad i §6:s per-tariffrad-tabell
markeras `EXKLUDERAS` (redan så i v6, oförändrat), men själva mekanismen som gör den
tillgängliga i UI:t ligger helt utanför den katalogdrivna aktiveringskedjan.
...
`ADAPTERREGISTER`-hoppet skulle en katalogaktivering skapa TVÅ val i UI för samma
underliggande normalprodukt, vilket bryter inventeringens egen dedupliceringsregel (§1).

### 6a.5 Kraftringens parametriserade motortyp — explicit, typad diskriminator (Codex-beslut tillämpat)
...
härledd från leverantörs-ID (`if member_id == "kraftringen"` vore just den implicita
härledningen förbudet gäller). Ny katalog-/policyfält:
`kapacitet_bindning_variant: Literal["golvfri", "golvbegransad"]` (namn preliminärt) på
policyn, satt explicit per tariff-ID i `policyregister.py` — E.ON/Navirums åtta rader
`"golvfri"` (`volym × base_rate × (0,02 × (Tf − 60) + 0,2)`), Kraftringen `"golvbegransad"`
(`volym × base_rate × max(0,2; 0,2 + (Tf − 60) × 0,02)`). Motorn (`faktura.py`/
`fjarrvarme.ts`) grenar på diskriminatorn, aldrig på leverantörs-ID direkt. En okänd eller
saknad diskriminator BLOCKERAR — motorn gissar aldrig vilken regel som gäller. Minst ett
golden-/gränstest per variant, inklusive Kraftringens golv vid exakt `Tf=60` (faktorn ska
vara `0,2`, inte `0,2+0=0,2` av en slump utan av att `max(0,2; 0,2) == 0,2`).

## 7. Livscykel för `remaining_information_requests` som berör `ready`-raderna (granskning 2026-09-08-005/-006, P1)
...
gäller, så `oppna_tariff_ider()` aldrig råkar öppna en produkt som ska förbli blockerad —
samma representation, inget "delas eller läggs till" kvar som öppet val.
...
**Samtliga fyra frågor v6 lämnade öppna är nu besvarade av Codex i granskning
`2026-09-08-006`s avsnitt "Beslut på V6:s öppna frågor" och tillämpade rakt av i denna v7
— ingen omfrågan:**
...
4. **Kraftringens parametriserade motortyp:** godkänd, UNDER FÖRUTSÄTTNING att
   ...
   leverantörs-ID). **Beslut tillämpat:** §6a.5 specificerar
   `kapacitet_bindning_variant`-diskriminatorn.
...
**Ingen ny öppen fråga identifierad i v7-rundan.**
