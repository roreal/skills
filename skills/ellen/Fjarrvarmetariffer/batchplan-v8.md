# Batchplan v8.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-08 av Claude. Ersätter `batchplan-v7.md` i sin helhet (v7 ändras INTE i
...
(supersedes `2026-09-08-006`). Bygger på dispositionerna i
[`tariffinventering-v8.md`](tariffinventering-v8.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v8** (se granskning `2026-09-08-007` för fullständig motivering):

1. **Ny batch 0** — grundkontraktet (diskriminerad värdetyp, bandbindning, serialisering,
   motorparameter, produkt-API, UI) som ett eget, granskningsbart steg FÖRE batch 1. Ingen
   tariffbatch med ett bandkrav (1, 3, 3b, 4, 5a, 5b, 5c, 6) kan börja förrän batch 0 är
   klar och testad.
2. **Umeås aktiveringsordning gjord körbar** (batch 4): `kontrollera_kompositgrind()`
   körs nu INUTI `godkanda()`s egen loop, som ett andra försök begränsat till fyndet
   `"kapacitetsformel med multiplikator"` — inte "efter" en loop som redan filtrerat bort
   raden. `B=14`-testet flyttat från kompositgrinden till kontraktsfasaden
   (`harled_resultatstatus`), eftersom bara den har en verklig `IndataPost` att pröva.
3. **Kraftringens diskriminator omdöpt och given en motorväg** (batch 3, §6a.5):
   `flodeskorrigering_variant` (domänriktigt namn, ersätter det missvisande
   `kapacitet_bindning_variant`), med en ny motorparameter som faktiskt bär den från
   fasaden till justeringsfunktionen.
4. **Stockholm Exergi (batch 7): adapterkontrollen är nu nåbar och dispatchen verklig.**
   `ADAPTERREGISTER` kontrolleras mot den RÅA katalogen (inte den redan filtrerade
   `godkanda()`-mängden), och dispatchen återanvänder den befintliga `_kraver_kontrakt`-
   markören i stället för ett obeskrivet nytt fält. En 17-fälts årsindatamodell (12
   kallenergivärden + 5 vintermånaders returtemperatur, som seriekrav) ersätter de
   tidigare fria motorargumenten.
5. **Jönköpings allow-list är nu en verklig domänregel** (batch 5b, §6a.6): ny
   `KravPost.tillatna_varden`, inte bara ett UI-val — ett direkt fasadanrop med t.ex.
   17 kr/mån blockeras nu mekaniskt.
6. **Dispositionerna 7/55/30/92 oförändrade** — samtliga v8-fynd var körbarhets-/
   typningsfel i redan beslutade kontraktsdesigner, ingen post flyttar.

## Batch 0 — Grundkontrakt: diskriminerad värdetyp, bandbindning och en gemensam produktingång (NY, granskning 2026-09-08-007, P1)

**Krävs FÖRE varje batch som bär ett bandkrav** (1, 3, 3b, 4, 5a, 5b, 5c, 6 — se
`tariffinventering-v8.md` §6a.2:s 42-radstabell). Utan detta grundarbete blir varje sådan
tariff `blocked`/felklassad som konfigurationsfel: `beraknaBesparingsvardeKontrakt`
(`besparingsvarde.ts`) bygger i dag `IndataPost` ENDAST för `kapacitetBindning`.
...
- **Innehåll:**
  ...
  2. `Tariffpolicy.kapacitet_band_bindning`, `till_prisar()` bevarar `nivaer[].id`, ny
     motorparameter `vald_niva_id` i `arskostnad`/`manadskostnad` (§6a.2).
  3. Ny hjälpfunktion `byggIndataFranPolicy(policy, falt)` i `besparingsvarde.ts`, kallad
     från `beraknaBesparingsvardeKontrakt` i stället för det nuvarande enda
     `kapacitetBindning`-inlägget (§6a.2).
  4. `KravPost.tillatna_varden` (§6a.6) och `KravPost.maxvarde`/`Tariffpolicy.kapacitet_
     multiplikator_bindning`/`Tariffpolicy.flodeskorrigering_variant` (§6a.1/6a.3/6a.5) —
     samtliga nya kontraktsfält samlas i SAMMA commit som grundarbetet, eftersom de delar
     samma `policyFranGenererad()`-mappningsbehov (§6a.1s tabell).
- **Filer:** `resultatkontrakt.py`/`.ts` (samtliga nya fält + `vardetyp`-gren i
  `harled_resultatstatus`/`harledResultatstatus`), `katalog.py` (`till_prisar`
  bandbevaring), `faktura.py`/`fjarrvarme.ts` (`vald_niva_id`-parameter),
  `besparingsvarde.ts` (`byggIndataFranPolicy`), `energiPotential.ts`/`KalkylatorPage.tsx`
  (oförändrad `falt`-vidareförmedling, verifierad med test), genererad testdata.
- **Teststrategi:** enhetstest per nytt `KravPost`-fält (giltigt/ogiltigt värde, kombination
  med förbjudna fält som `minvarde`+`band_id`), en `byggIndataFranPolicy`-svit som bevisar
  att ETT policykrav utan `falt`-motsvarighet ger `blocked` (inte `undefined`) och att ett
  extra `falt`-fält utan policykrav ignoreras, samt ett integrationstest med en syntetisk
  policy som har både ett numeriskt och ett band-ID-krav samtidigt.
- **Visas för användaren:** inget — infrastruktur utan egen UI-yta.
...
- **Modell:** TVÅ separata nya justeringstyper i `justeringar.py` (speglad inline i `fjarrvarme.ts`) —
  ...
  **RÄTTAT P1 (granskning `2026-09-08-006`) — den NAKNA `grind()` ändras INTE.** Den
  fortsätter avvisa varje post med `post_multiplier` (`kapacitetsformel med multiplikator`)
  för alla tariffer, oförändrat. I stället körs en NY, separat funktion
  `kontrollera_kompositgrind(tariff, policy)` EFTER `grind()`/generatorns kontraktsgrind
  (inventeringens §6a.3) — den tar policyn som explicit argument och godkänner
  `post_multiplier` bara när det NYA `Tariffpolicy`-fältet `kapacitet_multiplikator_
  bindning` är satt och den bundna `KravPost` finns i `kravda_falt`. `B` valideras mot
...
- **Filer:** katalograder ×4, `justeringar.py` (två nya formler, speglade inline i
  ...
  `kontrollera_kompositgrind()` samt den rättade `godkanda()`-loopen som anropar den
  INUTI sig själv (§6a.3 — den nakna `grind()` i `katalog.py` ändras INTE),
  `faktura.py`/`fjarrvarme.ts` (ny kapacitetsberäkning för Umeå), `policyregister.py` ×4.
- **Teststrategi:** golden-värde per tariff, gränstest för respektive formels
  ...
  multiplikator (ingen policy/bindning) också stoppas av samma funktion.
...
- **Modell:** en NY, ÅTSKILD årsindatamodell (inte de befintliga `monthly`-skopade
  `kall_energi_mwh`/`returtemperatur_c`-kraven, som `Tariffpolicy`s förbud mot dubblerade
  nycklar hindrar från återanvändning i en enda årssamlad indata): 12 `KravPost`
  `kall_energi_mwh_arsserie` (`rullande=True`, `vardetyp="number_series"`, exakt 12 element,
  kravs_for=`("annual",)`) och 5 `returtemperatur_c_vintermanader` (samma mönster, exakt 5
  element, november–mars). Kontraktsfasaden binder dessa till en NY, valfri
  `returtemp_c_per_manad`-parameter i den lågnivåkod som redan periodiserar per månad — det
  gamla enskalära `returtemp_c`-argumentet används INTE för denna väg. Bygg ett
  källverifierat årsreferensfall, motsvarande Sandvikens granskningskedja
  (`2026-09-06-004`→`2026-09-07-003`).
...
- **Filer:** `resultatkontrakt.py`/`.ts` (17 nya `KravPost`, `returtemp_c_per_manad`-
  parametern, seriekravens längdvalidering), `policyregister.py` (utökad
  `_stockholm_exergi_policy` + nytt `ADAPTERREGISTER`), `generera.py`
  (`bygg_ts_fran_katalog()`s nya rå-katalog-preflight + `_bearbeta_leverantorsfil()`s
  `_kraver_kontrakt`-sättning).
- **Teststrategi:** eget källverifierat referensfall (inte bara Åkermannen-fixturens
  ...
  blockerar), regressionstest att `monthly_invoice`-vägen är oförändrad, ett test att
  `ADAPTERREGISTER`-preflighten kastar på saknad/stale mappning eller policytäckning utan
  adapter, OCH ett test att generatorn producerar EXAKT ETT UI-val för Stockholm Exergi
  (inte två) efter preflighten.
