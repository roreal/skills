# Tariffinventering v8.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-08 av Claude. Ersätter `tariffinventering-v7.md` i sin helhet (v7 ändras
INTE i efterhand — kvar som historik), som svar på Codex omgranskning
[2026-09-08-007](../conversations/reviews/2026/09/2026-09-08-omgranskning-tariffinventering-v7.md)
(`status: changes-required`, supersedes `2026-09-08-006`) av v7. Ingen produktkod, tariffdata
...
`enkey-agents@fd8f8da`/`neptune_academy@f1df177` — bl.a. `bygg_ts_fran_katalog()`s faktiska
anropsordning (`godkanda(katalog)` FÖRE loopen), `beraknaBesparingsvardeKontrakt`s
`IndataPost`-uppbyggnad (endast `kapacitetBindning` i dag) och `_validera_varde`/
`valideraVarde`s strikta talkontroll — inte bara omskrivna från granskningen.

**Vad som är nytt i v8, i korthet** (se granskning `2026-09-08-007` för fullständig
motivering till varje punkt):

1. **Umeås aktiveringsordning är nu körbar.** v7:s `kontrollera_kompositgrind(tariff, policy)`
   skulle köras EFTER `godkanda(katalog)` redan filtrerat bort raden — verifierat mot den
   verkliga `bygg_ts_fran_katalog()`: den anropar `godkanda(katalog)` FÖRST och itererar
   sedan bara de rader `grind()` gav `None` för; en senare kontroll kan inte återinföra en
   redan bortfiltrerad rad (Codex reproducerade: `grind()` gav fortsatt
   `"kapacitetsformel med multiplikator"`, noll rader från `godkanda()`). §6a.3 är omskriven
   med exakt pseudokod för VAR kompositkontrollen anropas — INUTI `godkanda()`s egen loop,
   som ett andra försök bara för fyndet `"kapacitetsformel med multiplikator"`, aldrig för
   något annat fynd. `B=14`-testet är flyttat till indatakontraktet
   (`harled_resultatstatus` via `minvarde`/`maxvarde`), eftersom ingen verklig `IndataPost`
   finns vid det statiska aktiveringstillfället `kontrollera_kompositgrind` kör i.
2. **Bandkontraktet har en diskriminerad värdetyp i stället för en global strängvidgning.**
   v7 föreslog `Varde = float | Sequence[float] | str` rakt av — verifierat att det hade
   avvisat/förvirrat samtliga befintliga numeriska konsumenter (`_validera_varde`/
   `valideraVarde`, `float(post.varde)` i kontraktsfasaden, `Record<string, number>` i
   `BesparingsvardeArgs.falt`/`KalkylatorInputs.falt`, `parseFloat` på kalkylatorsidan).
   §6a.2 inför i stället `KravPost.vardetyp: "number" | "number_series" | "band_id"`
   (fältspecifik, inte global) — numeriska krav fortsätter avvisa strängar, bandkravet
   kräver en icke-tom sträng och förbjuder `minvarde`/`maxvarde`/`heltal`, och det validerade
   band-ID:t hamnar ALDRIG i motorns numeriska `falt`-dictionary, bara i den separata
   `vald_niva_id`-parametern.
3. **En gemensam produktingång för ALLA policykrav, inte bara kapacitet.** Verifierat genom
   att läsa `beraknaBesparingsvardeKontrakt` (`besparingsvarde.ts`): den bygger i dag
   `IndataPost` ENDAST för `kapacitetBindning` — varje policy med ett band-, flödes-,
   temperatur- eller B-krav utöver kapacitet blir därför `blocked` och felklassas som ett
   konfigurationsfel. §6a.2 lägger till detta som uttryckligt GRUNDARBETE (ny batch 0, se
   batchplanen) före första tariffbatchen: produktadaptern måste bygga `IndataPost` för
   EXAKT de policydeklarerade fälten, med rätt typ, och föra dem från `KalkylatorPage` via
   `energiPotential` och `besparingsvarde` till kontraktsfasaden.
4. **Stockholm Exergis adapterkontroll är nu nåbar och har en verklig årsdispatch.**
   v7:s `ADAPTERREGISTER`-kontroll skulle köras "när generatorn behandlar en katalogtariff"
   — men Stockholm-raden filtreras bort av `godkanda()` INNAN den når den loopen (`energiform`
   + `utreds`). §6a.4 flyttar kontrollen till en separat preflight mot den RÅA
   `katalog["tariffs"]`-listan, körd inne i `bygg_ts_fran_katalog()` direkt efter
   `ur_katalogen = godkanda(katalog)`, oberoende av om Stockholm-raden finns i den filtrerade
   mängden. Dispatchen återanvänder den REDAN BEFINTLIGA `_kraver_kontrakt`-markören (samma
   mekanism som redan tvingar varje annan kontraktsgated tariff genom fasaden) i stället för
   att hitta på ett nytt fält — markören sätts på leverantörsfilens prispost när dess policy
   får `annual_forward`-täckning. En ny årsindatamodell (12 namngivna månatliga
   kallenergi-`KravPost`, 5 namngivna månatliga returtemperatur-`KravPost`, alla
   `kravs_for=("annual",)`, skilda nycklar från de befintliga månadskraven) ersätter
   dagens fria `mwh_kallt_per_manad`/enda `returtemp_c`-argument.
5. **Kraftringens diskriminator omdöpt och given en verklig motorväg.** v7:s
   `kapacitet_bindning_variant` saknades i mappningstabellen och hade ingen beskriven väg
   till justeringsmotorn (fasaden skickar i dag bara numeriska `falt`, `faktura.py`/
   `fjarrvarme.ts` får aldrig policyn). §6a.5 döper om fältet till det domänriktiga
   `flodeskorrigering_variant` (det väljer en variant av `supply_temperature_adjusted_flow`,
   inte en kapacitetsbindning), lägger det i mappningstabellen, och specificerar den nya
   motorparametern som bär diskriminatorn från fasaden till justeringsfunktionen.
6. **Jönköpings fyrvärdesval har fått en domänmässig allow-list.** Ny §6a.6:
   `KravPost.tillatna_varden: tuple[float, ...] | None` (samma mönster som `minvarde`/
   `maxvarde`) gör 0/10/25/50 kr/mån till en verklig kontraktsregel i stället för bara en
   UI-begränsning — ett direkt fasadanrop med t.ex. 17 kr/mån blockeras nu mekaniskt.
   Explicit `0` skiljs från saknad indata genom att produktadaptern (grundarbetet i punkt 3)
   bara bygger en `IndataPost` när användaren faktiskt gjort ett val — inget värde byggs
   "tomt".
7. **Batch 0 — grundkontrakt, ny batch FÖRE batch 1** (`batchplan-v8.md`): täcker punkt
   2–3 ovan (värdetyp, bandbindning, serialisering, motorparameter, produkt-API, UI) som ett
   eget granskningsbart steg. De sju batcharna 1/3/3b/4/5a/5b/5c/6 som bär ett bandkrav
   hänvisar nu till §6a.2:s 42-radstabell i stället för att (motsägelsefullt) bara nämna
   det för Borlänge/C4.
8. **P2 rättat:** §6:s steg 3-text ändrad från "samtliga 45 kräver EN NY policyregisterpost"
   till korrekt "44 nya katalogpolicyer + 1 UTÖKAD leverantörsfilspolicy" (motsade tidigare
   sin egen §6a.4). §7:s sammanfattningsstycke bytt `oppna_tariff_ider()` (borttaget namn)
   mot `blockerade_tariff_ider()`/`kontrollera_kompositgrind()`, den funktion som faktiskt
   används. Batch 4:s felaktiga `justeringar.py`/`.ts`-referens (ingen `justeringar.ts`
   existerar) rättad till `justeringar.py` (speglad inline i `fjarrvarme.ts`).
9. **Oförändrat från v7 (redan korrekt och återverifierat):** 78 katalog-ID:n, 14
   variant-ID:n (10/4 efter Jönköping), 42-raders bandtabell, Borlänge/C4/Falus
   gränsfallsbugg, Lidköpings kända metodavvikelse, dispositionerna 7/55/30/92 — inga nya
   produktbeslut från Robert krävs för dessa tekniska rättningar, per granskningens egen
   instruktion.
...
Negativa test krävs för samtliga sex fält ovan: en genererad policy med fältet satt som TypeScript
...
samma post — ett band-ID har ingen numerisk gräns. `harled_resultatstatus`/
`harledResultatstatus` grenar på `f.vardetyp` INNAN den anropar `_validera_varde`: för
`"number"`/`"number_series"` oförändrat (kastar på sträng); för `"band_id"` kräver den att
`post.varde` är en icke-tom `str` (kastar annars) — bandkravet hamnar alltså aldrig i vägen
för `_validera_varde`s numeriska kontroll, och det numeriska kontraktet försvagas inte alls.
...
`batchplan-v8.md`), inte något Sandvikens nuvarande kapacitets-only-adapter redan täcker:
...
2. Ny hjälpfunktion i `besparingsvarde.ts`, t.ex. `byggIndataFranPolicy(policy: Tariffpolicy,
   ...
   (`Number(...)` för `"number"`, oförändrad sträng för `"band_id"`). Ett policydeklarerat
   fält UTAN motsvarande `falt`-nyckel byggs INTE — det blir korrekt `saknade`/`blocked` i
...
INGEN egen ändring utöver att den redan anropar `godkanda(katalog)`; hela fixet ligger i
`godkanda()` själv.
...
1. Den nakna `grind()` (`katalog.py`) ändras INTE — den fortsätter avvisa varje
   ...
   sammansatta `godkanda()`-loopen ovan kan öppna raden.
...
### 6a.4 Stockholm Exergi — nåbar adapterkontroll, verklig dispatch och en typad årsindatamodell (rättat P1, granskning 2026-09-08-007)
...
Kastar (`raise`) om registret pekar på en katalograd som inte finns, eller en målpolicy som
saknas/inte täcker `annual_forward` — aldrig en tyst utebliven rad. `godkanda()`/
katalogloopen rörs INTE av detta — `ADAPTERREGISTER`-kontrollen är en oberoende preflight,
inte ett filter inuti den befintliga loopen.
...
Ny, ÅTSKILD årsindatamodell — 17 nya `KravPost`, alla `kravs_for=("annual",)`, alla med
UNIKA nycklar skilda från de befintliga `monthly`-kraven:

- 12 st `kall_energi_mwh_arsserie_01` … `_12` (januari–december), ELLER en enda
  `KravPost(nyckel="kall_energi_mwh_arsserie", rullande=True, vardetyp="number_series")`
  vars `IndataPost.varde` är en 12-elements `Sequence[float]` i kalenderordning
  (`_ar_serie`/serier är redan tillåtna för `rullande=True`-fält, se
  `_validera_varde`/kommentaren om E.ON/Navirums rullande signatur) — REKOMMENDERAD väg:
  en (1) serie-`KravPost`, inte tolv skalära, håller policyn kort och gör
  längdvalideringen (exakt 12 element) enkel att uttrycka på ETT ställe.
- 1 st `KravPost(nyckel="returtemperatur_c_vintermanader", rullande=True,
  vardetyp="number_series")` — exakt 5 element, ordnade november–mars (samma
  vintermånader som redan gäller för det befintliga `monthly`-kravet).
- Båda seriekraven valideras i `harled_resultatstatus` med en explicit längdkontroll (12
  respektive 5) INNAN de binds — en serie av fel längd BLOCKERAR, mappas aldrig till fel
  månad genom att bara zippa index.
- Kontraktsfasadens `berakna_arskostnad_med_kontrakt`-motsvarighet för Stockholm läser
  dessa två serier och konstruerar `mwh_kallt_per_manad`/en NY
...
**Test (utöver v7:s lista):** saknad/stale `ADAPTERREGISTER`-mappning kastar; fel
`provider_id`/`tariff_id` kastar; policytäckning utan adapter kastar; en seriekrav-post med
11 eller 13 element (kall energi) resp. 4 eller 6 element (returtemp) blockerar; oförändrad
...
fillista (`batchplan-v8.md`), där Jönköpings basrad redan ligger.
...
**Oförändrat i v8 (granskning `2026-09-08-007`):** enbart tekniska kontraktsrättningar
(aktiveringsordning, värdetyp, produktingång, Stockholm-dispatch, Kraftringens motorväg,
Jönköpings allow-list) — ingen post flyttar mellan dispositioner. 7 + 55 + 30 = 92
oförändrat, precis som granskningens egen instruktion (punkt 7) föreskrev.
