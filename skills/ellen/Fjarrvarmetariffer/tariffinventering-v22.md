# Tariffinventering v22.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v21.md` i sin helhet (v21 ändras
...
(`status: changes-required`, supersedes `2026-09-09-014`) av v21/`batchplan-v21.md`. Ingen
...
blockerade / 92 totalt** (bas 7/47/24, variant 0/10/4; se §4.2 och §8) — v22 rättar bara
attesteringskontraktets enda-källa-egenskap och dokumentidentiteten, flyttar ingen post.
Alla kodpåståenden nedan är verifierade genom att läsa den verkliga koden i
...
**Vad som är nytt i v22, i korthet** (se granskning `2026-09-09-015` för fullständig
motivering till varje punkt):

1. **`forkontrolleraPolicyIndata` har EN attesteringskälla, inte två** (§6a.2, rättar P1
   #1): den utskrivna signaturen förblir FYRA parametrar,
   `forkontrolleraPolicyIndata(policy, prisar, indata, omfattning)`, och läser
   `post.attesterad` ur den redan byggda `IndataPost`-kartan. Den batchplan-egna
   femparametersskissen (`indataAttestering: Record<string, boolean>` som separat
   parameter) är borttagen — den fanns bara i `batchplan-v21.md`, aldrig i inventeringens
   egna anropsskisser, men var normativ nog för att skapa två konkurrerande
   sanningskällor. Batch 0 punkt 12 (ett separat rättelselager efter punkterna 5–8) är
   struken helt, inte bara tömd.
2. **Dokumentidentiteten rättad** (rättar P1 #2): denna fil bär nu en korrekt V22-rubrik,
   inledning och "vad som är nytt"-sammanfattning i stället för att av misstag behålla
   v20:s rubrik/inledning oförändrad genom v21. Samtliga aktiva/normativa korsreferenser i
   BÅDA dokumenten till `tariffinventering-v20.md`/`batchplan-v20.md` är uppdaterade till
   v22; äldre versioner nämns bara i uttryckligt historiska stycken (som avsnitten nedan).
3. Samtliga P1/P2-fynd t.o.m. granskning `2026-09-09-014` (den strikta
   `TariffpolicyOptions`-konstruktorn, samtliga sju bindningskontroller, den fullständiga
   attesteringskanalen från UI till `IndataPost`, fail-closed besparingsförmåga m.fl.) är
   BEVARADE oförändrade i sak.

**Vad som var nytt i v21, i korthet** (se granskning `2026-09-09-014` för fullständig
motivering till varje punkt):

1. **`skapaTariffpolicy()`s options-typ OCH returvärde blir EN komplett, STRIKT
   konstruktionskälla för SAMTLIGA åtta planerade `Tariffpolicy`-fält** (§6a.1, rättar P1
   #1): en namngiven, exporterad `TariffpolicyOptions`-typ; `flodeskorrigeringVariant`
   typad som den slutna unionen `'golvfri' | 'golvbegransad'`, inte `string`; konstruktionen
   validerar samtliga SJU nyckelbärande bindningar (inte bara de tre ursprungliga) mot
   `kravdaFalt`, plus bindningsspecifik `vardetyp`. Negativa direkt-/genereringstester för
   okänd variant, hängande bindningsnyckel och fel bindningstyp.
2. **Batch 0:s attesteringskontrakt beskrivet i punkterna 5–8, inte som ett separat
   tillägg** (rättar P1 #4 i granskning `2026-09-09-013`) — grunden för v22:s P1 #1-fix
   ovan, som tog bort den kvarvarande dubbla signaturen batchplanen ändå introducerade.

**Vad som var nytt i v20, i korthet** (se granskning `2026-09-09-013` för fullständig
motivering till varje punkt):

1. **`skapaTariffpolicy()`s options-typ OCH returvärde blir EN komplett konstruktionskälla
   för SAMTLIGA åtta planerade `Tariffpolicy`-fält, inte bara `stodjerBesparing`** (§6a.1,
   rättar P1 #1): `kapacitetBandBindning`, `kapacitetMultiplikatorBindning`,
   `flodeskorrigeringVariant`, `kallenergiArsserieBindning`, `stodjerAktuellArskostnad`,
   `returtemperaturArsserieBindning`, `ersatterKatalograd` och `stodjerBesparing` läggs till
   i SAMMA ändring av options-typen och returobjektet, i BÅDA språken. v19:s felaktiga
   påstående att TypeScript inte skulle klaga på ett extra objektfält i ett direkt
   objektliteral är rättat och ersatt med ett reproducerat `TS2353`-fel mot dagens verkliga,
   orörda fyrfältiga `opts`-typ.
2. **Attesteringen har nu en fullständig, typad kanal från formulär till `IndataPost`**
   (§6a.2, rättar P1 #2): ny parallell karta `policyFaltAttestering: Record<string,
   boolean>`, trådad genom `KalkylatorPage.tsx` → `KalkylatorInputs`/
   `Tariffberakningsunderlag`/`BesparingsvardeArgs` → `argsFranInputs` →
   `byggKontraktIndata`/`byggIndataFranPolicy` → `IndataPost.attesterad`.
   `forkontrolleraPolicyIndata` ger nu ett fältnära `'ej_attesterat'`-fel för ett närvarande
   men icke attesterat attesteringskrävande fält, oberoende av den auktoritativa kontrollen
   i `harledResultatstatus` (§6a.7.6, oförändrad sedan v19).
3. **`batchplan-v20.md` är ett genuint fräscht v20-dokument, inte en patchad v18/v19-fil**
   (rättar P1 #3): korrekt rubrik, föregångare, inventeringslänk och dokumentationsscope;
   den fiktiva `besparingsvarde.ts`/"motsvarande Pythonfil"-referensen borttagen; Batch 0/5d
   speglar hela attesteringskedjan och det kompletta policykonstruktionskontraktet.
4. **Två kvarvarande aktiva motsägelser rättade** (rättar P1 #4): `PolicyFaltMetadata`s
   utskrivna objektform får `kravAttestering: boolean`; Sandvikens sidtest motiverar nu sin
   gröna väg med det gällande `stodjerBesparing === true` i stället för det tidigare,
   fail-open `!== false`-språket.
5. Samtliga P1/P2-fynd t.o.m. granskning `2026-09-09-012` (v19:s femparametersguard,
   fail-closed besparingsförmåga, validatorägarskap, Tm-käll-/attesteringsmodell m.fl.) är
   BEVARADE oförändrade i sak.

**Vad som var nytt i v19, i korthet** (se granskning `2026-09-09-012` för fullständig
motivering till varje punkt):
...
1. **Parserkontraktet uttrycker nu genuint en frånvarande state-nyckel, och den gamla
   ...
   'min' | 'max' | 'heltal' | 'okant_val' | 'ej_attesterat'`) delas nu mellan parsern OCH domänvalidatorn i
...
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
- **Källstatus:** källgranskad (verifieringslistan 2026-09-04)
- **Katalogstatus:** `production_ready: false`, `investigation: null` — lokalt aktiverad 2026-09-14 (granskning 2026-09-14-008, skills@abec8e9)
- **Motorstatus:** Implementerad och aktiv (`flow_difference`, `justeringar.py`/`faktura.py`/`fjarrvarme.ts`, handoff 2026-09-14-001)
- **Kontraktsstatus:** registrerad i `POLICYREGISTER` och aktiverad; `godkanda(katalog)` returnerar raden
...
- **UI-status:** valbar i kalkylatorn; permanent omockat E2E-/produktprov (scenario 16, kalkylator.smoke.mjs; besparingsvardeBatch4Katalogaktivering.test.ts)
- **Årsreproducerbar med nuvarande underlag:** Ja
...
- **Kvarstående arbete:** Inget — lokalt aktiverad. Väntar på Codex granskning av aktiveringsdiffen (handoff 2026-09-14-001) före push.
- **Disposition:** `implemented_source_verified_annual`
...
- **Katalogstatus:** `production_ready: false`, `investigation: null` — lokalt aktiverad 2026-09-14 (granskning 2026-09-14-008, skills@abec8e9)
- **Motorstatus:** Implementerad och aktiv (`asymmetric_flow_difference` samt `kapacitet_multiplikator_bindning` för `B`, handoff 2026-09-14-001)
- **Kontraktsstatus:** registrerad i `POLICYREGISTER` (`_UMEA_ENKEL_POLICY`) och aktiverad; `godkanda(katalog)` returnerar raden (via kompositgrinden, §6a.3)
- **Teststatus:** `test_batch_4_jamtkraft_umea.py`/`resultatkontrakt.batch4.test.ts` (golden-facit vid B=0,93/1/1,401, B=14-blockering, kompositgrind, `godkanda()`s icke-muteringsinvariant mot den aktiverade katalogen) plus `besparingsvardeBatch4Katalogaktivering.test.ts` (domän-/kontraktsnivåprov mot den skarpa TARIFFER, inget UI) och ett permanent omockat E2E-produktbytesscenario i `kalkylator.smoke.mjs` (granskning 2026-09-14-009)
- **UI-status:** valbar i kalkylatorn; permanent omockat E2E-/produktprov (scenario 17, kalkylator.smoke.mjs; besparingsvardeBatch4Katalogaktivering.test.ts)
- **Årsreproducerbar med nuvarande underlag:** Ja
...
- **Kvarstående arbete:** Inget — lokalt aktiverad (motor, `kontrollera_kompositgrind`, policy, katalogproveniens, den skarpa artefakten och omockade UI-/E2E-prov, handoff 2026-09-14-001) — inklusive B=0,93/1,401-gränsfallen och B=14-blockeringen via kontraktsförkontrollen. Den tidigare issue-texten om omappad B är borttagen. Väntar på Codex granskning av aktiveringsdiffen före push.
- **Disposition:** `implemented_source_verified_annual`
...
|---|---|---|---|---|---|---|---|
| `e-on-jarfalla-jarfalla-och-upplands-bro-bostader-2026--bas-delvarme` | `e-on-jarfalla-jarfalla-och-upplands-bro-bostader-2026` | `03_1` (https://www.eon.se/content/dam/eon-se/swe-documents/swe-jamfor-fjarrvarmepriser--bro-balsta-jarfalla-kungsangen-2026.pdf) | 36-månadersmetoden för kunder med bas-/delvärmekälla i stället för fullvärme — leverantörens medelvärde av de TRE HÖGSTA DYGNSMEDELEFFEKTERNA (kW) senaste 36 månaderna inkl. fakturamånaden (verifieringslistan) | RÄTTAT (granskning `2026-09-13-040`, P2) — fyra policyfält: Debiterbar effekt (kW, fakturan — leverantörens 36-månadersberäkning, samma fält som huvudfallet men annan beräkningskälla), bekräftat effektband-ID (samma generiska band-ID-krav som bastariffen, gäller även enbandsraden), medelframledningstemp `Tf` (°C, fakturan, hör till den separata flödeskorrigeringen) OCH flöde (`flode_m3`, m³, fakturan) — PLUS en obligatorisk fakturamånad (`ÅÅÅÅ-MM`) för effektfältet, eftersom leverantören räknar fram ett nytt 36-månadersfönster per fakturamånad. "Samma tre fält, ingen fjärde" (batchplanens historiska formulering) avsåg bara att INGEN rå 36-månadersserie eller egen topp-tre-motor byggs — det tog aldrig bort det generiska band-ID-kravet eller dess fakturamånad. | mwh; kr/schablon blockerade | 3b (efter batch 3) | `implemented_source_verified_annual` |
...
katalogcommit `skills@053a429`):** samtliga åtta varianters
...
| `lulea-energi-lulea-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete i `faktura.py`/`fjarrvarme.ts` (säsongsflöde som 12-elements kalendermånadsserie), lokalt implementerad bakom `investigation.status="utreds"`. Väntar på Codex godkännande av rättningsrundan innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
| `malarenergi-vasteras-och-hallstahammar-24-lagenheter-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny kapacitetsfri policyregisterpost (`kapacitet_bindning=None`) + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr. R03 omscopad bort från denna rad till de två andra Mälarenergi-raderna. Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON, `remaining_information_requests` |
...
| `nevel-gimo-osterbybruk-och-osthammar-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källsann `capacity.billing_basis_method` ifylld mot leverantörens publicerade prislista (nevel-pricelist-2026). Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
| `oresundskraft-angelholm-normal-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källsann `capacity.billing_basis_method` ifylld mot den aktuella prisändringsmodellen 2026-2028. Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
| `oresundskraft-helsingborg-normal-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källsann `capacity.billing_basis_method` ifylld mot den aktuella prisändringsmodellen 2026-2028. Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
...
| `piteenergi-norrfjarden-och-sjulnas-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källa omverifierad 2026-09-15 (pite-small-2026). Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
| `piteenergi-pitea-centrala-natet-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källa omverifierad 2026-09-15 (pite-central-2026). Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
...
| `tekniska-verken-linkoping-linkoping-2026` | passerar redan idag (steg 1–2) | **Uppdaterad 2026-09-16 (Batch 5c, granskning 2026-09-16-001):** genomförd, inte längre planerad. Ny policyregisterpost + delat `volume.months`-motorarbete, lokalt implementerad bakom spärr; källsann `capacity.billing_basis_method` ifylld mot leverantörens webbsida (effektsignatur, 1 nov–31 mar, DVUT -17,6 °C). Väntar på Codex godkännande innan aktivering. | `policyregister.py`, `faktura.py`/`fjarrvarme.ts`, katalog-JSON |
...
5. **`skapaTariffpolicy()`s options-typ OCH returvärde blir EN komplett, ensam
   konstruktionskälla för SAMTLIGA planerade `Tariffpolicy`-fält (rättat P1, granskning
   `2026-09-09-013` — v19 utökade options/retur med ENDAST `stodjerBesparing` och lämnade
   sju andra planerade fält kvar utanför den uttryckliga kedjan, trots att transporttabellen
   ovan redan listade dem: `kapacitetBandBindning`, `kapacitetMultiplikatorBindning`,
   `flodeskorrigeringVariant`, `kallenergiArsserieBindning`, `stodjerAktuellArskostnad`,
   `returtemperaturArsserieBindning`, `ersatterKatalograd`).** Dagens verkliga signatur
   (rad ~187–225 i `resultatkontrakt.ts`):
   ```ts
   ...
   ```
   får ALLA ÅTTA planerade fält i `opts` samtidigt, i SAMMA ändring — inte ett i taget över
   flera versioner. **Rättat P1 (granskning `2026-09-09-014`, fynd 2) — v20:s `opts` var
   INTE en "komplett, ensam konstruktionskälla" trots att den transporterade alla åtta
   fält: `flodeskorrigeringVariant?: string` motsade den redan beslutade tvåvärdesunionen i
   §6a.5, och kommentaren "befintlig validering ... oförändrad" lämnade fyra av de sju
   nyckelbärande bindningarna helt ovaliderade.** `opts` är nu en namngiven, exporterad typ
   `TariffpolicyOptions` med exakt samma fältformer i Python, JSON-mappning och TypeScript:
   ```ts
   export interface TariffpolicyOptions {
     tackning?: readonly Tackningsomrade[];
     kapacitetBindning?: string;
     kallenergiBindning?: string;
     returtemperaturBindning?: string;
     kapacitetBandBindning?: string;
     kapacitetMultiplikatorBindning?: string;
     flodeskorrigeringVariant?: 'golvfri' | 'golvbegransad';
     kallenergiArsserieBindning?: string;
     stodjerAktuellArskostnad?: boolean;
     returtemperaturArsserieBindning?: string;
     ersatterKatalograd?: string;
     stodjerBesparing?: boolean;
   }

   export function skapaTariffpolicy(
     tariffId: string,
     kravdaFalt: readonly KravPost[],
     opts: TariffpolicyOptions = {}
   ): Tariffpolicy {
     const tackning = opts.tackning ?? [];
     const nycklar = kravdaFalt.map((f) => f.nyckel);
     if (new Set(nycklar).size !== nycklar.length) {
       throw new Error(`${tariffId}: dubblerade nycklar i kravdaFalt`);
     }
     for (const omrade of tackning) {
       if (!TACKNINGSOMRADEN.includes(omrade)) {
         throw new Error(`${tariffId}: okänt täckningsområde i tackning: ${omrade}`);
       }
     }
     // Samtliga SJU nyckelbärande bindningar (de tre ursprungliga plus de fyra nya) —
     // INTE bara de tre ursprungliga som v20:s kommentar felaktigt kallade "fyra
     // ursprungliga bindningsfält" (dagens verkliga konstruktor validerar bara tre, se
     // resultatkontrakt.ts rad ~206–211). Varje tuppel bär också den `vardetyp` bindningen
     // MÅSTE peka mot, enligt §6a.2–§6a.4:s kontrakt.
     for (const [attributnamn, bindning, kravdVardetyp] of [
       ['kapacitetBindning', opts.kapacitetBindning, 'number'],
       ['kallenergiBindning', opts.kallenergiBindning, 'number'],
       ['returtemperaturBindning', opts.returtemperaturBindning, 'number'],
       ['kapacitetBandBindning', opts.kapacitetBandBindning, 'band_id'],
       ['kapacitetMultiplikatorBindning', opts.kapacitetMultiplikatorBindning, 'number'],
       ['kallenergiArsserieBindning', opts.kallenergiArsserieBindning, 'number_series'],
       ['returtemperaturArsserieBindning', opts.returtemperaturArsserieBindning,
         'number_series'],
     ] as const) {
       if (bindning === undefined) continue;
       const krav = kravdaFalt.find((f) => f.nyckel === bindning);
       if (krav === undefined) {
         throw new Error(`${tariffId}: ${attributnamn} ${bindning} finns inte i kravdaFalt`);
       }
       if ((krav.vardetyp ?? 'number') !== kravdVardetyp) {
         throw new Error(
           `${tariffId}: ${attributnamn} ${bindning} pekar på ett krav med vardetyp ` +
             `'${krav.vardetyp ?? 'number'}', förväntade '${kravdVardetyp}'`
         );
       }
       // Vilken omfattning bindningen måste gälla ('annual'/'monthly') kontrolleras
       // INTE här — se respektive fasad (oförändrat sedan granskning 2026-09-05-003).
     }
     if (
       opts.flodeskorrigeringVariant !== undefined &&
       !(['golvfri', 'golvbegransad'] as const).includes(opts.flodeskorrigeringVariant)
     ) {
       throw new Error(
         `${tariffId}: okänd flodeskorrigeringVariant ${opts.flodeskorrigeringVariant}`
       );
     }
     return {
       tariffId, kravdaFalt, tackning,
       kapacitetBindning: opts.kapacitetBindning,
       kallenergiBindning: opts.kallenergiBindning,
       returtemperaturBindning: opts.returtemperaturBindning,
       kapacitetBandBindning: opts.kapacitetBandBindning,
       kapacitetMultiplikatorBindning: opts.kapacitetMultiplikatorBindning,
       flodeskorrigeringVariant: opts.flodeskorrigeringVariant,
       kallenergiArsserieBindning: opts.kallenergiArsserieBindning,
       stodjerAktuellArskostnad: opts.stodjerAktuellArskostnad ?? false,
       returtemperaturArsserieBindning: opts.returtemperaturArsserieBindning,
       ersatterKatalograd: opts.ersatterKatalograd,
       stodjerBesparing: opts.stodjerBesparing ?? false,
     };
   }
   ```
   `ersatterKatalograd` bär inget `KravPost`-nyckelanspråk (den pekar på ett katalog-ID via
   den redan etablerade adapter-preflight-bijektionen i §6a.3, inte ett krav) och ingår
   därför INTE i bindningskontrollen ovan — den är åtta fält i `opts`, men bara sju av dem
   är "nyckelbärande bindningar" i denna kontrolls mening.

   `policyFranGenererad()` använder EXAKT samma `TariffpolicyOptions`-typ när den bygger
   sitt anrop — ingen separat, avvikande konstruktionsväg för genererade policyer.
   Motsvarande Python-konstruktör (`Tariffpolicy.__init__`/dataklassfälten i
   `resultatkontrakt.py`) får samma åtta fält och samma bindnings-/`vardetyp`-validering på
   samma sätt, med samma fail-closed-defaults (`False`/`None`) som TypeScript-sidan.

   **Nya negativa tester (krävda av granskning `2026-09-09-014`, fynd 2):** (a) en okänd
   `flodeskorrigeringVariant` (t.ex. `'annat'`) kastar vid direkt konstruktion, i stället för
   att accepteras som en fri sträng; (b) en av de FYRA NYA nyckelbärande bindningarna som
   pekar på en nyckel som INTE finns i `kravdaFalt` kastar, exakt som de tre ursprungliga
   redan gör idag; (c) en bindning som pekar på ett krav med FEL `vardetyp` (t.ex.
   `kapacitetBandBindning` mot ett `vardetyp: 'number'`-krav, eller `kallenergiArsserieBindning`
   mot ett skalärt `vardetyp: 'number'`-krav) kastar, både direkt och via
   `policyFranGenererad()` på en felaktigt konstruerad genererad rad.

   **Rättat felaktigt TypeScriptpåstående (granskning `2026-09-09-013`, P1):** v19 hävdade
   att TypeScripts strukturella typning INTE klagar på ett extra objektfält i ett direkt
   objektliteral när `exactOptionalPropertyTypes` inte används. Det är FEL och reproducerat
   falskt: ett isolerat scratch-anrop `skapaTariffpolicy('t', kf, { tackning: [],
   kapacitetBindning: 'x', stodjerAktuellArskostnad: true })` mot dagens verkliga,
   ORÖRDA fyrfältiga `opts`-typ gav `npx tsc --noEmit --strict --skipLibCheck`-felet
   `TS2353: Object literal may only specify known properties, and
   'stodjerAktuellArskostnad' does not exist in type '{ tackning?: ...; kapacitetBindning?:
   ...; kallenergiBindning?: ...; returtemperaturBindning?: ... }'` (kontrollfilen låg i
   `neptune-marketing/src/utils/`, togs bort efter körningen). TypeScripts s.k.
   "excess property check" gäller just DIREKTA objektliteraler mot en känd måltyp — den är
   INTE begränsad till `exactOptionalPropertyTypes`, som bara styr `undefined`-hantering på
   redan kända optionella fält, en helt annan kontroll. Konsekvensen om `opts`-typen INTE
   utökas är alltså inte tyst bortfall via strukturell typning (det påståendet var fel) utan
   ett HÅRT kompileringsfel — vilket i sig bevisar varför punkt 5 måste utöka `opts`-typen
   med alla åtta fält i SAMMA ändring som transporttabellen, annars kompilerar inte
   `policyFranGenererad()`s anrop över huvud taget.

   Testa BÅDE direkt konstruktion (`skapaTariffpolicy(id, falt, { ...alla åtta fält })` ger
   ett `Tariffpolicy`-objekt med alla åtta fält lästa tillbaka oförändrade) OCH genererad
   policy (`policyFranGenererad()` på en JSON-rad med alla åtta snake_case-fälten satta ger
   samma resultat) för VARJE fält, inte bara `stodjerBesparing`/`kravAttestering`.
...
7. **`PolicyFaltMetadata`** (§6a.2, den UI-riktade listan `policyFaltMetadata()` bygger):
   ...
   varje krav — se §6a.7.6 nedan för hur UI:t använder den, och se den rättade
   objektformen längre ned i §6a.2 (granskning `2026-09-09-013`, P1 — samma egenskap
   saknades tidigare i sjäva den utskrivna objektformen, inte bara i denna sammanfattning).
   ...
   **Löst med två separata typer:**
   ...
   - `PolicyInputValue = number | string | readonly number[]` (oförändrad typ, men nu ENDAST
     ...
     **Löst med EN delad orsaksunion, deklarerad EN gång och återanvänd överallt:**
     ...
     'heltal' | 'okant_val' | 'ej_attesterat'` (den sista medlemmen ny, rättat P1, granskning
     `2026-09-09-013` — se `forkontrolleraPolicyIndata` nedan) (`resultatkontrakt.ts`, samma
     namn i batchplanen — v12:s
...
2. `KalkylatorPage.tsx` bygger `policyFalt: Record<string, PolicyInputValue>` VID SUBMIT
   ...
   initialisering-motivering — och slår upp respektive `PolicyRawFormValue` i
   ...
   **Attesteringskanalen (ny, rättat P1, granskning `2026-09-09-013` — v19 gjorde
   `IndataPost.attesterad` till en auktoritativ domänmarkör, §6a.7.6, men gav den ingen
   typad kanal genom det här steget).** I SAMMA submit-hantering bygger sidan
   `policyFaltAttestering: Record<string, boolean>` från ett separat React-state
   `policyFaltAttesteringState: Record<string, boolean>` — EN kryssruta per `KravPost` där
   `kravAttestering===true` (punkt 5s metadata-lista avgör vilka fält som visar kryssrutan),
   satt av `onChange` precis som `policyFaltRavarden` redan sätts av textfältens `onChange`.
   Ett krav med `kravAttestering===true` vars kryssruta INTE är ikryssad ger `false` i
   `policyFaltAttesteringState` (fail-closed default för state utan explicit interaktion) —
   sidan visar då ett fältnära formulärfel DIREKT vid submit (nästa punkt) i stället för att
   skicka en tyst `attesterad:false` vidare till domänlagret.
   **Rättat P1 (granskning `2026-09-09-013`) — v19 gjorde `IndataPost.attesterad` till en
   auktoritativ domänmarkör (§6a.7.6 nedan) men gav den INGEN typad kanal genom
   `byggIndataFranPolicy` eller något av de skriftliga produkt-DTO-kontrakten
   (`KalkylatorInputs`/`Tariffberakningsunderlag`/`BesparingsvardeArgs`, se dessa nedan) — ett
   normalt UI-/produktanrop kunde alltså aldrig producera `attesterad:true`, och Lidköping
   skulle ALLTID blockeras av den nya statuskontrollen. Löst med ETT valt, typat
   end-to-end-kontrakt: en separat, parallell karta `policyFaltAttestering: Record<string,
   boolean>` — INTE en omstrukturering av `PolicyInputValue` (som redan har tre etablerade
   varianter, `number | readonly number[] | string`, och inte behöver bära ett fjärde,
   orelaterat booleskt fält per nyckel).**
   Ny hjälpfunktion i `besparingsvarde.ts`, `byggIndataFranPolicy(policy: Tariffpolicy,
   policyFalt: Record<string, PolicyInputValue>, policyFaltAttestering: Record<string,
   boolean> = {}): Map<string, IndataPost>`: itererar
   `policy.kravdaFalt`, slår upp varje `nyckel` i `policyFalt` (INTE det numeriska `falt`),
   och bygger en `IndataPost` per TRÄFF med `kallaTyp: 'supplier_value'`, exakt det
   `PolicyInputValue`-formade värdet, OCH `attesterad: policyFaltAttestering[nyckel] ??
   false` — ingen ytterligare typkonvertering sker i denna
   ...
   validering (saknade fält, källtyp, OCH — nytt — attestering för krav med
   `kravAttestering===true`) som redan fanns innan detta tillägg. Ett
   ...
   aldrig motorn. En `policyFaltAttestering`-nyckel utan motsvarande krav ignoreras på samma
   sätt; ett krav vars `nyckel` saknas i `policyFaltAttestering` får `attesterad: false`
   (fail-closed default, samma mönster som `stodjerBesparing`).
...
4. Python-sidans motsvarande produktväg (i dag bara Sandvikens direkta
   ...
   Mapping[str, float | str | Sequence[float]], policy_falt_attestering:
   Mapping[str, bool] | None = None)` i förberedelse för framtida
...
5. **Genererad UI-metadata — prispost- och omfattningsmedveten (rättat P1, granskning
   ...
   kravAttestering: boolean }` (rättat P1, granskning `2026-09-09-013` — den utskrivna
   objektformen saknade `kravAttestering` trots att punkt 7 i den kompletta
   konstruktionskedjan ovan uttryckligen lägger till egenskapen; satt från
   `f.kravAttestering ?? false` för varje krav, precis som `etikett`/`hjalptext` redan
   sätts från motsvarande `KravPost`-fält).
...
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel, genom en
   ...
   **Rättat P2 (granskning `2026-09-09-005`) — "filtrerar nu IDENTISKT" med
   ...
   - Fält i `relevanta` som FINNS i `indata` kontrolleras och kan hamna i `ogiltiga`:
     ...
     träff finns; och — **nytt, rättat P1, granskning `2026-09-09-013`** — för fält där
     `f.kravAttestering === true`: om det MOTTAGNA `IndataPost.attesterad !== true` (fältet
     finns i `indata` — annars vore det redan `saknade` ovan — men attesteringsflaggan på
     just den posten är `false`/`undefined`), lägg till `{nyckel, orsak: 'ej_attesterat'}`.
     Detta ger UI:t ett FÄLTNÄRA fel ("kryssa i attesteringsrutan") i stället för det sena,
     generiska `blocked`-resultatet `harledResultatstatus` annars skulle ge — SAMMA
     information, bara tillgänglig TIDIGARE i kedjan, exakt som resten av
     `forkontrolleraPolicyIndata` redan gör för övriga fältnära fel. Kontrollen är oberoende
     av `harledResultatstatus`s egen, AUKTORITATIVA attesteringskontroll (§6a.7.6) — ett
     direkt fasadanrop som kringgår `forkontrolleraPolicyIndata` helt fångas ändå där.
...
kapacitetKw: number, policyFalt: Record<string, PolicyInputValue>, policyFaltAttestering:
Record<string, boolean> = {}):
Map<string, IndataPost>` (femte parametern ny, rättat P1, granskning `2026-09-09-013` —
trådar attesteringskartan vidare till `byggIndataFranPolicy`) i `besparingsvarde.ts` —
anropar `byggIndataFranPolicy` (§6a.2, punkt
...
export function beraknaArsprodukt(underlag: Tariffberakningsunderlag): ArsprodukResultat {
  ...
  const policyFaltAttestering = underlag.policyFaltAttestering ?? {}; // NYTT, rättat P1,
                                                                        // granskning
                                                                        // `2026-09-09-013`
  const indata = byggKontraktIndata(gated.policy, prisar, kapacitetKw, policyFalt, policyFaltAttestering);
...
export interface Tariffberakningsunderlag {
  ...
  policyFaltAttestering?: Record<string, boolean>; // NYTT, rättat P1, granskning
                                                     // `2026-09-09-013` — se §6a.2
...
// policyFaltAttestering/onskadTyp på KalkylatorInputs (samma status som i övriga
// §6a).
export function argsFranInputs(inputs: KalkylatorInputs): Tariffberakningsunderlag {
  ...
  return {
    ...
    policyFaltAttestering: inputs.policyFaltAttestering, // NYTT, rättat P1, granskning
                                                           // `2026-09-09-013` — samma
                                                           // passthrough-mönster som
                                                           // policyFalt självt
    energyProvenance,
  };
}
...
export interface BesparingsvardeArgs {
  ...
  policyFaltAttestering?: Record<string, boolean>; // NYTT, additivt fält, rättat P1,
                                                     // granskning `2026-09-09-013` — samma
                                                     // status som policyFalt ovan
}
```
...
kapacitetKw, args.policyFalt ?? {}, args.policyFaltAttestering ?? {})` (rättat P1,
granskning `2026-09-09-013` — den delade byggaren tar nu även attesteringskartan, tråden
till `byggIndataFranPolicy` ovan) som `beraknaArsprodukt` använder, och kör
...
bevisligen oberörd av detta tillägg (Sandviken har `stodjerBesparing === true`, rättat P1,
granskning `2026-09-09-013` — den tidigare `!== false`-motiveringen var kvarlämnad,
fail-open text som motsade det gällande, fail-closed kontraktet i §6a.4 ovan; explicit
`policy.stodjer_besparing = True` satt av `policyregister.py` för Sandviken, och kastar
inte). Ett FJÄRDE test verifierar att `stodjerAktuellArskostnad(prisar)`
...
aktivering):** `harnosand-energi-miljo-harnosand-2026`s bastariffrad (§3–4)
flyttades från `blocked_external_info` till `ready_to_implement` sedan
HEMAB besvarat R02/A2 (se `assessment-harnosand-volymrabatt-2026-09-24`):
intervalltabellen är faktureringsgrundande och prislistans räkneexempel
(45 050 kr) är felaktigt — korrekt marginalt avdrag vid 1750 MWh är
50 575 kr exklusive moms. Tabellen ovan går från **75 implemented / 2
ready / 14 blocked / 1 not_applicable** till **75 implemented / 3 ready /
13 blocked / 1 not_applicable av 92**. `investigation.status` hålls kvar
på `utreds` och `godkanda(katalog, policyregister=POLICYREGISTER)` är
OFÖRÄNDRAD (74 fysiska katalograder) — Härnösand aktiveras inte i denna
runda. Motor (`marginal_annual_volume_discount` med nytt `settlement`-fält
...
`implemented_source_verified_annual` i SAMMA commit som katalogens `investigation`-spärr
togs bort (`skills@abec8e9`), enligt skyldigheten som antecknades nedan efter Batch
3b-aktiveringen. Tabellen ovan är en mekanisk räkning; den matchar den katalog-mekaniskt
verifierade dispositionen **37 implemented / 27 ready / 28 blocked av 92**
(`godkanda(katalog)` == 37; se
`skills/ellen/conversations/sessions/2026/09/2026-09-14-batch-4-jamtkraft-umea.md`).
Batch 4 har inga varianter, så varianttabellens 8/2/4 = 14 är oförändrad.
