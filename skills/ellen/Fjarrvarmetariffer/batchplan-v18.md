# Batchplan v18.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v17.md` i sin helhet (v17 ändras INTE i
...
(supersedes `2026-09-09-010`). Bygger på dispositionerna i
[`tariffinventering-v18.md`](tariffinventering-v18.md) §4.1 (47 bastariffer) och §5 (10
...
blockerade / 92 totalt** — v18 gör bara Lidköpings implementationsplan körbar, flyttar ingen
post.

**Vad som är nytt i v18** (se granskning `2026-09-09-011` för fullständig motivering):
...
- **Innehåll:**
  ...
  9. **Namngiven, gemensam produktentry för "aktuell årskostnad" (Stockholm OCH Lidköping),
     ...
     **Gällande definition** (ersätter v13–v17:s indirekta kontroll av vilken
     ...
     fält på `Tariffpolicy`, satt av `policyregister.py` per tariff (se
     `tariffinventering-v18.md` §6a.4 för den fullständiga historiken/motiveringen till
     bytet från den tidigare `kallenergiArsserieBindning`-baserade kontrollen). Ett separat,
     oberoende fält `stodjerBesparing(prisar): boolean` (default `true`, explicit `false`
     för Stockholm och Lidköping) styr om `beraknaBesparingsvardeKontrakt` kastar
     `Produktbegransning` — de två förmågorna är alltså inte längre varandras negation,
     en framtida tariff kan i princip ha båda `true` samtidigt. Kontrollen — den enda
     tillåtna rådatagrinden, ALDRIG `prisar.policy?.X` direkt — är därmed `false` för
     icke-fjärrvärmesystem, legacytariffer OCH Sandviken (kontraktsgatad men utan
     `stodjer_aktuell_arskostnad`-flagga), `true` för Stockholm OCH Lidköping (§6a.7).
...
- **Rättat P1 (granskning `2026-09-09-010`) — v16:s motorkod var en fristående skiss utan
  ...
  hjälpfunktionsanrop — se `tariffinventering-v18.md` §6a.7.1–§6a.7.3 för den fullständiga,
...
- **Fail-closed:** saknad serie → `missing_policy_fields`; fel längd/icke-ändligt element →
  ...
  `Tm_m` är ett RUNTIMEKRAV per kundfall, matat in som `kallaTyp: 'snapshot'` (§6a.7.6 —
  transkriberat från faktura/leverantörsbesked, INTE ett fritt gissat tal) — resultatet
  visas `uppskattat` tills fakturaverifierat.
...
- **Rättat P1 (granskning `2026-09-09-011`) — fillistan placerade katalogändringen i fel
  ...
  etappen. Det är sant för DENNA dokumentationscommit (v18/`tariffinventering-v18.md`
...
- **Fillista, Batch 5d (planerad FRAMTIDA implementation — INTE genomförd av v18/denna
  ...
  - `resultatkontrakt.ts`/`.py`: `beraknaArskostnadMedKontrakt`/motsvarigheten extraherar
    ...
    (§6a.7.6); `Tariffpolicy.stodjer_aktuell_arskostnad`/`stodjer_besparing` och
    TypeScript-speglarna (§6a.4).
  - `besparingsvarde.ts`/motsvarande Pythonfil: ny `Produktbegransning`-klass/typ, guard i
    `beraknaBesparingsvardeKontrakt` (§6a.4).
  ...
  - `KalkylatorPage.tsx`: tre serieinmatningsfält, obligatorisk källattesteringskryssruta på
    `Tm_m`, separat `Produktbegransning`-felvisning skild från `KontraktBlockerat`.
  - **Namngivna testfiler** (P2, granskning `2026-09-09-011`): Pythonregistrering/exakt
    ...
    (`besparingsvarde.produktbegransning.test.ts`), och ett katalog→policy→fasad→motor→
    `Kostnad.justering`-integrationstest (`generera.lidkoping.integration.test.ts`/
    `test_generera_lidkoping_integration.py`).
  - **Aktivering** (§6a.7) sker FÖRST efter granskad kod och gröna tester för samtliga
    ...
    i v18/denna dokumentationscommit.
- **Golden-/gräns-/negativtester:** §6a.7:s tolv testfall (nio ursprungliga + tre nya i
  v17, se `tariffinventering-v18.md` §6a.7 slutet), inklusive ett end-to-end-test mot
  ...
  produktkod ändras av `tariffinventering-v18.md`/`batchplan-v18.md`.
...
- **Fail-closed före/efter-regel för besparingsvärdering (nytt P1, granskning
  ...
  `KontraktBlockerat` (se `tariffinventering-v18.md` §6a.4 för klass/guard/UI-mappning).
- **Namngiven, gemensam produktentry för "aktuell årskostnad", EN gren, EN förmågetabell
  ...
  historik). Ett separat `stodjerBesparing(prisar): boolean` (default `true`, explicit
  `false` för Stockholm och Lidköping) styr `Produktbegransning`-guarden oberoende — och
  kontrolleras BÅDE av `calcResultForOnskadTyp` (UX-förkontroll) OCH INUTI
  `beraknaArsprodukt` självt (den auktoritativa spärren, se Batch 0 punkt 9).
...
**Oförändrat i v17/v18 (granskning `2026-09-09-010`/`2026-09-09-011`):** Batch 5d:s
omfattning (2 bastariffer) är oförändrad genom både rundorna — v17/v18 gör bara
implementationsplanen körbar och korrekt (fullständig motorkedja, förmågekontrakt,
elementvis `minExklusiv`, korrekt Tm-proveniens, konkret `Produktbegransning`, se
`tariffinventering-v18.md` §6a.7/§6a.4). Batchsumman 57 och räkningen 7/57/28 av 92 är
oförändrade.
