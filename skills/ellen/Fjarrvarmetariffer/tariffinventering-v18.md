# Tariffinventering v18.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v17.md` i sin helhet (v17 ändras
INTE i efterhand — kvar som historik), som svar på omgranskning
[2026-09-09-011](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v17.md)
(`status: changes-required`, supersedes `2026-09-09-010`) av v17/`batchplan-v17.md`. Ingen
...
blockerade / 92 totalt** (bas 7/47/24, variant 0/10/4; se §4.2 och §8) — v18 gör bara
...
**Vad som är nytt i v18, i korthet** (se granskning `2026-09-09-011` för fullständig
...
```ts
// besparingsvarde.ts / resultatkontrakt.ts — delad av båda validatorerna
function vardefelForKrav(f: KravPost, varde: Varde): PolicyValideringsOrsak | null {
...
```python
# resultatkontrakt.py — mirror, delad av harled_resultatstatus och
# forkontrollera_policy_indata
...
```python
# besparingsvarde.py (domänlagrets Pythonmotsvarighet)
class Produktbegransning(Exception):
    def __init__(self, tariff_id: str, orsak: str = "besparing_ej_stodd") -> None:
        self.tariff_id = tariff_id
        self.orsak = orsak
        super().__init__(f"{tariff_id}: produkten stöder inte denna beräkning ({orsak})")
```

**Konkret guard, i `beraknaBesparingsvardeKontrakt`, FÖRST — innan indata byggs eller
valideras (spärren gäller produkten, inte kundens data):**

```ts
export function beraknaBesparingsvardeKontrakt(
  args: BesparingsvardeArgs, prisar: GenereradPrisarspost
): Besparingsvarde {
  const gated = kontraktsgatadPolicy(prisar);
  if (gated && stodjerBesparing(prisar) === false) {
    throw new Produktbegransning(prisar.tariff_id, 'besparing_ej_stodd');
  }
  // ... befintlig kropp, oförändrad i övrigt
}
```
...
**Test:** ett `beraknaBesparingsvarde`-anrop för Stockholm med `onskadTyp:'besparing'`
kastar `Produktbegransning('stockholm-exergi-2026', 'besparing_ej_stodd')`; samma för
Lidköping (§6a.7.7); ett anrop för Sandviken (kontraktsgated, `stodjerBesparing`
implicit `true`) genomförs OFÖRÄNDRAT, utan att kasta.
...
plats för `Produktbegransning` (§6a.4, "Beslut"-stycket ovan) så snart
`policy.stodjerBesparing === false` — ingen ny gren krävs för att uppnå den blockeringen,
bara guarden i den redan existerande funktionen.
...
**Rättat P2 (granskning `2026-09-09-011`) — `stodjer_aktuell_arskostnad` användes ÄVEN som
negationen av "stödjer besparing", vilket i praktiken låser en framtida tariff som skulle
kunna stödja BÅDA produkterna.** Löst med ETT ytterligare, oberoende deklarativt fält:

```python
# resultatkontrakt.py, Tariffpolicy
@dataclass(frozen=True)
class Tariffpolicy:
    ...
    stodjer_aktuell_arskostnad: bool = False
    stodjer_besparing: bool = True  # explicit opt-OUT, till skillnad från fältet ovan —
                                     # de allra flesta kontraktsgatade tariffer stödjer
                                     # besparing redan i dag (Sandviken), så default är True
```
...
export interface Tariffpolicy {
  ...
  stodjerBesparing?: boolean;         // default true vid frånvaro
}
```
...
export function stodjerBesparing(prisar: GenereradPrisarspost): boolean {
  const gated = kontraktsgatadPolicy(prisar);
  if (!gated) return true; // icke-kontraktsgatade/legacy-tariffer: opåverkade, alltid true
  return gated.policy.stodjerBesparing !== false;
}
```

`policyregister.py` sätter `stodjer_besparing=False` explicit för Stockholm OCH Lidköping
(båda saknar en källförsvarbar före/efter-regel, §6a.4/§6a.7.7) — Sandviken och alla
framtida kontraktsgatade tariffer utan ett sådant undantag förblir `True` (default). De två
...
skulle påstå sig vara ett verifierat leverantörsvärde — samma risk v17 identifierade. Löst
med ett NYTT, generiskt UI-/policymetadatafält, oberoende av käll-/noggrannhetskontraktet:
`policyFaltMetadata`-posten för `Tm_m` (§6a.1:s tabell) får ett booleskt
`kravAttestering: true`-fält (mirror `krav_attestering` i Python-genereringen av
`policyFaltMetadata`); `KalkylatorPage.tsx` renderar då en obligatorisk bekräftelseruta
("Jag intygar att värdet är hämtat från Lidköpings faktura eller ett direkt skriftligt
besked, inte ett eget uppskattat tal") INTILL `Tm_m`-fältet och blockerar submit tills den
är ikryssad — en REN UI-nivå-spärr (samma mönster som §6a.2 punkt 2:s befintliga
fältnära-fel-vid-submit-genväg), inte en ändring av domänlagrets käll-/statuskontrakt.
`Q_m`/`T_m` kräver ingen sådan attestering — de är kundens egna, direkt avlästa
fakturavärden utan samma risk för ett gissat tal.
