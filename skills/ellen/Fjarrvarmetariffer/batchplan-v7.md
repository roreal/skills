# Batchplan v7.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-08 av Claude. Ersätter `batchplan-v6.md` i sin helhet (v6 ändras INTE i
...
(supersedes `2026-09-08-005`). Bygger på dispositionerna i
[`tariffinventering-v7.md`](tariffinventering-v7.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v7** (se granskning `2026-09-08-006` för fullständig motivering):

1. **Bandkontraktet (§6a.2) omfattar nu 42 rader, inte 2** — samtliga batchar med en
   berörd tariff hänvisar till den konsoliderade tabellen i stället för att bara nämna
   Borlänge/C4 (batch 5a/5b/3/4).
2. **Umeås aktiveringsgrind arkitektoniskt separerad** (batch 4): den nakna `grind()`
   ändras INTE längre — en ny, separat `kontrollera_kompositgrind()` (§6a.3) körs efter,
   tar policyn som explicit argument. `B`-taket omräknat till `1,401` (inte det gissade
   exakta `1,4`).
3. **Kraftringens diskriminator namngiven explicit** (batch 3, §6a.5):
   `kapacitet_bindning_variant`, aldrig implicit härledd från leverantörs-ID — Codex-beslut
   tillämpat.
4. **Stockholm Exergi (batch 7): EN utökad policy, inte två poster med samma nyckel.**
   v6:s "andra separata policy" hade tyst skrivit över den befintliga —
   `POLICYREGISTER[tariff_id]` kan bara bära en post. `ADAPTERREGISTER` nu typat
   (`provider_id`/`tariff_id`/`kravd_tackning`), generatorn kastar om målpolicyn saknar
   täckningen i stället för att bara hoppa över blint.
5. **Jönköpings accessavgift flyttad till batch 5b** som en räknad varianttäckning
   (Codex/Roberts beslut) — ny "6 + 1"-räkning för den batchen, disposition 55/30 (upp från
   54/31), variantsplit 10/4 (upp från 9/5).
6. **Räkningen i Sammanfattningen verifierad additivt** (45 bastariffer + 10 variantrader
   = 55, per batch) — inte bara påstådd.
...
- **Modell (rättat i v4, granskning 2026-09-08-003, P1):** NY motorkod:
  ...
  `kapacitet_bindning_variant: Literal["golvfri", "golvbegransad"]` (inventeringens §6a.5,
  Codex-beslut i granskning `2026-09-08-006`) — satt per tariff-ID i `policyregister.py`,
  ALDRIG härlett implicit från leverantörs-ID. En okänd/saknad diskriminator BLOCKERAR.
...
- **Filer:** katalograder ×4, `justeringar.py`/`.ts` (två nya formler), NY funktion
  `kontrollera_kompositgrind()` (§6a.3 — INTE en ändring av `katalog.py`s nakna `grind()`),
...
- **Teststrategi:** golden-värde per tariff, gränstest för respektive formels
  ...
  `B=0.93`, `B=1.401` och en mellanliggande punkt; ett negativt test för `B=14` utanför
...
Stämmer mot inventeringens §8: 7 implementerade + 55 redo (batchade ovan: 6+1+9+8+4+8+7+8+3
= 54 tariffer... se not) + 30 blockerade (26 bas + 4 variant) = 92.
