# Batchplan v9.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-08 av Claude. Ersätter `batchplan-v8.md` i sin helhet (v8 ändras INTE i
...
(supersedes `2026-09-08-007`). Bygger på dispositionerna i
[`tariffinventering-v9.md`](tariffinventering-v9.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v9** (se granskning `2026-09-08-008` för fullständig motivering):

1. **Batch 0 kompletterad med UI-metadata och felklassning** — v8:s `byggIndataFranPolicy`
   var bara en hjälpfunktion utan källa till band-/enum-/serieval (`KalkylatorPage.tsx`
   renderar i dag allt som `type="number"`). Ny genererad `PolicyFaltMetadata` (§6a.2 punkt
   5) samt en ny `KontraktBlockerat.saknadeFalt`-gren som skiljer legitimt saknad kundindata
   från ett trasigt policykontrakt (§6a.2 punkt 6). Batch 0:s test är nu ett riktigt
   end-to-end-fall som renderar/fyller/beräknar number + band-ID + enum + serie tillsammans.
2. **Umeås sammansatta grind kör nu ALLA återstående kontroller** (batch 4, §6a.3): efter
   att multiplikatorbindningen kvitterats körs `grind()` en gång till på en kopia där bara
   det fyndet är neutraliserat — annars kunde en okänd `issue` eller okänd justeringstyp
   ligga dold bakom multiplikatorfyndet och aldrig kontrolleras (Codex reproducerade att
   Umeås rad har alla tre samtidigt). `bygg_ts_fran_katalog()` för nu sitt eget
   `policyregister`-argument uttryckligen till `godkanda()`.
3. **Ny diskriminerad värdeunion på `IndataPost.varde` själv, inte bara `KravPost`** (batch
   0, §6a.2): `Varde = float | Sequence[float] | str`, plus ett nytt generiskt
   kardinalitetsfält `KravPost.antal_varden` som gör `number_series` till en verklig
   diskriminator (v8:s `rullande=True` tillät en serie men krävde den inte).
4. **Stockholm Exergi (batch 7): EN årsmodell, inte två motsägande.** v8 kallade modellen
   omväxlande "17 nya KravPost" och en tvåserielösning. §6a.4 committar till exakt två
   serie-`KravPost` (12/5 element) med två nya statiska bindningar
   (`kallenergi_arsserie_bindning`/`returtemperatur_arsserie_bindning`), breddar det
   befintliga effektkravet till även `annual`, och definierar en fail-closed regel: Stockholm
   kan ge en uppskattad aktuell årskostnad men exkluderas från besparingsvärdering tills en
   källmässigt försvarbar före/efter-transformationsregel för kallenergin finns. Adapter-
   preflighten kontrollerar nu även `provider_id` mot den byggda leverantörsmängden och den
   omvända regeln (policytäckning utan adapter kastar).
5. **Dispositionerna 7/55/30/92 oförändrade** — samtliga v9-fynd var körbarhets-/
   typningsfel i redan beslutade kontraktsdesigner, ingen post flyttar.

## Batch 0 — Grundkontrakt: diskriminerad värdetyp, bandbindning, seriekardinalitet och en genererad produkt-/UI-ingång (granskning 2026-09-08-007/-008, P1)

**Krävs FÖRE varje batch som bär ett bandkrav** (1, 3, 3b, 4, 5a, 5b, 5c, 6 — se
`tariffinventering-v9.md` §6a.2:s 42-radstabell) OCH före batch 7 (Stockholms seriekrav
...
- **Innehåll:**
  ...
  2. `IndataPost.varde: Varde = float | Sequence[float] | str` (**nytt i v9**) — den
     diskriminerade typunionen måste ligga på `IndataPost` själv, inte bara `KravPost`,
     annars kan ett `band_id`-värde inte konstrueras typkorrekt (§6a.2).
  ...
  4. `Tariffpolicy.kapacitet_band_bindning`, `till_prisar()` bevarar `nivaer[].id`, ny
     motorparameter `vald_niva_id` i `arskostnad`/`manadskostnad` (§6a.2).
  5. Ny hjälpfunktion `byggIndataFranPolicy(policy, falt)` i `besparingsvarde.ts`, kallad
     från `beraknaBesparingsvardeKontrakt` i stället för det nuvarande enda
     `kapacitetBindning`-inlägget (§6a.2), inklusive `number_series`-hantering.
  6. **Genererad UI-metadata (nytt i v9):** ny `policyFaltMetadata(policy)`-funktion i
     `resultatkontrakt.ts` som ger `KalkylatorPage.tsx` inmatningstyp, etikett, hjälptext,
     obligatoriskhet, tillåtna alternativ (bandval från den valda tariffens `nivaer[].id`,
     enumval från `tillatnaVarden`) och seriekardinalitet — ingen av dagens `type="number"`-
     genererade `indatafalt` räcker för band/enum/serie (§6a.2 punkt 5).
  7. **Felklassning (nytt i v9):** `KontraktBlockerat.saknadeFalt?: string[]` skiljer
     legitimt saknad kundindata (visas som ett vanligt formulärfel) från ett strukturellt
     ogiltigt värde (fel `vardetyp`, fel längd, okänt band-/enumval — förblir ett kastat
     kontraktsfel, når bara ett direkt API-anrop som kringgår det genererade UI:t) (§6a.2
     punkt 6).
  8. `KravPost.tillatna_varden` (§6a.6) och `KravPost.maxvarde`/`Tariffpolicy.kapacitet_
     ...
     (§6a.1/6a.3/6a.4/6a.5) — samtliga nya kontraktsfält samlas i SAMMA commit som
     grundarbetet, eftersom de delar samma `policyFranGenererad()`-mappningsbehov (§6a.1s
     tabell).
- **Filer:** `resultatkontrakt.py`/`.ts` (samtliga nya fält, `vardetyp`-/`antal_varden`-gren
  i `harled_resultatstatus`/`harledResultatstatus`, `IndataPost.varde`s vidgade typ),
  `katalog.py` (`till_prisar` bandbevaring), `faktura.py`/`fjarrvarme.ts`
  (`vald_niva_id`-parameter), `besparingsvarde.ts` (`byggIndataFranPolicy`,
  `KontraktBlockerat.saknadeFalt`), `KalkylatorPage.tsx` (renderar `policyFaltMetadata`
  i stället för att anta `type="number"` för allt), `energiPotential.ts` (oförändrad
  `falt`-vidareförmedling, verifierad med test), genererad testdata.
- **Teststrategi:** enhetstest per nytt `KravPost`-fält (giltigt/ogiltigt värde, kombination
  ...
  `byggIndataFranPolicy`-svit som bevisar att ETT policykrav utan `falt`-motsvarighet ger
  `blocked` med `saknadeFalt` satt (inte en generisk kastad `Error`) och att ett extra
  `falt`-fält utan policykrav ignoreras, ett integrationstest med en syntetisk policy som har
  ett numeriskt, ett band-ID- och ett seriekrav SAMTIDIGT, samt ETT syntetiskt
  end-to-end-test som faktiskt renderar `policyFaltMetadata` i ett testformulär, fyller i
  number + band-ID + enum + serie, och bevisar att den beräknade kostnaden stämmer — inte
  bara ett direkt anrop till `byggIndataFranPolicy`.
...
- **RÄTTAT P1 (granskning `2026-09-08-007`) — kontrollen måste köras mot den RÅA
  ...
  i en SEPARAT preflight mot `katalog["tariffs"]` direkt, körd omedelbart efter
  `ur_katalogen = godkanda(katalog)` i samma funktion (inventeringens §6a.4 har full
  pseudokod) — oberoende av om katalograden finns kvar i `ur_katalogen`.
...
- **Filer:** `resultatkontrakt.py`/`.ts` (2 nya serie-`KravPost`, 2 nya
  ...
  breddat effektkrav + nytt `ADAPTERREGISTER`), `generera.py` (`bygg_ts_fran_katalog()`s
  nya rå-katalog-preflight, inkl. `provider_id`-/omvänd-kontrollen, +
  `_bearbeta_leverantorsfil()`s `_kraver_kontrakt`-sättning), `besparingsvarde.ts`
  (det nya typade besparings-exkluderingsfelet).
- **Teststrategi:** eget källverifierat referensfall (inte bara Åkermannen-fixturens
  ...
  ett test att `ADAPTERREGISTER`-preflighten kastar på fel `provider_id`, fel `tariff_id`,
  saknad/stale mappning, ELLER policytäckning utan adapter (den omvända regeln), ett test
  att effektkravet nu gäller BÅDA `monthly`/`annual`, ett test att ett besparingsanrop för
  Stockholm kastar det typade exkluderingsfelet, OCH ett test att generatorn producerar
  EXAKT ETT UI-val för Stockholm Exergi (inte två) efter preflighten.
- **Visas för användaren:** mwh-läge med obligatoriska fält (uppskattad aktuell årskostnad),
  under leverantörs-ID:t `stockholm-exergi`; kr och schablon BLOCKERADE (frusen status,
  samma som inventeringens rad och §2:s generella regel); ingen besparingssiffra visas för
  denna produkt förrän en verifierad före/efter-regel finns.
