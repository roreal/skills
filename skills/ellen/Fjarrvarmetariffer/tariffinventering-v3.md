# Tariffinventering v3.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-08 av Claude. Ersätter `tariffinventering-v2.md` i sin helhet (v2 ändras
...
(`status: changes-required`) av v2 och den ursprungliga överlämningen
...
**Vad som är nytt i v3, i korthet** (se granskning `2026-09-08-002` för fullständig
motivering till varje punkt):

1. **Obligatoriska indata rättade för 16 rader** med en priced justeringspost som saknades:
   sex fullårsflödestariffer (Borlänge, Falu Falun, Falu ytterorter, Habo, Jönköping,
   Mjölby), sju säsongsflödestariffer med varje tariffs FAKTISKA `months`-lista (inte ett
   generellt oktober–april-antagande — Luleå 9 månader, Öresundskraft-raderna 5 månader,
   PiteEnergi-raderna 6 månader, Nevel/Tekniska Verken Linköping/Mälarenergi 7 månader),
   Kraftringens `Tf`, Finspångs villkorade flöde, Stockholm Exergis period-/
   upplösningskontrakt för de fem vintermånaderna. Se §4.
2. **Eskilstuna flyttad från `ready_to_implement` till `blocked_external_info`** (§4.2) —
   nätreferensen är inte verifierad som statiskt katalogvärde, `ready` fick inte längre
   vara villkorat av en framtida extern verifiering.
3. **Samtliga sju specialvariantfamiljer (14 variant-ID:n) integrerade i den räknade
   kontrollmängden** (§5) i stället för att stå utanför totalen — kontrollmängden är nu
   **92 räknade enheter** (78 bastariffer + 14 varianter), inte 78. Se §6.
4. **Primärkälla och verklig giltighet på samtliga 78 bastariffrader** — `source_id` +
   klickbar URL, samt `valid_from`/`valid_to` explicit som `unknown` där katalogen har
   `null` (69 av 78 rader).
5. **Gotland Taxa 17:s beskrivning rättad** (§3) — `foregaende_ars_mwh` är inte en
   `volume_discount`-justeringspost (raden har `adjustments: []`); den styr i stället
   TARIFFVALET mellan Taxa 17/Taxa 21 i den befintliga volymrabatt-bandvalslogiken.
6. **Batchplanen (`batchplan-v3.md`) omarbetad**: batch 5 delad i tre undergrupper efter
   faktisk motorsemantik (ingen justering / fullårsflöde / säsongsflöde, i stället för v2:s
   enda felaktiga 5a/5b-delning), Eskilstuna borttagen från batch 4, nya delbatcher 3b/6b
   för de nu räknade E.ON/Navirum- och Finspång-varianterna, alla ID:n fullständigt
   utskrivna, den maskinspecifika länken i teknisk-kartläggningen rättad.
...
- **Prisår/giltighet:** 2026, `optimate-fjarrvarme-2026.json` (se proveniens)
- **Primärkälla:** `03_0` (https://www.prisdialogen.se/wp-content/uploads/2020/11/Prisandringsmodell-2025-Eon-Jarfalla.pdf)
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
...
- **Prisår/giltighet:** 2026, `optimate-fjarrvarme-2026.json` (se proveniens)
- **Primärkälla:** `04_0` (https://www.prisdialogen.se/wp-content/uploads/2020/11/Prisandringsmodell-2025-Eon-Malmo.pdf)
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
- **Källstatus:** källgranskad (verifieringslistan 2026-09-04; teknisk-kartläggning v4)
- **Katalogstatus:** `production_ready: false`, `investigation.status: utreds` — väntar på denna implementationsomgång, inte på nytt leverantörsbesked
- **Motorstatus:** NYTT MOTORARBETE (supply_temperature_adjusted_flow)
...
- **Prisår/giltighet:** 2026, `optimate-fjarrvarme-2026.json` (se proveniens)
- **Primärkälla:** `25_0` (https://www.prisdialogen.se/wp-content/uploads/2020/11/Prisandringsmodell-2025-Eon-Navirum-Norrkoping.pdf)
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
- **Källstatus:** källgranskad (verifieringslistan 2026-09-04; teknisk-kartläggning v4)
- **Katalogstatus:** `production_ready: false`, `investigation.status: utreds` — väntar på denna implementationsomgång, inte på nytt leverantörsbesked
- **Motorstatus:** NYTT MOTORARBETE (supply_temperature_adjusted_flow)
...
- **Prisår/giltighet:** 2026, `optimate-fjarrvarme-2026.json` (se proveniens)
- **Primärkälla:** `26_0` (https://www.prisdialogen.se/wp-content/uploads/2020/11/Prisandringsmodell-2025-Eon-Navirum-Orebro.pdf)
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
- **Källstatus:** källgranskad (verifieringslistan 2026-09-04; teknisk-kartläggning v4)
- **Katalogstatus:** `production_ready: false`, `investigation.status: utreds` — väntar på denna implementationsomgång, inte på nytt leverantörsbesked
- **Motorstatus:** NYTT MOTORARBETE (supply_temperature_adjusted_flow)
...
- **Prisår/giltighet:** 2026, `optimate-fjarrvarme-2026.json` (se proveniens)
- **Primärkälla:** `46_0` (https://www.prisdialogen.se/wp-content/uploads/2020/11/Prisandringsmodellen-VanerEnergi-2025.pdf)
- **Giltighet:** valid_from=unknown (katalogens `valid_from` är null), valid_to=unknown (katalogens `valid_to` är null)
- **Källstatus:** källgranskad (verifieringslistan 2026-09-04)
- **Katalogstatus:** `production_ready: false`, `investigation.status: utreds` — väntar på denna implementationsomgång, inte på nytt leverantörsbesked
- **Motorstatus:** Befintlig (`selected_band_affine`/säsongsenergi)
...
| Variant-ID | Bastariff | Vad den kräver | Obligatorisk indata | Inmatningsläge | Batch | Disposition |
|---|---|---|---|---|---|---|
| `e-on-jarfalla-jarfalla-och-upplands-bro-bostader-2026--bas-delvarme` | `e-on-jarfalla-jarfalla-och-upplands-bro-bostader-2026` | 36-månadersmetoden för kunder med bas-/delvärmekälla i stället för fullvärme; egen formel, källverifierad i verifieringslistan men inte kartlagd i detalj i teknisk-kartläggning v4 (som bara behandlar fullvärme) | Debiterbar effekt, 36 månaders rullande medelvärde av framledningstemp (avtalet), flöde | mwh; kr/schablon blockerade | 3b (efter batch 3) | `ready_to_implement` |
| `e-on-jarfalla-jarfalla-och-upplands-bro-ovriga-fastigheter-2026--bas-delvarme` | samma, övriga fastigheter | samma | samma | samma | 3b | `ready_to_implement` |
| `e-on-malmo-malmo-och-burlov-bostader-2026--bas-delvarme` | samma, Malmö/Burlöv bostäder | samma | samma | samma | 3b | `ready_to_implement` |
| `e-on-malmo-malmo-och-burlov-ovriga-fastigheter-2026--bas-delvarme` | samma, övriga fastigheter | samma | samma | samma | 3b | `ready_to_implement` |
| `navirum-energi-norrkoping-och-soderkoping-norrkoping-och-soderkoping-bostader-2026--bas-delvarme` | samma | samma | samma | samma | 3b | `ready_to_implement` |
| `navirum-energi-norrkoping-och-soderkoping-norrkoping-och-soderkoping-ovriga-fastigheter-2026--bas-delvarme` | samma | samma | samma | samma | 3b | `ready_to_implement` |
| `navirum-energi-orebro-kumla-och-hallsberg-orebro-kumla-och-hallsberg-bostader-2026--bas-delvarme` | samma | samma | samma | samma | 3b | `ready_to_implement` |
| `navirum-energi-orebro-kumla-och-hallsberg-orebro-kumla-och-hallsberg-ovriga-fastigheter-2026--bas-delvarme` | samma | samma | samma | samma | 3b | `ready_to_implement` |
| `sodertorns-fjarrvarme-sodertorns-fjarrvarme-2026--kundvald-effekt` | `sodertorns-fjarrvarme-sodertorns-fjarrvarme-2026` | Kundvald effekt (i stället för SFAB:s rekommenderade) med egen överuttagsavgift; formeln är inte kartlagd i detalj och skiljer sig från katalogschemats `capacity_overrun`-typ | Okänt tills källfrågan är löst | samtliga blockerade | Ej batchad | `blocked_external_info` — fråga: "Vilken exakt formel/sats gäller för överuttagsavgiften vid kundvald effekt, och skiljer den sig från standardschemats `capacity_overrun`?" |
| `kraftringen-kraftringen-2026--brunnshog` | `kraftringen-kraftringen-2026` | Brunnshögs egen nätdel/prisstruktur, inte kartlagd — bara ordinarie nät är verifierat i §4.1 | Okänt tills källfrågan är löst | samtliga blockerade | Ej batchad | `blocked_external_info` — fråga: "Vilken är Brunnshögs egen prislista/formel, och skiljer den sig från Kraftringens ordinarie nät?" |
| `tekniska-verken-linkoping-linkoping-2026--lagtemperatur` | `tekniska-verken-linkoping-linkoping-2026` | Lågtemperaturleveransens egen tariffstruktur, inte kartlagd i detalj | Okänt tills källfrågan är löst | samtliga blockerade | Ej batchad | `blocked_external_info` — fråga: "Vilken är lågtemperaturleveransens fullständiga prisstruktur (kapacitet, energi, ev. justeringar)?" |
| `finspangs-tekniska-verk-finspang-2026--spetsvarmetillagg` | `finspangs-tekniska-verk-finspang-2026` | Spetsvärmetillägget (20 %); känt procentvillkor men inte kartlagt vilka kunder/perioder som utlöser det | P-värde, returtemp, flöde (samma som bastariffen) + bekräftelse av utlösningsvillkoret | mwh; kr/schablon blockerade | 6b (efter batch 6) | `ready_to_implement` |
| `jonkoping-energi-jonkoping-och-granna-2026--accessavgift` | `jonkoping-energi-jonkoping-och-granna-2026` | Avtalsberoende accessavgift (0/10/25/50 kr/mån) — beror på det enskilda avtalet, inte en publicerad allmän regel | Okänt tills Robert beslutar om kundformulärfält eller permanent uteslutning | samtliga blockerade | Ej batchad | `blocked_external_info` — VÄNTAR PÅ ROBERT-BESLUT (produktbeslut, inte källfråga): ska accessavgiften bli ett kundformulärfält (kunden anger sitt avtalade belopp) eller uteslutas permanent ur produkten? Claude får inte avgöra detta själv. |
| `boras-energi-och-miljo-boras-sjomarken-sandared-dalsjofors-fristad-2026--miljotillagg` | `boras-energi-och-miljo-boras-sjomarken-sandared-dalsjofors-fristad-2026` | Miljötillägget "Bra Miljöval" (31 SEK/MWh) — ett kundvalt UI-tillval, inte en automatisk prisdel | Kryssruta/kundval i UI, inget nytt fält utöver bastariffens | mwh; kr/schablon blockerade | 6 (samma batch som grundformeln) | `ready_to_implement` |

**14 variant-ID:n totalt:** 10 `ready_to_implement` (åtta E.ON/Navirum-varianter, Finspångs
tillägg, Borås tillägg), 4 `blocked_external_info` (Södertörn, Kraftringen Brunnshög,
Tekniska Verken Linköping lågtemperatur, Jönköping accessavgift). Jönköpings variant väntar
uttryckligen på ett Robert-BESLUT, inte bara ett leverantörssvar — se öppen fråga 1 nedan.
...
utanför den — kontrollmängden är därför nu **92 räknade enheter**, inte 78. Verifierat: 7 +
55 + 30 = 92, och 78 (bas) + 14 (variant) = 92.
...
2. **E.ON/Navirums 36-månadersvarianter och Finspångs spetsvärmetillägg** (batch 3b/6b,
   §5): föreslås som egna, mindre delbatcher EFTER respektive grundformels batch. Bekräfta
   ordningen, eller flagga om de i stället ska in i SAMMA batch som grundformeln.
3. **Batch 5-uppdelningen** (5a/5b/5c i `batchplan-v3.md`) grupperar nu efter faktisk
...
Samtliga övriga öppna frågor från v1 och v2 är besvarade av Codex i granskningarna
`2026-09-08-001` och `2026-09-08-002` och tillämpade rakt av i denna v3: Vattenfall helt
`blocked` som grupp (batch 8), Sundsvall Matfors följer teknisk-kartläggning v4,
leverantörsvärde-mönstret kräver tariffvis kontroll av VARJE prisdel (genomfört i §4 för
samtliga 45 `ready`-bastariffer), och kända specialvarianter får aldrig bli `not_applicable`
(genomfört i §5 — alla sju integrerade i den räknade kontrollmängden).
