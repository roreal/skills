# Batchplan v16.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v15.md` i sin helhet (v15 ändras INTE i
efterhand — kvar som historik), som svar på TVÅ Codex-dokument samtidigt: omgranskning
[2026-09-09-007](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v15.md)
(supersedes `2026-09-09-005`) och bedömning
[2026-09-09-006](../conversations/reviews/2026/09/2026-09-09-bedomning-lidkoping-energi-leverantorssvar.md)
av Lidköping Energis leverantörssvar. Bygger på dispositionerna i
[`tariffinventering-v16.md`](tariffinventering-v16.md) §4.1 (47 bastariffer) och §5 (10
...
prioritering. Dispositionerna ändras enligt bedömning 006: **7 implementerade / 57 redo / 28
blockerade / 92 totalt** (från 7/55/30/92, se ny batch 5d nedan).

**Vad som är nytt i v16** (se granskning `2026-09-09-007` och bedömning `2026-09-09-006` för
...
1. **`beraknaArsprodukt`s fullständiga kropp är omskriven och verifierad kompilerbar som
   ...
   `tariffinventering-v16.md` §6a.4, ingen separat beskrivning här; batchplanen refererar
...
- **Innehåll:**
  ...
  5. **Rättat P1 (granskning `2026-09-08-009`) — egen, typad produkt-DTO i stället för det
     ...
     **Rättat P1 (granskning `2026-09-09-007`) — v15:s inventeringsdokument motsade denna
     ...
     speglat i `tariffinventering-v16.md` §6a.4. `beraknaBesparingsvarde(args)` skickar redan
...
`tariffinventering-v16.md` §6a.7 för fullständig design, formel, fail-closed-kontrakt och
...
- **Tariffer:** `lidkoping-energi-lidkoping-041-kw-2026`, `lidkoping-energi-lidkoping-42-kw-2026`
  ...
  `tariffinventering-v16.md` §4.2 och §8).
...
- **Fail-closed:** saknad serie → `missing_policy_fields`; fel längd/icke-ändligt element →
  ...
  (division med noll får aldrig nå motorn odetekterad). `Tm_m` är ett RUNTIMEKRAV per
  kundfall (leverantörssvaret ger metoden, inte de tolv faktiska talen) — resultatet visas
  `uppskattat` tills fakturaverifierat.
- **Visas för användaren:** tre serieinmatningar (eller fakturaimport) för `Q_m`/`T_m`/
  `Tm_m`, tydliga enheter (m³, °C) och kalendermånader; kr/schablon blockerade som för
  övriga kontraktsgatade tariffer.
- **Planerat, INTE genomfört av detta dokument:** katalogens `adjustments`-post, ny
  `POLICYREGISTER`-post, generering, UI-formulärfälten, samt golden-/gräns-/negativtester
  (§6a.7:s nio testfall). Ingen JSON eller produktkod ändras av `tariffinventering-v16.md`/
  `batchplan-v16.md`.
...
adderas: 55 + 2 = **57**, exakt matchande `tariffinventering-v16.md` §8: 47 bastariffer +
