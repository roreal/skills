# Batchplan v10.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-08 av Claude. Ersätter `batchplan-v9.md` i sin helhet (v9 ändras INTE i
...
(supersedes `2026-09-08-008`). Bygger på dispositionerna i
[`tariffinventering-v10.md`](tariffinventering-v10.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v10** (se granskning `2026-09-08-009` för fullständig motivering):

1. **En egen, typad produkt-DTO ersätter antagandet att det numeriska `falt` räcker** (batch
   0, §6a.2): nytt, PARALLELLT `policyFalt: Record<string, PolicyInputValue>` genom
   `KalkylatorPage` → `energiPotential` → `besparingsvarde` → `IndataPost` — det befintliga
   numeriska `falt` rörs inte och förblir legacyvägens fria fakturafält. `IndataPost.varde`
   får en NY, egen typ `IndataVarde = Varde | str`; den delade `Varde`-aliasen (som även
   `_validera_varde`/`valideraVarde` använder som sin parametertyp) rörs INTE — v9 hade
   påstått "ingen global vidgning" men vidgat just den delade aliasen ändå.
2. **UI-metadatan blir prispost- och omfattningsmedveten** (batch 0, §6a.2 punkt 6):
   `policyFaltMetadata(policy, prisar, omfattning)` (v9 tog bara `policy` och kunde därför
   varken bygga bandalternativ, skilja `monthly`/`annual` eller undvika att dubblera
   kapacitetsfältet). `KravPost` får obligatoriska `etikett`/`hjalptext`-fält i stället för
   en outtalad runtime-härledning; bevarade band-ID:n valideras unika/icke-tomma vid
   katalogbyggnad.
3. **Både saknad OCH ogiltig kundindata blir typade användarfel** (batch 0, §6a.2 punkt 7):
   nytt `KontraktBlockerat.ogiltigaFalt` vid sidan av `saknadeFalt` — v9 lät fel typ/längd/
   enumval förbli ett generiskt kastat kontraktsfel med motiveringen att UI:t "aldrig" kunde
   producera dem, vilket inte stämmer (delvis ifylld serie, gammalt sparat bandval, direkt
   API-anrop).
4. **Stockholm Exergi får en namngiven produktentry** (batch 7, §6a.4): ny
   `ArsprodukResultat`-union och `beraknaArsprodukt(args, onskadTyp)` — v9:s lösning stannade
   vid att motorlagret kastar för en besparingsförfrågan, utan att beskriva HUR den aktuella
   årskostnaden når `calcResult`/`KalkylatorPage` för visning.
5. **Adapterpreflighten blir en injicerbar, testbar tvåvägsfunktion utan `pass`** (batch 7,
   §6a.4): nytt `Tariffpolicy.ersatter_katalograd`-fält gör den omvända bijektionen exakt,
   utan v9:s självmotsägande "OR"-regel eller ett historiskt "redan godkänd"-undantag som
   inte går att kontrollera reproducerbart. `kontrollera_adapterpreflight()` tar
   `policyregister`/`adapterregister` som explicita argument, samma injektionsmönster som
   §6a.3:s `godkanda()`.
6. **Dispositionerna 7/55/30/92 oförändrade** — samtliga v10-fynd var körbarhets-/
   typningsfel i redan beslutade kontraktsdesigner, ingen post flyttar.

## Batch 0 — Grundkontrakt: diskriminerad värdetyp, bandbindning, seriekardinalitet och en genererad produkt-/UI-ingång (granskning 2026-09-08-007/-008/-009, P1)
...
- **Innehåll:**
  ...
  7. **Felklassning — saknad OCH ogiltig indata (rättat P1, granskning `2026-09-08-009`):**
     ...
     (§6a.2 punkt 6).
  8. **Namngiven produktentry för Stockholms aktuella årskostnad (nytt P1, granskning
     `2026-09-08-009`):** ny `ArsprodukResultat`-diskriminerad union och funktionen
     `beraknaArsprodukt(args, onskadTyp)` i `besparingsvarde.ts` — den enda produktentrypunkten
     `calcResult` anropar för en kontraktsgated tariff, med `KalkylatorPage.tsx`-rendering för
     `{ typ: 'aktuell_arskostnad' }` utan besparingsfält (§6a.4).
...
- **Filer:** `resultatkontrakt.py`/`.ts` (samtliga nya fält, `vardetyp`-/`antal_varden`-gren
  ...
  `policyFaltMetadata`-signatur), `katalog.py` (`till_prisar` bandbevaring + unikhetskontroll,
  `kontrollera_adapterpreflight`), `faktura.py`/`fjarrvarme.ts` (`vald_niva_id`-parameter),
