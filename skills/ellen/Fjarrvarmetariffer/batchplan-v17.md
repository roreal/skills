# Batchplan v17.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v16.md` i sin helhet (v16 ändras INTE i
...
(supersedes `2026-09-09-007`). Bygger på dispositionerna i
[`tariffinventering-v17.md`](tariffinventering-v17.md) §4.1 (47 bastariffer) och §5 (10
...
blockerade / 92 totalt** — v17 gör bara Lidköpings implementationsplan körbar, flyttar ingen
post.

**Vad som är nytt i v17** (se granskning `2026-09-09-010` för fullständig motivering):
...
- **Innehåll:**
  ...
  5. **Rättat P1 (granskning `2026-09-08-009`) — egen, typad produkt-DTO i stället för det
     ...
     **Rättat P1 (granskning `2026-09-09-007`) — v15:s inventeringsdokument motsade denna
     ...
     speglat i `tariffinventering-v17.md` §6a.4. `beraknaBesparingsvarde(args)` skickar redan
...
`tariffinventering-v17.md` §6a.7 för fullständig design, formel, fail-closed-kontrakt och
...
- **Tariffer:** `lidkoping-energi-lidkoping-041-kw-2026`, `lidkoping-energi-lidkoping-42-kw-2026`
  (båda flyttade från `blocked_external_info` till `ready_to_implement` i denna version, se
  `tariffinventering-v17.md` §4.2 och §8).
...
- **Rättat P1 (granskning `2026-09-09-010`) — v16:s motorkod var en fristående skiss utan
  anropskedja till den verkliga kostnadsmotorn.** V17:s fillista (nedan) täcker nu HELA
  vägen katalog → policy → fasad → motor → `Kostnad.justering`, inte bara ett direkt
  hjälpfunktionsanrop — se `tariffinventering-v17.md` §6a.7.1–§6a.7.3 för den fullständiga,
...
- **Förmågekontrakt (nytt, §6a.4/§6a.7.7):** `policyregister.py` sätter
  ...
  årskostnad" nåbar via `beraknaArsprodukt` — SAMMA explicita flagga blockerar samtidigt
  Lidköpings besparingsväg (`beraknaBesparingsvardeKontrakt` kastar ett typat
  produktbegränsningsfel, samma mönster som Stockholm) tills en källförsvarbar
...
- **Visas för användaren:** tre serieinmatningar (eller fakturaimport) för `Q_m`/`T_m`/
  `Tm_m`, tydliga enheter (m³, °C) och kalendermånader, med obligatorisk källhjälptext på
  `Tm_m`-fältet (§6a.7.6); kr/schablon blockerade som för övriga kontraktsgatade tariffer.
- **Fillista (planerad, INTE genomförd av detta dokument):**
  ...
  - `resultatkontrakt.ts`/`.py`: `beraknaArskostnadMedKontrakt`/motsvarigheten extraherar
    ...
    `Tariffpolicy.stodjer_aktuell_arskostnad`/`stodjerAktuellArskostnad` (§6a.4).
  - `policyregister.py`: ny Lidköping-policy med de tre `number_series`-kraven
    (`minVarde`/`minExklusiv` på `Tm_m`), `stodjer_aktuell_arskostnad=True`.
  - `generera.py`: katalogens `adjustments`-post med `type: "signed_monthly_flow_adjustment"`
    och `_policy_till_json()`-serialisering av de två nya `KravPost`-/`Tariffpolicy`-fälten.
  - `KalkylatorPage.tsx`: tre serieinmatningsfält, källhjälptext på `Tm_m`.
  - Katalog-JSON och tariffaktivering ändras INTE av denna etapp (§6a.7).
- **Golden-/gräns-/negativtester:** §6a.7:s tolv testfall (nio ursprungliga + tre nya i
  denna version, se `tariffinventering-v17.md` §6a.7 slutet), inklusive ett end-to-end-test
  mot `Kostnad.justering` (inte bara ett direkt beräkningsfunktionsanrop) och ett explicit
  test att Lidköpings besparingsväg blockeras. Ingen JSON eller produktkod ändras av
  `tariffinventering-v17.md`/`batchplan-v17.md`.
...
**Oförändrat i v17 (granskning `2026-09-09-010`):** Batch 5d:s omfattning (2 bastariffer)
är oförändrad — v17 gör bara implementationsplanen körbar (fullständig motorkedja,
förmågekontrakt, `minExklusiv`, se `tariffinventering-v17.md` §6a.7/§6a.4). Batchsumman
57 och räkningen 7/57/28 av 92 är oförändrade.
