# Tariffinventering v13.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v12.md` i sin helhet (v12 ändras
INTE i efterhand — kvar som historik), som svar på Codex omgranskning
[2026-09-09-003](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v12.md)
(`status: changes-required`, supersedes `2026-09-09-002`) av v12. Ingen produktkod, tariffdata
...
`harledResultatstatus`s faktiska `blocked`-retur (bara för SAKNADE fält, rad 378–380, listan
själv aldrig returnerad) mot dess `Error`-kastande valideringsloop (rad 385–420),
...
`calcResult`s tre `beraknaBesparingsvarde`-anrop med mid/min/max-grad (rad 500–538), och
`_bearbeta_leverantorsfil()`s verkliga policykälla (`policyregister.py` via
`_policy_till_json()`, `generera.py` rad 46–63) — inte bara omskrivna från granskningen.

**Vad som är nytt i v13, i korthet** (se granskning `2026-09-09-003` för fullständig
...
`batchplan-v13.md`), inte något Sandvikens nuvarande kapacitets-only-adapter redan täcker:
   ...
   - `PolicyInputValue = number | string | readonly number[]` (oförändrad typ, men nu ENDAST
     ...
     **Löst med EN delad orsaksunion, deklarerad EN gång och återanvänd överallt:**
     ...
     - `inmatningstyp: 'number'` (inklusive Jönköpings `enum_val`, som är ETT UI-läge för ett
       numeriskt `number`-krav, se P2-rättningen nedan): om `ravarde === undefined` eller
       den trimmade strängen är tom → `{status:'saknat'}`. Annars avvisar icke-ändligt/
       icke-parsbart tal med `{status:'ogiltigt', orsak:'numerik'}`, och konverterar annars
       med `Number(...)` till `{status:'parsed', varde}` — Jönköpings värde blir alltså
       TALET `10`, inte strängen `"10"`, och kan jämföras mot `tillatnaVarden: readonly
       number[]`.
     - `inmatningstyp: 'band_id_val'`: samma tomhetskontroll som ovan ger `{status:'saknat'}`;
       annars `{status:'parsed', varde}` med strängen OFÖRÄNDRAD — `Number(...)` anropas
       ALDRIG för band-ID.
     - `inmatningstyp: 'number_series'`: om `ravarde === undefined` → `{status:'saknat'}`.
       Om `ravarde.length !== antalVarden` → `{status:'ogiltigt', orsak:'kardinalitet'}`
       OMEDELBART, ingen elementvis parsning påbörjas. Annars trimmas och kontrolleras VARJE
       element enligt det skalära `number`-fallet ovan: om NÅGOT element är tomt (efter
       trim) → HELA serien `{status:'saknat'}`; om NÅGOT element (som inte är tomt) är
       icke-ändligt/icke-parsbart → HELA serien `{status:'ogiltigt', orsak:'numerik'}`;
       annars `{status:'parsed', varde: number[]}` — ALDRIG en delvis parsad serie med
       nollor på de tomma platserna.
...
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel, genom en
   ...
   **Löst med en separat valideringsfunktion FÖRE fasadanropet (Codex alternativ A),** i
   ...
   ReadonlyMap<string, IndataPost>): { saknade: readonly string[]; ogiltiga: readonly
   PolicyValideringsFel[] }` (`resultatkontrakt.ts`, mirror `forkontrollera_policy_indata` i
   Python) — körs av `beraknaBesparingsvardeKontrakt`/`beraknaArsprodukt` (§6a.4) OMEDELBART
   efter `byggIndataFranPolicy` men INNAN `beraknaArskostnadMedKontrakt`/
   `harledResultatstatus` någonsin anropas. Itererar `policy.kravdaFalt` EN gång:

   - Fält som INTE finns i `indata` läggs i `saknade` (bara nyckelnamnet — samma form som
     `KontraktBlockerat.saknadeFalt` redan har i dag) och får ALDRIG någon vidare
     värdekontroll (det finns inget värde att kontrollera).
   - Fält som FINNS i `indata` kontrolleras och kan hamna i `ogiltiga`: `vardetyp` (skalärt
     tal för `number`, icke-tom talserie för `number_series`, icke-tom sträng för
     `f.vardetyp === 'band_id'`), `antalVarden` för seriekrav, `tillatnaVarden`-medlemskap
     för enum, `f.minVarde`/`f.maxVarde` (samtliga element för en serie) och `f.heltal`
     (`Number.isInteger` på samtliga element) med orsak `'numerik'` för alla tre gräns-/
     heltalsfall (samma kontroller `harledResultatstatus`s kastande loop, rad 385–420, redan
     gör i dag — bara flyttade hit FÖRE den kastande loopen nås), och — för fält där
   ...
   **Löst genom att `forkontrolleraPolicyIndata` själv beräknar BÅDA listorna** direkt ur
   ...
   för hand. `KontraktBlockerat` får sitt fält `ogiltigaFalt?: PolicyValideringsFel[]` fyllt
   FRÅN denna funktions `ogiltiga`, och dess BEFINTLIGA `saknadeFalt?: string[]` fylls nu FRÅN
   denna funktions `saknade` — inte längre från `harledResultatstatus`s väg, som därmed inte
   längre är den enda platsen `saknadeFalt` kan härledas ur.

   Om `saknade.length > 0` ELLER `ogiltiga.length > 0`: `beraknaBesparingsvardeKontrakt`/
   `beraknaArsprodukt` kastar `KontraktBlockerat` med respektive fält satt (aldrig en generisk
   `Error` för dessa fall) och `harledResultatstatus` anropas ALDRIG i det fallet — den gröna
   vägen (`saknade`/`ogiltiga` båda tomma) fortsätter oförändrat till
...
export function stodjerAktuellArskostnad(prisar: GenereradPrisarspost): boolean {
  const policy = kontraktsgatadPolicy(prisar); // enda tillåtna rådatagrind
...
INGEN `onskadTyp`-parameter, eftersom den bara har en gren att dispatcha till. Logik:

1. Kör `stodjerAktuellArskostnad(prisar)` FÖRST, som funktionens EGEN, auktoritativa spärr —
   kastar ett typat fel om `false` (samma fail-closed-princip som §6a.2s
   `forkontrolleraPolicyIndata`: domänlagret litar aldrig på att anroparen redan kontrollerat
   detta). Sidnivå-wrapperns kontroll (nedan) är en UX-förkontroll som undviker ett onödigt
   anrop och ger ett tidigare, fältnära fel i formuläret — INTE den enda spärren.
2. Anropar `byggKontraktIndata` en gång.
3. Kör `forkontrolleraPolicyIndata` (§6a.2, punkt 6) — ett fältnära `KontraktBlockerat` för
   kundindatafel, precis som den befintliga kontraktsgated vägen redan gör.
4. ETT anrop till `beraknaArskostnadMedKontrakt`, returnerar `{ typ: 'aktuell_arskostnad',
...
**Löst genom att klyva typen i en delad bastyp och en besparingsspecifik utökning**, och
extrahera `argsFranInputs` ur `calcResult`s BEFINTLIGA, oförändrade logik för scope-
uppskalning och proveniens (verifierat i `energiPotential.ts`, rad 440–545: `calcResult`
härleder redan `totalMwhForTariff` via rumsvärme-scope-uppskalning när `rumsvarmeAngiven`,
och `mwhProvenansBekraftad`/`energyProvenance` innan den anropar `beraknaBesparingsvarde` tre
gånger med `mid`/`min`/`max`-graderna — ingen av dessa härledningar är ny, bara flyttad ut ur
`calcResult`s kropp och namngiven):
...
export function argsFranInputs(inputs: KalkylatorInputs): Tariffberakningsunderlag {
  // Extraherad, oförändrad ur calcResults befintliga rumsvärme-scope-uppskalning
  // (energiPotential.ts rad ~460–490) och mwhProvenansBekraftad-härledning
  // (rad ~500–520) — ingen ny beräkningslogik, bara namngiven och återanvändningsbar.
  const totalMwh = inputs.rumsvarmeAngiven
    ? skalaUppRumsvarmeTillTotal(inputs) // oförändrad, befintlig hjälpfunktion
    : inputs.totalMwh;
  return {
    totalMwh,
    ...
    energyProvenance: mwhProvenansBekraftad(inputs) ? 'confirmed_mwh' : undefined,
  };
}

// calcResult självt förblir OFÖRÄNDRAT (se ovan): dess tre befintliga
// beraknaBesparingsvarde-anrop (min/mid/max) fortsätter bygga sin egen
// BesparingsvardeArgs inline, precis som i dag — BesparingsvardeArgs' fält
// paverkbarMwh/besparingsgrad ändras inte, och Tariffberakningsunderlag är en
// STRUKTURELL DELMÄNGD av BesparingsvardeArgs (samma fältnamn/typer för de fält
// de har gemensamt), inte ett separat, inkompatibelt schema.
...
export function calcResultForOnskadTyp(inputs: KalkylatorInputs): KalkylatorResultUnion {
  if (inputs.onskadTyp === 'aktuell_arskostnad') {
    ...
    if (!stodjerAktuellArskostnad(prisar)) {
      ...
      // INTE den enda spärren — beraknaArsprodukt upprepar samma kontroll (se ovan)
    }
...
fillista (`batchplan-v13.md`), där Jönköpings basrad redan ligger.
