# Tariffinventering v12.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v11.md` i sin helhet (v11 ändras
...
(`status: changes-required`, supersedes `2026-09-09-001`) av v11. Ingen produktkod, tariffdata
eller genererad fil är ändrad av detta dokument. Alla kodpåståenden nedan är verifierade
genom att läsa den verkliga koden i `enkey-agents@fd8f8da`/`neptune_academy@f1df177` — bl.a.
`harledResultatstatus`s faktiska `blocked`-retur (bara för SAKNADE fält, rad 378–380) mot
dess `Error`-kastande valideringsloop (rad 385–420) och `KalkylatorResult`s obligatoriska,
icke-valfria `savingsMin`/`savingsMax`/`annualSavingsKr`-fält (`energiPotential.ts`) — inte
bara omskrivna från granskningen.

**Vad som är nytt i v12, i korthet** (se granskning `2026-09-09-002` för fullständig
motivering till varje punkt):

1. **Parserns felresultat är nu en typkorrekt, diskriminerad union** (§6a.2): `parsaPolicyIndata`
   returnerar `{status:'parsed', varde} | {status:'saknat'} | {status:'ogiltigt', orsak}` i
   stället för det tidigare `PolicyInputValue | PolicyValideringsFel` som inte kunde bära
   `'saknat'`. Tomt/blankt fält klassas identiskt oavsett hur React-statet initialiserades.
2. **`valideraPolicyIndata` täcker nu ALLA kundvärdesregler** (§6a.2): `minVarde`, `maxVarde`,
   `heltal` läggs till bredvid de befintliga kontrollerna, och bandfält identifieras via
   `vardetyp === 'band_id'` i stället för en metadataegenskap validatorn inte tar emot.
   `maxVarde`-transporten null-normaliseras (`?? undefined`).
3. **`bygg_ts()` får ett verkligt, körbart anropskontrakt** (§6a.4): `kontrollera_adapterpreflight`
   kör riktning 1 (katalogberoende) BARA när en riktig katalog skickas in; `bygg_ts()` skickar
   `rak_katalog=None` och kör bara riktning 2 (bijektionens reverse-led) — en uttryckligen
   avgränsad, mindre garanti än `bygg_ts_fran_katalog()`s fulla tvåvägskontroll, inte en låtsad
   fullständig bijektion.
4. **Årsproduktdispatchen har en entydig källa till `onskadTyp` och en verklig argumentbyggare**
   (§6a.4): `onskadTyp` finns bara på `KalkylatorInputs`, en ny `stodjerAktuellArskostnad(prisar)`
   ger explicit produktförmåga, `argsFranInputs` extraheras ur `calcResult`s befintliga logik.
   Besparingsvägen är uttryckligen den BEFINTLIGA, oförändrade `calcResult`-vägen — inte en
   omkonstruktion inne i `beraknaArsprodukt`, som nu bara har EN gren (`aktuell_arskostnad`).

**Historik — vad som var nytt i v11** (bevarat för spårbarhet, INTE en beskrivning av v12:s
aktuella design; punkt 3 och 5 nedan beskrivs som v11 löste dem, men båda rättades sedan
YTTERLIGARE i v12 — se "Vad som är nytt i v12" ovan och §6a.2/§6a.4 för den gällande
designen):
...
Negativa test krävs för samtliga tolv fält ovan: en genererad policy med fältet satt som
TypeScript läser tillbaka korrekt (inte `undefined`), och en policy UTAN fältet som fortsatt
ger `undefined` (inte t.ex. tomsträng eller `null` feltolkat som ett giltigt bandval) — utom
`etikett`/`hjalptext`, som är OBLIGATORISKA på varje `KravPost` och därför testas med ett
kastat fel vid genereringstillfället i stället för ett tillåtet `undefined`-fall.
...
`batchplan-v12.md`), inte något Sandvikens nuvarande kapacitets-only-adapter redan täcker:
...
1. **Rättat P1 (granskning `2026-09-08-009`) — separat, typad produkt-DTO i stället för att
   ...
   **Löst med två separata typer:**
   ...
   - `PolicyInputValue = number | string | readonly number[]` (oförändrad typ, men nu ENDAST
     ...
     PolicyParseResultat` (`resultatkontrakt.ts`), anropad först vid submit (samma tidpunkt
     som `faltForBerakning` redan byggs i dag).

     **Rättat P1 (granskning `2026-09-09-002`) — resultattypen kan nu faktiskt UTTRYCKA
     "saknat värde".** Den tidigare deklarationen `PolicyInputValue | PolicyValideringsFel`
     hade inget sätt att representera ett tomt/blankt fält: `PolicyValideringsFel` (§6a.2 punkt
     6) bär bara `orsak: 'typ' | 'numerik' | 'kardinalitet' | 'okant_val'` — ingen av dessa
     betyder "inget värde alls" — och att pressa in en tom sträng i `PolicyInputValue` självt
     hade gjort den omöjlig att skilja från Jönköpings giltiga `enum_val`-noll. Ny, diskriminerad
     union i stället: `type PolicyParseResultat = {status: 'parsed'; varde: PolicyInputValue} |
     {status: 'saknat'} | {status: 'ogiltigt'; orsak: 'typ' | 'numerik' | 'kardinalitet'}`
     (`resultatkontrakt.ts`, mirror i Python om/när ett eget produktkonsument-anrop tillkommer).
     ...
     tömts av användaren (båda ger `{status:'saknat'}`, aldrig en `undefined`-nyckel som
     tyst hoppar över kontrollen). `'okant_val'` ingår MEDVETET inte i denna funktions
     `orsak`-mängd — bandvalets giltighet mot `prisar.kapacitet.nivaer[].id` kräver
     prisdata som `parsaPolicyIndata` (bara `metadata`+`ravarde`) inte har tillgång till, och
     avgörs i stället av `valideraPolicyIndata` (punkt 6) i nästa steg:
     - `inmatningstyp: 'number'` (inklusive Jönköpings `enum_val`, som är ETT UI-läge för ett
       numeriskt `number`-krav, se P2-rättningen nedan): trimmar strängen, avvisar tom sträng
       som "saknat värde" (INTE `Number('')===0`), avvisar icke-ändligt/icke-parsbart tal,
       konverterar annars med `Number(...)` — Jönköpings värde blir alltså TALET `10`, inte
       strängen `"10"`, och kan jämföras mot `tillatnaVarden: readonly number[]`.
     - `inmatningstyp: 'band_id_val'`: trimmar strängen, avvisar tom sträng som saknat värde,
       returnerar annars OFÖRÄNDRAD som sträng — `Number(...)` anropas ALDRIG för band-ID.
     - `inmatningstyp: 'number_series'`: kräver att `ravarde` har EXAKT `antalVarden` element
       (annars `PolicyValideringsFel` med orsak `'kardinalitet'` omedelbart, ingen
       elementvis parsning ens påbörjas), trimmar och parsar VARJE element som ett skalärt
       `number`-fält ovan — om NÅGOT element är tomt/icke-ändligt blir HELA serien ett
       `PolicyValideringsFel` (orsak `'numerik'` eller `'saknat'`), aldrig en delvis parsad
       serie med nollor på de tomma platserna.
...
2. `KalkylatorPage.tsx` bygger `policyFalt: Record<string, PolicyInputValue>` VID SUBMIT
   ...
   `ogiltiga: PolicyValideringsFel[]`-lista (`{nyckel, orsak}` — samma form `valideraPolicyIndata`
   i punkt 6 redan producerar, så BÅDA felkällorna münnar ut i EN gemensam
   `PolicyValideringsFel[]`-typ). Finns NÅGON post i endera listan stoppas submit direkt och
   mappas till `KontraktBlockerat.saknadeFalt`/`ogiltigaFalt` (punkt 6) UTAN att ens nå
   `besparingsvarde.ts` — `byggIndataFranPolicy` (nästa) tar därför bara emot fält där VARJE
   `PolicyParseResultat` var `status:'parsed'`, och bygger `Record<string, PolicyInputValue>`
   genom att packa upp `.varde` från just de posterna.
...
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel, genom en
   ...
   **Löst med en separat valideringsfunktion FÖRE fasadanropet (Codex alternativ A),** i
   ...
   `valideraPolicyIndata(policy: Tariffpolicy, prisar: GenereradPrisarspost, indata:
   ReadonlyMap<string, IndataPost>): PolicyValideringsFel[]` (`resultatkontrakt.ts`, mirror
   `valider_policy_indata` i Python) — körs av `beraknaBesparingsvardeKontrakt`/
   `beraknaArsprodukt` (§6a.4) OMEDELBART efter `byggIndataFranPolicy` men INNAN
   `beraknaArskostnadMedKontrakt`/`harledResultatstatus` någonsin anropas. Itererar
   `policy.kravdaFalt`, och för varje fält som FINNS i `indata` (saknade fält lämnas
   OBERÖRDA — de hanteras precis som i dag av `harledResultatstatus`s egna `blocked`-väg,
   ingen dubblering av den kontrollen): kontrollerar `vardetyp` (skalärt tal för `number`,
   icke-tom talserie för `number_series`, icke-tom sträng för `f.vardetyp === 'band_id'`),
   `antalVarden` för seriekrav, `tillatnaVarden`-medlemskap för enum, och — för fält där
   `f.vardetyp === 'band_id'` (samma diskriminator som resten av §6a.2 använder för att peka
   ut bandfält, INTE en separat `band_id_val`-UI-metadataegenskap denna funktion aldrig tar
   emot; `valideraPolicyIndata` ser bara `Tariffpolicy`/`prisar`/`IndataPost`, inte
   `PolicyFaltMetadata`) — slår upp värdet mot `prisar.kapacitet.nivaer[].id` och returnerar
   `{nyckel, orsak: 'okant_val'}` om ingen träff finns.

   **Rättat P1 (granskning `2026-09-09-002`) — täckningen var ofullständig.** Den tidigare
   versionen kontrollerade bara `vardetyp`/`antalVarden`/`tillatnaVarden`/bandmedlemskap —
   ALDRIG `KravPost.minVarde`/`maxVarde`/`heltal` (§6a.1, §6a.3), trots att dessa är precis
   de fält `harledResultatstatus`s kastande loop (rad 385–420) i dag validerar och som denna
   funktion finns till för att flytta UT ur den kastande vägen. Ett värde som klarade
   `vardetyp`-kontrollen men låg utanför `minVarde`/`maxVarde` eller bröt ett `heltal`-krav
   (t.ex. Falus 500 kW-tak, §6a.1) föll alltså ändå igenom till `harledResultatstatus` och
   kastade ett okontrollerat `Error` i stället för att ge ett fältnära `PolicyValideringsFel`.
   `valideraPolicyIndata` kontrollerar nu, för varje fält som FINNS i `indata` och har
   `f.vardetyp === 'number'` eller `'number_series'`: `f.minVarde` (samtliga element, för en
   serie), `f.maxVarde` (samma), och `f.heltal` (`Number.isInteger` på samtliga element) —
   samma tre kontroller `harledResultatstatus` redan gör, bara flyttade hit FÖRE den kastande
   loopen nås, med orsak `'numerik'` för alla tre (gräns- och heltalsbrott är båda numeriska
   fel, ingen ny `orsak`-variant krävs).

   Returnerar en TOM lista om allt är giltigt (den vanliga, gröna vägen fortsätter oförändrat
   till `harledResultatstatus`); en ICKE-TOM lista `{nyckel: string; orsak: 'typ' | 'numerik' |
   'kardinalitet' | 'okant_val'}[]` STOPPAR anropet direkt.

   `KontraktBlockerat` får ett nytt fält `ogiltigaFalt?: PolicyValideringsFel[]` vid sidan av
   det befintliga `saknadeFalt?: string[]`. `beraknaBesparingsvardeKontrakt`/
   `beraknaArsprodukt` kastar `KontraktBlockerat` med `ogiltigaFalt` satt när
   `valideraPolicyIndata` returnerar en icke-tom lista (aldrig en generisk `Error` för dessa
   fall), och fortsätter annars in i den befintliga `saknadeFalt`-vägen (oförändrad — den
   härleder redan `saknade`-listan korrekt från `harledResultatstatus`s `blocked`-status).
   ...
   redan av `valideraPolicyIndata` men klassas som `Error`, inte `PolicyValideringsFel`,
   eftersom de inte är kundens fel att rätta.
   ...
   det generiska felmeddelandet. Ska testas identiskt i BÅDA språken (rättat P2, granskning
   `2026-09-09-002`: v11 påstod detta redan var gjort — ingen sådan testsvit existerar ännu i
   den checkade-in koden, detta är ett åtagande för Batch 0, inte en genomförd verifiering):
   tom/delvis ifylld serie,
   icke-ändligt tal, okänt band-ID mot en verklig prisposts `nivaer`, ett Jönköpingsvärde
   utanför `tillatnaVarden` — via BÅDE UI:t (punkt 5:s end-to-end-test) OCH ett direkt
   produktanrop som kringgår formuläret helt.
...
katalogberoende adapter som Stockholms). `bygg_ts()` förblir korrekt och FULLT verifierad
(båda riktningarna, ingen avgränsning) för leverantörsfiler UTAN `ersatter_katalograd` —
riktning 2 är fortsatt en fullständig kontroll för den fallgruppen, det är bara riktning 1 som
är villkorad.

`_bearbeta_leverantorsfil()` (den funktion som faktiskt bygger en leverantörs `prisar`-lista,
och som VERIFIERAT används av BÅDE `bygg_ts()` och `bygg_ts_fran_katalog()`) läser
`ersatter_katalograd` från katalog-JSON:s policydeklaration precis som den redan läser
`kravda_falt`/`tackning` — inget separat kodpar krävs för de två anropsvägarna, eftersom
`ersatter_katalograd` är REN DATA på policyn, inte ett kontrollflöde.
...
**Rättat P1 (granskning `2026-09-08-009`) — namngiven produktentry och diskriminerad
resultattyp; utan detta är den aktuella årskostnaden inte NÅBAR för användaren.** v9:s
domänbeslut ovan (uppskattad aktuell årskostnad tillåten, besparingsvärdering blockerad) är
rätt, men beskrev bara motorlagrets undantag — inte hur `calcResult`/`KalkylatorPage` faktiskt
DISPATCHAR till den. `_kraver_kontrakt` gör bara att `beraknaBesparingsvarde` väljer
`beraknaBesparingsvardeKontrakt` i STÄLLET för legacyvägen; v9 kräver SAMTIDIGT att just den
funktionen kastar för Stockholm när besparing efterfrågas. `calcResult`/`KalkylatorPage` har i
dag bara resultatmodellen `Besparingsvarde` (`kostnadFore`/`kostnadEfter`/`besparingKr`) — ett
ensamt motoranrop djupt i `beraknaArskostnadMedKontrakt` gör alltså inget visningsbart för
användaren utan en egen produktväg ovanpå.

**Rättat P1 (granskning `2026-09-09-001`), två sammanhängande fel — TRE grenar i stället för
en `annars`-gren som gör besparing onåbar; och en delad indatabyggare i stället för att bara
lägga kapaciteten i ETT av de tre anropen.**

v10:s dispatch lade BÅDE `onskadTyp === 'aktuell_arskostnad'` OCH "en tariff UTAN
`kallenergiArsserieBindning` som efterfrågar besparing" i SAMMA `annars`-gren, som gjorde ETT
`beraknaArskostnadMedKontrakt`-anrop och ALLTID returnerade `{typ:'aktuell_arskostnad'}`.
Verifierat att detta gör v10:s egen `{typ:'besparing'}`-gren OKONSTRUERBAR av den beskrivna
dispatchen — ett besparingsanrop för Sandviken och samtliga 44 framtida kontraktstariffer
UTAN Stockholms kallenergiserie skulle alltså INTE längre göra före-/efterberäkningen, och
bryta det sekundära produktmålet OCH den redan godkända Sandviken-vägen. Samtidigt lade v10
kapacitetsinlägget bara EFTER den generiska kartan inne i `beraknaBesparingsvardeKontrakt` —
den nya aktuell-årskostnad-vägen fick därför ALDRIG en `IndataPost` för Stockholms
annual-skopade effektkrav och skulle blockeras.

**Rättat P1 (granskning `2026-09-09-002`) — v11:s tre-grensdesign var ouppnåelig: dess egen
gren 2 KONSTRUERADES ALDRIG av den verkliga sidan.** Verifierat genom att läsa
`calcResult`/`KalkylatorPage.tsx`: sidan känner inte till `beraknaArsprodukt` alls — den
BEFINTLIGA, godkända Sandviken-vägen går genom `calcResult`s tre `beraknaBesparingsvarde`-
anrop, som redan routar en kontraktsgated policy till `beraknaBesparingsvardeKontrakt` via
`kontraktsgatadPolicy()` (verifierat i `besparingsvarde.ts`), helt OBEROENDE av
`beraknaArsprodukt`. v11:s gren 2 (`onskadTyp==='besparing'` utan `kallenergiArsserieBindning`
→ `{typ:'besparing', besparingsvarde}`) var alltså en parallell, ALDRIG anropad omkonstruktion
av något Sandvikens väg redan gör korrekt — ett dött kodpar som skulle behöva underhållas i
takt med den riktiga vägen utan att någonsin köras av produktsidan. Samma sak gäller gren 1:
besparingsblockeringen för Stockholm behöver inte en tredje dispatchpunkt, eftersom
`beraknaBesparingsvardeKontrakt` (den funktion `calcResult` FAKTISKT anropar) redan är rätt
plats för det typade produktbegränsningsfelet (§6a.4, "Beslut"-stycket ovan) så snart
`policy.kallenergiArsserieBindning` är satt — ingen ny gren krävs för att uppnå den
blockeringen, bara guarden i den redan existerande funktionen.

**Löst genom att smalna `beraknaArsprodukt` till EN gren** (bara det som faktiskt saknar en
produktentry i dag: Stockholms "aktuell årskostnad") **och låta besparing gå OFÖRÄNDRAT via
`calcResult`:**

```ts
export type ArsprodukResultat =
  { typ: 'aktuell_arskostnad'; kostnad: number; leverantor: string; prisar: number;
    kapacitetKw: number; status: Resultatstatus }; // Stockholm-fallet, enda varianten
```
...
`beraknaArsprodukt` (nedan) och den OFÖRÄNDRADE `beraknaBesparingsvardeKontrakt` — den är den
enda delade byggnadslogiken, inte en tredje parallell dispatch.

Ny, namngiven funktion `beraknaArsprodukt(args: BesparingsvardeArgs): ArsprodukResultat` i
`besparingsvarde.ts` — den ENDA produktentrypunkten för Stockholms "aktuell årskostnad"; den
tar INGEN `onskadTyp`-parameter, eftersom den bara har en gren att dispatcha till. Ny
kapacitetsfunktion `stodjerAktuellArskostnad(prisar: GenereradPrisarspost): boolean` (sann
bara när `prisar.policy?.kallenergiArsserieBindning` är satt) — anropas av sidnivå-wrappern
(nedan) FÖRE `beraknaArsprodukt` över huvud taget anropas, så ett felaktigt anrop för en
tariff utan förmågan aldrig når produktentryn (fail-closed på anroparsidan, inte genom att
`beraknaArsprodukt` behöver gissa vilken gren som var avsedd). Logik:

1. Anropar `byggKontraktIndata` en gång.
2. Kör `valideraPolicyIndata` (§6a.2, punkt 6) — ett fältnära `KontraktBlockerat` för
   kundindatafel, precis som den befintliga kontraktsgated vägen redan gör.
3. ETT anrop till `beraknaArskostnadMedKontrakt`, returnerar `{ typ: 'aktuell_arskostnad',
   kostnad: resultat.kostnad.summaInkl, leverantor, prisar: prisar.ar, kapacitetKw, status:
   resultat.status }` — `status.noggrannhet` bär `'snapshot'`-proveniensen (§6a.2s
   `harled_resultatstatus`-regler, ingen ny logik).
...
FÖRSÖKA visa besparing. `calcResult` självt ändras därför INTE — dess befintliga,
väldokumenterade kontrakt (fullständig poäng-/besparings-/rekommendationsberäkning) rörs
inte av detta tillägg. I stället en NY, tunn wrapper-funktion i `energiPotential.ts`:

**Rättat P1 (granskning `2026-09-09-002`) — `onskadTyp` hade TVÅ källor, och
`argsFranInputs` fanns bara i kodexemplet, aldrig definierad i texten.** v11:s
...
vilken som gällde). Dessutom anropade kodexemplet `argsFranInputs(inputs)` utan att den
funktionen NÅGONSIN definierades i dokumentet — ett verifierat hål, inte en detalj som kan
antas "redan finnas": `calcResult`s befintliga argumentbyggnad (leverantörs-/prisårsval,
kapacitet, m.fl. fält ur `KalkylatorInputs`) ligger i dag inline i `calcResult` självt.

**Löst genom att göra `KalkylatorInputs.onskadTyp` till den ENDA källan och extrahera
`argsFranInputs` som en genuin funktion ur `calcResult`s befintliga, oförändrade logik** (inte
ny logik — samma härledningar `calcResult` redan gör för leverantörs-/prisårsval och
kapacitet, bara namngivna och återanvändningsbara):

```ts
export function argsFranInputs(inputs: KalkylatorInputs): BesparingsvardeArgs {
  // Extraherad, oförändrad ur calcResults befintliga inline-uppbyggnad av
  // leverantör/prisår/kapacitet — ingen ny härledningslogik.
  ...
}
...
export function calcResultForOnskadTyp(inputs: KalkylatorInputs): KalkylatorResultUnion {
  if (inputs.onskadTyp === 'aktuell_arskostnad') {
    ...
    if (!stodjerAktuellArskostnad(prisar)) {
      throw new Error('Denna tariff stödjer inte aktuell årskostnad'); // fail-closed
    }
    const resultat = beraknaArsprodukt(argsFranInputs(inputs));
    return { typ: 'aktuell_arskostnad', ...resultat };
  }
  return { typ: 'fullstandig', resultat: calcResult(inputs) }; // OFÖRÄNDRAD väg
}
```

Ett nytt, deterministiskt fält `onskadTyp: 'besparing' | 'aktuell_arskostnad'` läggs till i
`KalkylatorInputs` — DEN ENDA KÄLLAN till detta beslut, aldrig en separat funktionsparameter.
Källan i UI:t är ett explicit kundval i formuläret (en radioknapp/växel bredvid
leverantörsvalet, synlig bara när den valda tariffen faktiskt stödjer `aktuell_arskostnad`
via `stodjerAktuellArskostnad`, annars låst till `'besparing'`).
...
BEFINTLIGA, oförändrade `calcResult`-vägen kastar produktbegränsningsfelet via
...
bevisligen oberörd av detta tillägg. Ett FJÄRDE test verifierar att
`stodjerAktuellArskostnad(prisar)` är `false` för Sandviken (ingen `kallenergiArsserieBindning`)
och `true` för Stockholm, och att `calcResultForOnskadTyp` kastar fail-closed INNAN
`beraknaArsprodukt` ens anropas om `onskadTyp:'aktuell_arskostnad'` begärs för en tariff där
förmågan är `false`.
...
fillista (`batchplan-v12.md`), där Jönköpings basrad redan ligger.
