# Batchplan v13.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v12.md` i sin helhet (v12 ändras INTE i
...
(supersedes `2026-09-09-002`). Bygger på dispositionerna i
[`tariffinventering-v13.md`](tariffinventering-v13.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v13** (se granskning `2026-09-09-003` för fullständig motivering):
...
`tariffinventering-v13.md` §6a.2:s 42-radstabell) OCH före batch 7 (Stockholms seriekrav
...
- **Innehåll:**
  ...
  7. **Typkorrekt parserresultat, EN delad orsaksunion (rättat P1, granskning
     ...
     initierade fält alltid klassas `saknat` identiskt.
  8. **Felklassning — saknad OCH ogiltig indata, nåbar även för ett direkt produktanrop
     (rättat P1, granskning `2026-09-08-009`/`2026-09-09-002`/`2026-09-09-003`):** ny
     `forkontrolleraPolicyIndata(policy, prisar, indata): { saknade: readonly string[];
     ogiltiga: readonly PolicyValideringsFel[] }` (`PolicyValideringsFel = { nyckel: string;
     orsak: 'typ' | 'numerik' | 'kardinalitet' | 'okant_val' }`) beräknar BÅDA listorna i EN
     iteration över `policy.kravdaFalt` mot den mottagna `indata`-kartan — `saknade` för
     fält som INTE finns i `indata`, `ogiltiga` (inkl. `minVarde`/`maxVarde`/`heltal`,
     bandmedlemskap via `f.vardetyp === 'band_id'`) för fält som finns men bryter en regel.
     **Rättat P1, granskning `2026-09-09-003`:** v12 påstod att den BEFINTLIGA
     `blocked`-vägen redan härledde `saknadeFalt` ur `harledResultatstatus` — verifierat
     FALSKT (`Resultatstatus` har inget fältnamnlistfält, `harledResultatstatus` kastar
     bort sin lokala `saknade`-lista och returnerar bara `blocked`), så `saknadeFalt` var i
     praktiken onåbart för ett anrop som byggde sin `indata`-karta direkt utan att passera
     `KalkylatorPage.tsx`. `forkontrolleraPolicyIndata` har INGET sådant beroende — den
     körs FÖRE fasadanropet i BÅDA de publika produktvägarna
     (`beraknaBesparingsvardeKontrakt`/`beraknaArsprodukt`) och fyller
     `KontraktBlockerat.saknadeFalt`/`ogiltigaFalt` direkt från sitt eget resultat. Båda
     felfälten visas fältnära i `KalkylatorPage.tsx` (en separat UI-nivå-förkontroll vid
     submit, som en UX-genväg — inte den enda spärren); interna kontrakts-/
  ...
  9. **Namngiven produktentry för Stockholms aktuella årskostnad, EN gren, EN
     ...
     läser `kontraktsgatadPolicy(prisar)?.kallenergiArsserieBindning !== undefined` — den
     enda tillåtna rådatagrinden, ALDRIG `prisar.policy?.X` direkt — och är därmed `false`
     för icke-fjärrvärmesystem, legacytariffer OCH Sandviken (kontraktsgatad men UTAN
     ...
     litar aldrig på att anroparen redan kontrollerat detta). `argsFranInputs(inputs):
     Tariffberakningsunderlag` extraheras ur `calcResult`s befintliga scope-/
     proveniensuppbyggnad som en FAKTISK funktion (**rättat P1, granskning
     `2026-09-09-003`:** v12:s kodexempel hade fortfarande en tom `...`-kropp), och
     `beraknaArsprodukt` tar denna smalare bastyp i stället för `BesparingsvardeArgs`
     (som kräver `paverkbarMwh`/`besparingsgrad` — fält en aktuell-årskostnadsberäkning
     inte har) — `calcResult` och `BesparingsvardeArgs` själva förblir OFÖRÄNDRADE. ETT
     nytt fält `onskadTyp: 'besparing' | 'aktuell_arskostnad'` på `KalkylatorInputs`
     (ENDA källan — v11 hade den även som en separat wrapper-parameter).
     `calcResultForOnskadTyp` i `energiPotential.ts` kastar fail-closed INNAN
     `beraknaArsprodukt` anropas om `stodjerAktuellArskostnad` är `false`. Besparingsvägen
     är den BEFINTLIGA, oförändrade `calcResult`-vägen (dess tre
     `beraknaBesparingsvarde`-anrop) — inte en gren i `beraknaArsprodukt`;
     `KalkylatorPage.tsx` renderar `{ typ: 'aktuell_arskostnad' }` utan besparingsfält
     (§6a.4) eller `{ typ: 'fullstandig' }` (den oförändrade resultatsidan).
...
- **Namngiven produktentry, EN gren, EN produktförmågetabell (rättat P1, granskning
  ...
  `stodjerAktuellArskostnad(prisar): boolean` läser `kontraktsgatadPolicy(prisar)?.
  kallenergiArsserieBindning !== undefined` — **rättat P1, granskning `2026-09-09-003`:**
  INTE "sann för varje kontraktsgated tariff" (v12:s påstående på detta ställe, som
  motsade Batch 0 punkt 9:s tabell där Sandviken är `false`) — och kontrolleras BÅDE av
  `calcResultForOnskadTyp` (UX-förkontroll) OCH INUTI `beraknaArsprodukt` självt (den
  auktoritativa spärren, se Batch 0 punkt 9). `calcResultForOnskadTyp(inputs:
