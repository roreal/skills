# Tariffinventering v10.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-08 av Claude. Ersätter `tariffinventering-v9.md` i sin helhet (v9 ändras
INTE i efterhand — kvar som historik), som svar på Codex omgranskning
[2026-09-08-009](../conversations/reviews/2026/09/2026-09-08-omgranskning-tariffinventering-v9.md)
(`status: changes-required`, supersedes `2026-09-08-008`) av v9. Ingen produktkod, tariffdata
...
`KalkylatorPage.tsx`s faktiska `parseFloat`-konvertering till `Record<string, number>` FÖRE
`calcResult`, `Varde`s dubbla användning (både `IndataPost.varde`s typ OCH
`_validera_varde`/`valideraVarde`s parametertyp), `Tariffpolicy.kravdaFalt`s avsaknad av
`etikett`/`hjalptext`, och `Tariffpolicy.kapacitetBindning`s befintliga, separata
`kapacitetKw`-fält i `KalkylatorPage` — inte bara omskrivna från granskningen.

**Vad som är nytt i v10, i korthet** (se granskning `2026-09-08-009` för fullständig
motivering till varje punkt):

1. **En egen, typad produkt-DTO ersätter påståendet att det numeriska `falt` räcker.** v9
   sa att formulärets `Record<string, string>` kunde föras OFÖRÄNDRAD genom
   `KalkylatorInputs.falt`/`BesparingsvardeArgs.falt` — verifierat att detta motsäger
   `KalkylatorPage.tsx`s faktiska `parseFloat`-konvertering och att den befintliga kedjan är
   numerisk-only och FORTSÄTTER vara det (Gotlands `foregaende_ars_mwh` m.fl., legacyvägen).
   §6a.2 lägger till ett NYTT, PARALLELLT `policyFalt: Record<string, PolicyInputValue>`
   genom hela kedjan, och en NY, egen `IndataVarde = Varde | str`-typ på `IndataPost.varde` —
   den delade `Varde`-aliasen (som ÄVEN är `_validera_varde`/`valideraVarde`s parametertyp)
   rörs INTE, till skillnad från v9:s påstådda-men-faktiska vidgning av just den aliasen.
2. **UI-metadatan blir prispost- och omfattningsmedveten.** v9:s `policyFaltMetadata(policy)`
   tog bara policyn, men bandalternativen finns i den VALDA prispostens `nivaer[].id` — en
   funktion utan `prisar` kan inte bygga sitt eget bandval. §6a.2 lägger till `prisar`/
   `omfattning`-parametrar, filtrerar bort `policy.kapacitetBindning` (som redan har ett eget
   UI-fält) och renderar bara krav för den efterfrågade omfattningen (Stockholms `monthly`-
   och `annual`-krav blandades annars ihop). `KravPost` får obligatoriska `etikett`/
   `hjalptext`-fält satta av `policyregister.py`, i stället för en outtalad
   runtime-härledning. Bevarade band-ID:n valideras unika/icke-tomma vid katalogbyggnad.
3. **Både saknad och ogiltig kundindata blir typade användarfel.** v9 gav bara
   `KontraktBlockerat.saknadeFalt` och lät fel typ/serielängd/enumval förbli ett generiskt
   kastat kontraktsfel, med motiveringen att ett genererat UI aldrig kan producera sådana
   värden — antagandet håller inte (delvis ifylld serie, gammalt sparat bandval efter en
   prisuppdatering, ett direkt API-anrop som kringgår formuläret). §6a.2 lägger till
   `KontraktBlockerat.ogiltigaFalt` vid sidan av `saknadeFalt`, båda visade fältnära i
   `KalkylatorPage.tsx`; interna kontrakts-/konfigurationsfel reserveras för fall utan ett
   specifikt användarfält att peka på.
4. **Stockholm får en namngiven produktentry och en diskriminerad resultattyp.** v9:s
   domänbeslut (uppskattad aktuell årskostnad tillåten, besparing blockerad) var rätt, men
   den faktiska produktdispatchen saknades — `calcResult`/`KalkylatorPage` har bara
   `Besparingsvarde`-modellen, och ett ensamt motoranrop gör inget visningsbart. §6a.4 lägger
   till `ArsprodukResultat` (diskriminerad union) och `beraknaArsprodukt(args, onskadTyp)`,
   med dispatch i `calcResult` och en egen resultatsektion i `KalkylatorPage.tsx` för
   `aktuell_arskostnad`, helt utan besparingsfält.
5. **Adapterpreflighten blir en injicerbar, testbar tvåvägsfunktion utan `pass`.** v9:s
   pseudokod läste fortfarande de globala `POLICYREGISTER`/`ADAPTERREGISTER`, och den omvända
   kontrollen innehöll bokstavligen `pass  # se produktionskoden` bakom en självmotsägande
   "OR"-regel som skulle förbjuda en NY, korrekt adaptermappad Stockholm-policy. §6a.4 lägger
   till ett explicit `Tariffpolicy.ersatter_katalograd`-fält som gör den omvända bijektionen
   exakt, utan historiska "redan godkänd"-undantag, och en injicerbar
   `kontrollera_adapterpreflight(rak_katalog, byggda_leverantorer, policyregister,
   adapterregister)` som `bygg_ts_fran_katalog()`/`bygg_ts()` båda anropar (delar
   `_bearbeta_leverantorsfil()`).
6. **P2 rättat:** terminologikrocken kring `Varde`/global vidgning (§6a.2) löst genom att
   INTE röra `Varde` alls (punkt 1 ovan). Commitproveniensen rättad: v9 levererades i
   `2cfa3be`, direkt ovanpå `b7790ca` (INTE "`fd372a2`/`b7790ca`" som en tidigare rundas
   leveranstext skrev) — verklig committid enligt `git log --format=%aI 2cfa3be`.
7. **Oförändrat från v9 (redan korrekt och återverifierat):** Umeås tvåpassgrind och
   ...
   dispositionerna 7/55/30/92 — inga nya produktbeslut från Robert krävs för dessa tekniska
   rättningar, per granskningens egen instruktion (punkt 7).
...
`batchplan-v10.md`), inte något Sandvikens nuvarande kapacitets-only-adapter redan täcker:
...
1. **Rättat P1 (granskning `2026-09-08-009`) — separat, typad produkt-DTO i stället för att
   ...
   En NY, separat typ `PolicyInputValue = number | string | readonly number[]`
   (`resultatkontrakt.ts`) och ett NYTT, valfritt argument tillagt genom HELA kedjan, PARALLELLT
   med (inte i stället för) det befintliga `falt`:
   - `KalkylatorInputs.policyFalt?: Record<string, PolicyInputValue>` (`energiPotential.ts`)
   - `BesparingsvardeArgs.policyFalt?: Record<string, PolicyInputValue>` (`besparingsvarde.ts`)
   - `KalkylatorPage.tsx` får ett NYTT React-state `policyFaltVarden: Record<string,
     PolicyInputValue>` (skilt från det befintliga `faltVarden: Record<string, string>`),
     ifyllt via den genererade UI-metadatan (punkt 5 nedan) — ett numeriskt policyfält
     konverteras med `Number(...)` vid inskick (inte `parseFloat` på en formulärsträng som
     redan blandar ihop legacy- och policyfält), ett `band_id`/`enum`-fält lagras och skickas
     som en OFÖRÄNDRAD sträng, ett `number_series`-fält samlas som en `number[]` från N
     separata talfält (ett per seriemånad) och skickas OSPRIDD (aldrig som en
     kommaseparerad sträng eller ett enda summerat tal).
2. Ny hjälpfunktion i `besparingsvarde.ts`, `byggIndataFranPolicy(policy: Tariffpolicy,
   ...
   `PolicyInputValue`-formade värdet — ingen typkonvertering sker i denna funktion, den
   ANTAR att anroparen (punkt 1, UI-lagret) redan levererat rätt form per fältets
   `vardetyp`; `harled_resultatstatus`/`harledResultatstatus` gör den faktiska
   typ-/kardinalitetsvalideringen (§6a.2, "Diskriminerad värdetyp"-avsnittet ovan). Ett
...
5. **Genererad UI-metadata — prispost- och omfattningsmedveten (rättat P1, granskning
   ...
   etikett, hjalptext, obligatorisk: true, tillatnaVarden?, seriekardinalitet? }`. `etikett`/
   `hjalptext` har en MASKINLÄSBAR källa som redan finns i katalogen och genereras genom
   samma väg som `Indatafalt.etikett`/`.hjalptext` (`indatafalt_for()`, `generera.py`) —
   `KravPost` (`resultatkontrakt.py`/`.ts`) får två nya, valfria fält `etikett: str | None =
   None`/`hjalptext: str | None = None`, satta av `policyregister.py` när en policy-post
   deklareras (samma ansvar som redan sätter `kalla`), i stället för att härledas från
   `nyckel`/`enhet` vid runtime — ett `KravPost` utan uttrycklig `etikett` kastar i
   ...
   **rättat P1, granskning `2026-09-08-009`:** varje bevarat band-ID valideras
   (`skapaTariffpolicy`/motsvarande katalogbyggnadssteg) vara en ICKE-TOM sträng och UNIK
   inom prispostens `nivaer`-lista INNAN motorn (`vald_niva_id`s `next(...)`-uppslagning) eller
   UI:t får använda dem — ett dubblerat eller tomt `id` kastar redan vid katalogbyggnad/
   generering, inte tyst vid runtime-uppslagning (annars skulle två prisrader med samma ID
   göra leverantörens bekräftade val tvetydigt, eftersom `next(...)` bara returnerar den
   FÖRSTA träffen). Enum-alternativ (`inmatningstyp: 'enum_val'`) hämtas från
   `KravPost.tillatnaVarden` (§6a.6). Varken enum- eller bandvärden får någonsin passera
   `parseFloat`/`Number(...)` — de renderas och lagras som strängar/diskreta val genom hela
   `KalkylatorPage.tsx` → `policyFalt`-kedjan (punkt 1).

   **Batch 0:s end-to-end-test (rättat P1, granskning `2026-09-08-009`):** renderar den
   VERKLIGA `KalkylatorPage`-komponenten (React Testing Library eller motsvarande, inte en
   fristående testkomponent som kan drifta bort från produktsidan) med en vald syntetisk
   prispost som bär alla fyra `vardetyp`, väljer ett band-ID, fyller en 12-elements serie och
   ett Jönköping-liknande enum, och verifierar att det beräknade resultatet matchar en
   handräknad referens — inte bara att `byggIndataFranPolicy` kan anropas isolerat.
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel (rättat P1,
   granskning `2026-09-08-009`, ersätter v9:s `saknadeFalt`-only-lösning).** v9 antog att ett
   genererat UI (punkt 5) aldrig KAN producera ett strukturellt ogiltigt värde och lät därför
   fel `vardetyp`, fel serielängd och okänt enum-/band-ID förbli ett kastat, generiskt
   kontraktsfel. Antagandet håller INTE: en användare kan lämna en 12-månadersserie delvis
   ifylld, skriva ett numeriskt värde som inte är ändligt, eller skicka ett gammalt sparat
   bandval efter en prisuppdatering (bandets `nivaer[].id` kan ändras mellan prisår) —
   dessutom är `calcResult`/produktadaptern en anropsyta även för framtida UI, API- och
   Ellen-integrationer, så otillförlitlig anropsindata är inte automatiskt begränsad bara för
   att DAGENS formulär försöker begränsa den.

   `KontraktBlockerat` får därför EN typad användarfelsgren som täcker BÅDA fallen, inte bara
   det saknade: nytt fält `ogiltigaFalt?: { nyckel: string; orsak: 'typ' | 'numerik' |
   'kardinalitet' | 'okant_val' }[]` vid sidan av det befintliga `saknadeFalt?: string[]`.
   `beraknaBesparingsvardeKontrakt` sätter ETTDERA (aldrig kastar en generisk `Error`) när
   `status.fullstandighet === 'blocked'` och orsaken kan härledas till ett SPECIFIKT
   policydeklarerat fält: `saknadeFalt` när nyckeln helt saknas i den byggda `policyFalt`-
   kartan, `ogiltigaFalt` när nyckeln finns men `harled_resultatstatus` avvisade värdets typ,
   numerik, serielängd (`antal_varden`) eller enum-/band-ID. Interna kontrakts-/
   konfigurationsfel (fortsatt en kastad `Error`, aldrig `KontraktBlockerat`) reserveras för
   fall UTAN ett specifikt användarinmatat fält att peka på: en saknad bindningsmålpost i
   policyn själv, fel bindningstyp (t.ex. `kapacitet_band_bindning` som pekar på ett
   `"number"`-krav i stället för `"band_id"`), en omöjlig policykombination, eller trasig
   genererad metadata (punkt 5). `KalkylatorPage.tsx` visar `saknadeFalt`/`ogiltigaFalt` som
   fältnära formulärfel (samma mönster som `kapacitetsfelText` redan använder för det
   befintliga kapacitetsfältet), inte det generiska felmeddelandet. Testat via BÅDA UI:t
   (punkt 5:s end-to-end-test) OCH ett direkt produktanrop som kringgår formuläret helt.
...
def kontrollera_adapterpreflight(
    ...
    bygg_ts_fran_katalog() OCH bygg_ts() (båda vägar delar
    _bearbeta_leverantorsfil(), se nedan), med SAMMA register hela vägen.
    Kastar ValueError på första fel; ändrar ingen katalog- eller
    leverantörsdata."""
    # Riktning 1: varje ADAPTERREGISTER-post pekar på en verklig, sammanhängande kedja.
    ...
    for leverantor in byggda_leverantorer.values():
        for prisarspost in leverantor["prisar"]:
            policy = prisarspost.get("policy") or {}
            ersatt_id = policy.get("ersatter_katalograd")
            if ersatt_id is not None and prisarspost.get("tariff_id") not in adapterade_mal:
                raise ValueError(
                    f"{prisarspost.get('tariff_id')}: policy har ersatter_katalograd="
                    f"{ersatt_id!r} men ingen ADAPTERREGISTER-post pekar på denna tariff"
                )
            # En policy UTAN ersatter_katalograd gör inget dedupliceringsanspråk —
            # en framtida direkt leverantörsprodukt med annual_forward men utan
            # katalogmotsvarighet är alltså uttryckligen TILLÅTEN, utan
            # historiska "redan godkänd"-undantag.
...
`ersatter_katalograd` är REN DATA på policyn, inte ett kontrollflöde. `bygg_ts()` (anropas
utan en katalog, bara leverantörsfiler) kör samma `kontrollera_adapterpreflight` med en TOM
`rak_katalog["tariffs"]`-lista och `adapterregister={}` om inga adapterposter är relevanta i
det anropet — riktning 2 (bijektionen) fungerar oförändrat eftersom den bara läser
leverantörernas egna policyer, inte katalogen.
...
anspråk på att ersätta en katalograd utan en matchande adapterpost (riktning 2) — aldrig en
tyst utebliven rad. `godkanda()`/katalogloopen rörs INTE av `ADAPTERREGISTER`-kontrollen i
sig — den körs som en separat preflight FÖRE `godkanda()` anropas, med samma injicerade
`policyregister` som §6a.3 redan kräver.
...
Ny, diskriminerad TypeScript-resultattyp i `besparingsvarde.ts` (mirror i Python för
framtida konsumenter, samma synk-princip som resten av kontraktet):

```ts
export type ArsprodukResultat =
  | { typ: 'besparing'; kostnadFore: number; kostnadEfter: number; besparingKr: number; status: Resultatstatus }
  | { typ: 'aktuell_arskostnad'; kostnad: number; status: Resultatstatus }; // Stockholm-fallet
```

Ny, namngiven funktion `beraknaArsprodukt(args: BesparingsvardeArgs & { onskadTyp:
'besparing' | 'aktuell_arskostnad' }): ArsprodukResultat` i `besparingsvarde.ts` — den ENDA
produktentrypunkten `calcResult` (`energiPotential.ts`) anropar för en kontraktsgated tariff.
Dispatch-logik:

1. Slår upp `kontraktsgatadPolicy(prisar)` (redan befintlig funktion) och bygger `IndataPost`
   via `byggIndataFranPolicy` (§6a.2, punkt 2).
2. Om `policy.kallenergiArsserieBindning` är satt (Stockholms fall, den enda tariffen i denna
   kontrollmängd med bindningen) OCH `args.onskadTyp === 'besparing'`: kastar det nya typade
   produktbegränsningsfelet (INTE `KontraktBlockerat` — det här är ingen kunddatalucka, det
   är att den efterfrågade PRODUKTFÖRMÅGAN saknar en källverifierad regel). `calcResult`
   fångar felet och returnerar en `KalkylatorResult` där besparingsfälten är `null`/utelämnade
   (aldrig ett påhittat `0`) med en produktorsakstext, INTE `KontraktBlockerat`s
   fältnära formulärfel (§6a.2, punkt 6) — de två felvägarna är avsiktligt skilda: en
   kunds ogiltiga fältvärde ska visas fältnära, en hel PRODUKTBEGRÄNSNING (denna tariff kan
   aldrig ge besparing) ska visas som en sidnivå-notis.
3. Annars (`onskadTyp === 'aktuell_arskostnad'`, eller en tariff utan
   `kallenergiArsserieBindning` som efterfrågar besparing): ETT anrop till
   `beraknaArskostnadMedKontrakt` med de byggda `IndataPost`-värdena, returnerar
   `{ typ: 'aktuell_arskostnad', kostnad: resultat.kostnad.summaInkl, status:
   resultat.status }` — `status.noggrannhet` bär `'snapshot'`-proveniensen (§6a.2s
   `harled_resultatstatus`-regler, ingen ny logik).

`KalkylatorPage.tsx`: när den valda tariffens `beraknaArsprodukt`-anrop returnerar `{ typ:
'aktuell_arskostnad' }`, renderas en EGEN resultatsektion (kostnad + "uppskattning" per
verifieringsspråket i `PROJECT_CHARTER.md` §3) UTAN besparingsfält — ingen tom eller
påhittad `besparingKr` visas någonsin. Kr- och schablonlägena förblir blockerade som redan
beskrivet (§2, generell regel: ett läge utan verifierad modell blockeras).

**Sidtest (nytt, granskning `2026-09-08-009`):** väljer Stockholm i den RIKTIGA
`KalkylatorPage`, fyller de 12/5-serierna via det genererade UI:t (§6a.2, punkt 5), begär
`onskadTyp: 'aktuell_arskostnad'`, och verifierar att sidan visar kostnaden UTAN någon
besparingssiffra synlig. Ett separat test begär `onskadTyp: 'besparing'` för samma tariff
och verifierar produktbegränsningsfelet (sidnivå-notis, inte ett fältnära formulärfel).
...
**Test (utöver v7:s lista, kompletterat granskning `2026-09-08-008`/`-009`):** saknad/stale
...
prisårspost) kastar; en policy med `ersatter_katalograd` satt men UTAN matchande
`ADAPTERREGISTER`-post kastar (riktning 2/bijektionen, ersätter den tidigare `pass`-skissen);
`kontrollera_adapterpreflight` anropad med två OLIKA injicerade register ger två olika
resultat (bevisar att registren faktiskt förs igenom, inte läses globalt av misstag); en
...
fillista (`batchplan-v10.md`), där Jönköpings basrad redan ligger.
