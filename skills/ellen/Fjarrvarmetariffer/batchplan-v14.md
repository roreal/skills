# Batchplan v14.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v13.md` i sin helhet (v13 ändras INTE i
...
(supersedes `2026-09-09-003`). Bygger på dispositionerna i
[`tariffinventering-v14.md`](tariffinventering-v14.md) §4.1 (45 bastariffer) och §5 (10
...
**Vad som är nytt i v14** (se granskning `2026-09-09-004` för fullständig motivering; samma
fyra P1-fynd som `tariffinventering-v14.md`, applicerade på batchplanens motsvarande
avsnitt):
...
`tariffinventering-v14.md` §6a.2:s 42-radstabell) OCH före batch 7 (Stockholms seriekrav
...
- **Innehåll:**
  ...
  8. **Felklassning — saknad OCH ogiltig indata, nåbar även för ett direkt produktanrop,
     ...
     använder, rad ~373–374) mot den mottagna `indata`-kartan — `saknade` för fält som INTE
  ...
  9. **Namngiven produktentry för Stockholms aktuella årskostnad, EN gren, EN
     ...
     `energyInputMode`, se tariffinventering-v14.md §6a.4 för den fullständiga, rättade
     definitionen), och `beraknaArsprodukt` tar denna smalare bastyp i stället för
     `BesparingsvardeArgs` (som kräver `paverkbarMwh`/`besparingsgrad` — fält en
     aktuell-årskostnadsberäkning inte har) — `calcResult` och `BesparingsvardeArgs` själva
     förblir OFÖRÄNDRADE. ETT nytt, VALFRITT fält `onskadTyp?: 'besparing' |
     ...
     `energySystem` inte är `fjarrvarme`, se ovan). Besparingsvägen är den BEFINTLIGA,
