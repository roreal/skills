...
- [ ] **Östersund, Frösön, Ås** (`jamtkraft-ostersund-froson-as-2026`)
  ...
  - **🟡 Godkänt för årsberäkning, implementerat bakom spärr (Batch 4, handoff 2026-09-14-001).** Alla katalogpriser, effektintervall, säsonger och flödesformeln stämmer. Debiteringseffekten är medelvärdet av de tre högsta dygnsmedeleffekterna under de senaste 36 månaderna inklusive fakturamånaden; ett fastställt värde gäller tills ett högre värde mäts, dock längst 12 månader. Kalkylatorn tar leverantörens redan beräknade värde och kräver en bekräftad fakturamånad — ingen egen 36-månadersserie.
  - [x] Metod för debiterbar effekt/kapacitet verifierad; `billing_basis_method` ifylld och kontraktsgated (Batch 4, ännu bakom lokal implementationsspärr, ej aktiverad).
  - [ ] Månadsperiodisering saknas eller behöver verifieras; årsbelopp får inte automatiskt delas med 12.
...
- [ ] **Umeå Enkel** (`umea-energi-umea-enkel-2026`)
  ...
  - [x] Kapacitetsmultiplikatorn B är strukturellt bunden och intervallvaliderad ([0,93, 1,401]) genom katalog → policy → kontraktsfasad → motor (Batch 4, ännu bakom lokal implementationsspärr, ej aktiverad). Andra avtalsalternativ ingår inte.
