# Tariffinventering v16.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v15.md` i sin helhet (v15 ändras
INTE i efterhand — kvar som historik), som svar på TVÅ Codex-dokument samtidigt: omgranskning
[2026-09-09-007](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v15.md)
(`status: changes-required`, supersedes `2026-09-09-005`) av v15/`batchplan-v15.md`, och
bedömning
[2026-09-09-006](../conversations/reviews/2026/09/2026-09-09-bedomning-lidkoping-energi-leverantorssvar.md)
av Lidköping Energis leverantörssvar (`status: source-approved-with-runtime-input`). Ingen
produktkod, tariffdata eller genererad fil är ändrad av detta dokument. Ingen tariff
aktiveras. Dispositionerna ändras enligt bedömning 006: **7 implementerade / 57 redo / 28
blockerade / 92 totalt** (från 7/55/30/92) — basfördelningen blir 7/47/24, variantfördelningen
oförändrad 0/10/4; se §4.2 och §8. Alla kodpåståenden nedan är verifierade genom att läsa den
verkliga koden i
...
**Vad som är nytt i v16, i korthet** (se granskning `2026-09-09-007` och bedömning
...
4. **Lidköping Energis två bastariffer flyttade från `blocked_external_info` till
   ...
   Ny batch 5d i `batchplan-v16.md` och en ny deklarativ, signerad
   nätmedelavkylningsjustering — se §6a.6.
...
- **Källstatus (rättat i v16, bedömning `2026-09-09-006`):** externt källunderlag GODKÄNT
  ...
  ger metoden, inte de faktiska talen) — se §6a.6.
- **Katalogstatus:** `production_ready: false`, `investigation.status: utreds` (kvarstår
  tills adjustments-posten §6a.6 beskriver är byggd — planeras, inte genomförd av detta
  dokument)
- **Motorstatus:** N/A tills batch 5d (§6a.6, `batchplan-v16.md`) är implementerad
- **Kontraktsstatus:** ej i `POLICYREGISTER`
- **Teststatus:** inga
- **UI-status:** inte valbar
- **Årsreproducerbar med nuvarande underlag:** Ja, källmässigt (metoden är komplett
  ...
  eller direkt leverantörsbesked) — se §6a.6:s runtime-krav.
...
- **Årsreproducerbar med nuvarande underlag:** Ja, källmässigt; Nej per konkret kundfall
  förrän `Tm`s tolv månadsvärden finns — se §6a.6.
...
leverantörssvar 2026-09-09 (bedömning `2026-09-09-006`, §6a.6) löser nu den externa
...
nya nätmedelavkylningsjustering, §6a.6) saknar sin planerade indatakanal.
...
**Ny, deklarativ justeringstyp — mirrorad Python/TypeScript, återanvänder batch 0:s
`number_series`-/policyfältmekanism (§6a.1/§6a.2) i stället för att uppfinna en ny
serieform:**

```ts
// resultatkontrakt.ts, planerat tillägg — samma diskriminatormönster som övriga
// AdjustmentSpec-varianter, ingen ny generisk mekanism.
export interface SignedMonthlyFlowAdjustment {
  typ: 'signed_monthly_flow_adjustment';
  ...
  nat_avkylning_falt: string;   // policyFalt-nyckel för Tm_m (number_series, 12 värden, °C,
                                 // supplier_value — INTE kundens egen indata)
}

// besparingsvarde.ts / resultatkontrakt.ts, planerad beräkningsfunktion — anropas EFTER
// forkontrolleraPolicyIndata har bevisat att alla tre serierna finns med rätt kardinalitet.
export function berakSignedMonthlyFlowAdjustment(
  spec: SignedMonthlyFlowAdjustment,
  indata: ReadonlyMap<string, IndataPost>
): number {
  const q = hämtaSerie(indata, spec.kund_volym_falt);       // 12 värden, ändliga, ≥ 0
  const t = hämtaSerie(indata, spec.kund_avkylning_falt);   // 12 värden, ändliga
  const tm = hämtaSerie(indata, spec.nat_avkylning_falt);   // 12 värden, ändliga, > 0
  let arsjustering = 0;
  for (let m = 0; m < 12; m++) {
    arsjustering += spec.faktor_n * q[m] * (1 - t[m] / tm[m]);
  }
  return arsjustering; // INTE Math.max(0, ...) — tvåsidigt resultat
}
```

```python
# resultatkontrakt.py, planerat tillägg — mirror av TS-varianten ovan
@dataclass(frozen=True)
class SignedMonthlyFlowAdjustment:
    typ: Literal["signed_monthly_flow_adjustment"]
    faktor_n: float
    kund_volym_falt: str
    kund_avkylning_falt: str
    nat_avkylning_falt: str


def berak_signed_monthly_flow_adjustment(
    spec: SignedMonthlyFlowAdjustment,
    indata: Mapping[str, IndataPost],
) -> float:
    q = hamta_serie(indata, spec.kund_volym_falt)
    t = hamta_serie(indata, spec.kund_avkylning_falt)
    tm = hamta_serie(indata, spec.nat_avkylning_falt)
    return sum(spec.faktor_n * q[m] * (1 - t[m] / tm[m]) for m in range(12))
    # INTE max(0, ...) — resultatet är tvåsidigt (avgift eller kreditering)
```

**Fail-closed-validering, återanvänder batch 0:s befintliga seriekontrakt i stället för en
ny mekanism:**
...
- `Tm_m` (nät-avkylningsserien) markeras med samma `kallaTyp: 'supplier_value'`-diskriminator
  redan använd för kapacitetsbindningar (§6a.4) — det är ett leverantörsfastställt värde,
  inte kundens egen fakturaindata, och ska INTE kunna skickas in fritt av kunden.
...
- Om en serie har fel längd, ett icke-ändligt element, eller (för `Tm_m` specifikt) något
  ...
  icke-ändligt, `'min'` för `Tm_m ≤ 0` via ett `minVarde: 0`-krav som INTE är inkluderande —
  se nedan) → `KontraktBlockerat(tariffId, 'invalid_policy_fields', { ogiltigaFalt })`.
  Ingen serie får ersättas med noll, ett upprepat värde eller ett gissat medeltal.
- `Q_m` kräver `minVarde: 0` (inkluderande — noll förbrukning en enskild månad är giltigt).
  ...
  `minVarde` STRIKT `> 0` — `KravPost.minVarde` är i dag inkluderande (§6a.1), så
  `Tm_m = 0` måste antingen fångas av en dedikerad `>0`-variant av gränskontrollen eller
  av en explicit kontroll i `berakSignedMonthlyFlowAdjustment` innan divisionen (motorn
  får ALDRIG dividera med ett odetekterat 0) — implementationsbeslutet tas i batch 5d, inte
  av detta dokument.
...
därför ett RUNTIMEKRAV som matas in per kundfall (från faktura eller direkt
leverantörsbesked), inte ett statiskt katalogvärde i `adjustments`. Saknas det för ett givet
kundfall ska HELA Lidköpings fullständiga tariff blockeras för det fallet — inte räknas med
ett gissat eller senast kända `Tm`. Resultatet ska visas märkt `uppskattat` i UI:t tills ett
konkret kundfall har fakturaverifierats (samma `uppskattat`/`snapshot`-språk
`Resultatstatus.noggrannhet` redan använder på andra ställen i §6a).
...
och §6a.6. Basfördelningen ändras 45/26 → **47/24**; variantfördelningen 10/4 är oförändrad;
