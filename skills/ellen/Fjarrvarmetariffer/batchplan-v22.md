# Batchplan v22.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-09 av Claude. Ersätter `batchplan-v21.md` i sin helhet (v21 ändras INTE i
...
1. **`forkontrolleraPolicyIndata` har EN attesteringskälla** (rättar P1 #1, granskning
   `2026-09-09-015`): Batch 0 punkt 8:s femparametersskiss
   (`indataAttestering: Record<string, boolean>` som separat parameter) är ersatt med den
   fyrparametriga signaturen `forkontrolleraPolicyIndata(policy, prisar, indata,
   omfattning)` som läser `post.attesterad` ur den redan byggda `IndataPost`-kartan — samma
   signatur inventeringens egna anropsskisser alltid använt. Punkt 12, som tidigare
   hänvisade till "den femte parametern" som ett eget rättelselager, är struken helt, inte
   bara tömd.
2. **Dokumentidentitet rättad** (rättar P1 #2, granskning `2026-09-09-015`): denna fil och
   `tariffinventering-v22.md` bär nu korrekta V22-rubriker/inledningar. Samtliga aktiva/
   normativa korsreferenser till `tariffinventering-v20.md`/`v21.md`/`batchplan-v20.md`/
   `v21.md` är uppdaterade till v22; äldre versioner nämns bara i uttryckligt historiska
   stycken (som avsnitten nedan).
3. Samtliga v18–v21-fynd (t.o.m. granskning `2026-09-09-014`) som INTE fick ett nytt fynd i
   granskning `2026-09-09-015` är BEVARADE oförändrade — den strikta
   `TariffpolicyOptions`-konstruktorn med samtliga sju bindningskontroller, femparameters-
   guarden, fail-closed besparingsförmåga, `vardefelForKrav`s ägarskap,
   Tm-käll-/attesteringsmodellen, kanonisk `type`-diskriminator, motortransporten,
   Lidköpings källgodkännande, dispositionen 7/57/28 av 92.

**Vad som var nytt i v21** (se granskning `2026-09-09-014` för fullständig motivering):
...
- **Innehåll:**
  ...
  5. **Rättat P1 (granskning `2026-09-08-009`, senast rättad `2026-09-09-014` P1 #1) —
     ...
     **Rättat P1 (granskning `2026-09-09-007`) — v15:s inventeringsdokument motsade denna
     ...
     följer samma mönster som ett andra additivt fält, speglat i `tariffinventering-v22.md`
  ...
  8. **Felklassning — saknad, ogiltig OCH icke attesterad indata, nåbar även för ett direkt
     ...
     `2026-09-09-015` P1 #1 — ENDA gällande signatur, ersätter `2026-09-09-014`
     P1 #1:s femparametersskiss): `forkontrolleraPolicyIndata(policy, prisar, indata,
     omfattning: Omfattning): { saknade: readonly string[]; ogiltiga: readonly
     PolicyValideringsFel[] }` förblir FYRA parametrar — attestering läses ur den redan
     BYGGDA `indata: ReadonlyMap<string, IndataPost>`-kartans `post.attesterad`-fält, inte
     ur en separat femte parameter. Byggaren (punkt 5) skriver `attesterad` in i varje
     `IndataPost` INNAN förkontrollen körs, så förkontrollen och den auktoritativa
     statusvalidatorn i `harledResultatstatus`/Pythonmotsvarigheten läser exakt samma
     värde — det finns inte två sanningskällor efter byggsteget.
     ...
     bryter en regel. **Ny gren (rättat P1, granskning `2026-09-09-014` P1 #1, signaturen
     rättad `2026-09-09-015` P1 #1):** för varje krav med `f.kravAttestering === true` som
     FINNS i `indata` men där den mottagna `post.attesterad !== true` (läst ur samma
     `IndataPost`, inte en separat parameter), läggs `{ nyckel: f.nyckel, orsak:
  ...
  9. **Namngiven, gemensam produktentry för "aktuell årskostnad" (Stockholm OCH Lidköping),
     ...
     **Gällande definition** (ersätter v13–v19:s indirekta/fail-open varianter — se
     `tariffinventering-v22.md` §6a.4 för den fullständiga historiken/motiveringen):
  ...
  (Punkt 12 borttagen, rättat P1 granskning `2026-09-09-015` — attesteringstransporten är
  helt beskriven i punkterna 5/6/7/8 ovan; ett separat rättelselager sist i listan gav
  tidigare intrycket av ett konkurrerande, femparametrigt kontrakt och är därför strukits,
  inte bara tömt.)
...
`tariffinventering-v22.md` §6a.7 för fullständig design, formel, fail-closed-kontrakt och
...
- **Tariffer:** `lidkoping-energi-lidkoping-041-kw-2026`, `lidkoping-energi-lidkoping-42-kw-2026`
  ...
  `tariffinventering-v22.md` §4.2 och §8).
...
- **Rättat P1 (granskning `2026-09-09-010`) — v16:s motorkod var en fristående skiss utan
  ...
  hjälpfunktionsanrop — se `tariffinventering-v22.md` §6a.7.1–§6a.7.3 för den fullständiga,
...
- **Rättat P1 (granskning `2026-09-09-011`) — fillistan placerade katalogändringen i fel
  ...
  etappen. Det är sant för DENNA dokumentationscommit (v22/`tariffinventering-v22.md`
...
- **Fillista, Batch 5d (planerad FRAMTIDA implementation — INTE genomförd av v22/denna
  ...
  - **Aktivering** (§6a.7) sker FÖRST efter granskad kod och gröna tester för samtliga
    ...
    i v22/denna dokumentationscommit.
- **Golden-/gräns-/negativtester:** §6a.7:s tolv testfall (nio ursprungliga + tre nya i
  v17, se `tariffinventering-v22.md` §6a.7 slutet), inklusive ett end-to-end-test mot
  ...
  produktkod ändras av `tariffinventering-v22.md`/`batchplan-v22.md`.
...
- **Fail-closed före/efter-regel för besparingsvärdering (nytt P1, granskning
  ...
  `KontraktBlockerat` (se `tariffinventering-v22.md` §6a.4 för klass/guard/UI-mappning).
...
adderas: 55 + 2 = **57**, exakt matchande `tariffinventering-v22.md` §8: 47 bastariffer +
...
komplett policykonstruktionskontrakt, se `tariffinventering-v22.md` §6a.7/§6a.4/§6a.1/§6a.2).
