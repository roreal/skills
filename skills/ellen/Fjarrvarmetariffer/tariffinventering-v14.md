# Tariffinventering v14.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v13.md` i sin helhet (v13 ändras
INTE i efterhand — kvar som historik), som svar på Codex omgranskning
[2026-09-09-004](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v13.md)
(`status: changes-required`, supersedes `2026-09-09-003`) av v13. Ingen produktkod, tariffdata
eller genererad fil är ändrad av detta dokument. Ingen tariff aktiveras. Dispositionerna
7 implementerade / 55 redo / 30 blockerade / 92 totalt är oförändrade (bekräftat av
granskningen). Alla kodpåståenden nedan är verifierade genom att läsa den verkliga koden i
...
6. **Felklassning — både saknad OCH ogiltig kundindata som typade användarfel, genom en
   ...
   **Konkret, konstruerbar `KontraktBlockerat`-väg (rättat P1, granskning `2026-09-09-004`):
   ...
   konstruktoranropspunkt eller någon ny `orsak`.** `KontraktBlockeratOrsak`
   (`resultatkontrakt.ts`) utökas från dagens 5 medlemmar (`missing_capacity`,
...
inte separata, anropbara hjälpfunktioner. **Löst genom att extrahera den VERBATIMA
inline-logiken** (oförändrad, bara flyttad) till en delad bastyp och en delad
hjälpfunktion, byggd enbart av fält som faktiskt finns på `KalkylatorInputs`:
...
export function argsFranInputs(inputs: KalkylatorInputs): Tariffberakningsunderlag {
  // Extraherad VERBATIM ur calcResults befintliga scope-uppskalnings-/
  // proveniensblock (energiPotential.ts, rad ~405–545) — ingen ny beräkningslogik,
  // bara flyttad ut ur calcResults kropp och namngiven. Byggd uteslutande av fält
  // som verifierat finns på KalkylatorInputs (energySystem, energyMwh, energyScope,
  // energyInputMode, leverantorId, kapacitetKw, falt) — inga påhittade fält.
  let totalMwh = inputs.energyMwh ?? 0;
  if (inputs.energyScope === 'rumsvarme' && inputs.energyInputMode === 'rumsvarme_andel') {
    // samma uppskalningsgren calcResult redan kör inline när scope är rumsvärme och
    // indataläget är en andel — oförändrad formel, bara flyttad hit.
    totalMwh = totalMwh; // se calcResults rad ~460–490 för den fullständiga, oförändrade formeln
  }
  const energyProvenance: 'confirmed_mwh' | undefined =
    inputs.energyInputMode === 'confirmed_mwh' ? 'confirmed_mwh' : undefined;
  return {
    totalMwh,
...
**Not till implementatören:** blocket ovan namnger fälten och platsen (rad ~405–545) den
faktiska uppskalnings-/proveniensformeln ska flyttas FRÅN — själva formelns exakta
villkorsgrenar (utöver `energyScope`/`energyInputMode`-diskriminatorerna, som är verifierat
riktiga fältnamn) ska kopieras verbatim ur `calcResult`s nuvarande kropp vid
implementationstillfället, inte återskapas ur minnet i detta planeringsdokument — samma
försiktighetsprincip som resten av detta dokument tillämpar mot gissade siffror/API:er.
...
export function calcResultForOnskadTyp(inputs: KalkylatorInputs): KalkylatorResultUnion {
  // Energisystem-grinden FÖRST (rättat P1, granskning 2026-09-09-004): v13 lät
  // stodjerAktuellArskostnad avgöra ensam, vilket hade tillåtit funktionen att slå upp
  // en kontraktsgated policy för ett icke-fjärrvärmesystem innan produkttypen ens är
  // relevant. Tabellen är nu: icke-fjärrvärme → alltid false (kortsluts här, aldrig ens
  // en kontraktsuppslagning); fjärrvärme + legacytariff → false; fjärrvärme + Sandviken
  // (kontraktsgatad, utan kallenergiArsserieBindning) → false; fjärrvärme + Stockholm
  // (kontraktsgatad, med kallenergiArsserieBindning) → true.
  if (inputs.onskadTyp === 'aktuell_arskostnad') {
    if (inputs.energySystem !== 'fjarrvarme') {
      throw new Error('Aktuell årskostnad stödjs bara för fjärrvärme'); // kortsluts INNAN
      // någon tariff/policy-uppslagning görs
    }
