# Tariffinventering v17.0 — fullständig kontrollmängd för kalkylator v1

Upprättad 2026-09-09 av Claude. Ersätter `tariffinventering-v16.md` i sin helhet (v16 ändras
INTE i efterhand — kvar som historik), som svar på omgranskning
[2026-09-09-010](../conversations/reviews/2026/09/2026-09-09-omgranskning-tariffinventering-v16.md)
(`status: changes-required`, supersedes `2026-09-09-007`) av v16/`batchplan-v16.md`. Ingen
...
blockerade / 92 totalt** (bas 7/47/24, variant 0/10/4; se §4.2 och §8) — v17 gör bara
...
**Vad som är nytt i v17, i korthet** (se granskning `2026-09-09-010` för fullständig
...
(`ValueError`/`Error`: "minvarde_exklusiv kräver att minvarde också är satt"). Kontrollen i
`harled_resultatstatus`/`harledResultatstatus` (och i `forkontrolleraPolicyIndata`s
fältnära motsvarighet, §6a.2) byts från `post.varde < f.minvarde` till: brott om
(`f.minvarde_exklusiv` och `not post.varde > f.minvarde`) ELLER (INTE
`f.minvarde_exklusiv` och `post.varde < f.minvarde`) — SAMMA orsak `'min'` oavsett gren,
bara gränsen (`<` mot `<=`) skiljer. Används av Lidköpings `Tm_m`-krav (`minVarde: 0,
minExklusiv: true`, §6a.7.5) — första fältet i katalogen som behöver en strikt undre gräns.
Isolerat compile-verifierad (`npx tsc --noEmit --strict --skipLibCheck --target es2020`,
kontrollfil i `/tmp`, borttagen efter körning): negativt element, exakt `0` med
`minExklusiv: true`, `0` med `minExklusiv` osatt och ett litet positivt tal gav samtliga
förväntat resultat (`'min'`/`'min'`/OK/OK).
...
Negativa test krävs för samtliga fjorton fält ovan: en genererad policy med fältet satt som
...
**Rättat P1 (granskning `2026-09-09-010`) — guarden var bunden till Stockholms specifika
`kallenergi_arsserie_bindning`-fält, vilket hade lämnat Lidköpings besparingsväg OSKYDDAD.**
Lidköpings tre serier (§6a.7) hålls, om de skickades oförändrade i ett syntetiskt
före/efter-par, per konstruktion identiska mellan de två anropen — flödesjusteringen skulle
då kvitta sig själv ur besparingen och ge ett tyst, falskt nollbidrag, exakt samma fara som
Stockholms kallenergiserie redan är skyddad mot. Guarden i `beraknaBesparingsvardeKontrakt`
byts därför från "policyn har `kallenergi_arsserie_bindning` satt" till "policyn har
`stodjer_aktuell_arskostnad=True`" (§6a.4 ovan) — SAMMA explicita förmågeflagga som gör
"aktuell årskostnad" nåbar gör nu OCKSÅ besparing blockerad, tills en tariffspecifik
före/efter-regel läggs till som ett medvetet undantag. Detta täcker Stockholm (redan
`True`) OCH Lidköping (ny `True` denna version) med EN gemensam kontroll, i stället för att
kräva en ny per-tariff-guard för varje framtida serie-baserad tariff som bara stödjer
aktuell årskostnad. Ändrar inte Stockholms befintliga, redan godkända beteende — Stockholm
hade redan `kallenergi_arsserie_bindning` satt, och har nu också
`stodjer_aktuell_arskostnad=True` (§6a.4 ovan), så guarden slår igen identiskt.
...
**Rättat P1 (granskning `2026-09-09-003`) — `stodjerAktuellArskostnad` hade TRE-FYRA
motsägande definitioner, och läste rådata direkt i stället för via den enda tillåtna
resolvern.** v12 sa på tre olika ställen (batchplanens sammanfattning, dess Batch 7-avsnitt,
och sidwrapperns kod) att funktionen var "sann för varje kontraktsgated tariff", medan
inventeringens EGEN kapacitetsfunktion och dess EGET testkrav (Sandviken → `false`, Stockholm
→ `true`) beskrev en SMALARE definition — sann bara när
`kallenergiArsserieBindning` finns, INTE för alla kontraktsgatade tariffer (Sandviken ÄR
kontraktsgatad men saknar denna bindning). Funktionen läste dessutom
`prisar.policy?.kallenergiArsserieBindning` DIREKT, i stället för via
`kontraktsgatadPolicy(prisar)` (`besparingsvarde.ts`, den dokumenterat ENDA platsen som får
tolka den råa `_kraver_kontrakt`/`policy`-markören) — en malformerad/inkonsistent
policymarkör hade alltså kunnat läsas OLIKA av `stodjerAktuellArskostnad` och av resten av
kontraktsvägen. Guarden fanns dessutom ENDAST i sidnivå-wrappern (`calcResultForOnskadTyp`
nedan); `beraknaArsprodukt` — en ny PUBLIK domänentry — var uttryckligen oskyddad: ett anrop
som kringgick wrappern hade nått motorn för en tariff utan förmågan.

**Löst med EN tabell, EN resolver, och guarden på BÅDA nivåer:**
...
export function stodjerAktuellArskostnad(prisar: GenereradPrisarspost): boolean {
  ...
  // KontraktsgatadPolicy = { policy: Tariffpolicy; harAnnualInverse: boolean } —
  // kallenergiArsserieBindning bor PÅ Tariffpolicy, alltså under .policy, inte direkt
  // på KontraktsgatadPolicy. v13 läste `gated?.kallenergiArsserieBindning` (fel objekt,
  // alltid undefined eftersom KontraktsgatadPolicy saknar det fältet på toppnivå) i BÅDA
  // tariffinventering-v13.md OCH batchplan-v13.md oberoende — rättat P1, granskning
  // `2026-09-09-004`.
  return gated?.policy.kallenergiArsserieBindning !== undefined;
}
```

Kapacitetstabellen (samma i BÅDA dokumenten): icke-fjärrvärmesystem → `false`; legacytariff
(ingen `_kraver_kontrakt`) → `false`; Sandviken (kontraktsgatad, men UTAN
`kallenergiArsserieBindning`) → `false`; Stockholm (kontraktsgatad, MED
`kallenergiArsserieBindning`) → `true`.

**Rättat P1 (granskning `2026-09-09-010`) — förmågan var en INDIREKT kontroll av
Stockholms specifika bindning, vilket gjorde Lidköpings "aktuell årskostnad" onåbar.**
Ovanstående `gated?.policy.kallenergiArsserieBindning !== undefined` är, PER
KONSTRUKTION, sann bara för tariffer som råkar använda just DEN mekanismen (Stockholms
sätt att härleda årsenergi ur en källenergiserie) — Lidköping behöver ingen sådan
härledning (energin är redan given som ett vanligt bekräftat MWh-värde), så förmågan
skulle förbli `false` trots att Lidköping har allt underlag `beraknaArsprodukt` behöver.
En förmåga fick alltså av misstag betydelsen "har DEN HÄR bindningen", inte "kan besvara
frågan aktuell årskostnad".

**Löst genom att göra förmågan till ett EXPLICIT fält på `Tariffpolicy`, satt av
`policyregister.py` per tariff, i stället för att härledas indirekt av vilken
bindningsmekanism policyn råkar ha:**

```python
# resultatkontrakt.py, Tariffpolicy — nytt fält
@dataclass(frozen=True)
class Tariffpolicy:
    ...
    stodjer_aktuell_arskostnad: bool = False
```

```ts
// resultatkontrakt.ts, Tariffpolicy — mirror
export interface Tariffpolicy {
  ...
  stodjerAktuellArskostnad?: boolean; // default false vid frånvaro, samma mönster som
                                       // övriga valfria Tariffpolicy-fält i §6a
}
```

```ts
export function stodjerAktuellArskostnad(prisar: GenereradPrisarspost): boolean {
  const gated = kontraktsgatadPolicy(prisar); // fortsatt enda tillåtna rådatagrind
  return gated?.policy.stodjerAktuellArskostnad === true;
}
```

`policyregister.py` sätter `stodjer_aktuell_arskostnad=True` explicit för VARJE policy som
har en fullständig, en-anrops "aktuell årskostnad"-väg — i dag Stockholm (via
`kallenergiArsserieBindning`) och, med denna etapp, Lidköping (via de tre serierna i
§6a.7). Sandviken förblir `False` (inget default ändras — fältets default är `False`,
samma "explicit opt-in"-mönster som `ersatter_katalograd`/`kapacitet_band_bindning`).
Kapacitetstabellen (samma i BÅDA dokumenten, RÄTTAD): icke-fjärrvärmesystem → `false`;
legacytariff (ingen `_kraver_kontrakt`) → `false`; Sandviken (kontraktsgatad, men
`stodjer_aktuell_arskostnad` osatt) → `false`; Stockholm (kontraktsgatad,
`stodjer_aktuell_arskostnad=True`) → `true`; **Lidköping (kontraktsgatad,
`stodjer_aktuell_arskostnad=True`, NY rad denna version) → `true`**.
`kallenergiArsserieBindning` finns kvar på `Tariffpolicy` och används fortsatt av
Stockholms egen härledningslogik (§6a.4 nedan) — den är bara inte längre KÄLLAN för
förmågekontraktet.
...
`argsFranInputs`) — den ENDA produktentrypunkten för Stockholms "aktuell årskostnad"; den tar
...
- `Tm_m` (nät-avkylningsserien) märks `kallaTyp: 'snapshot'` (§6a.7.6 nedan för det VALDA
  proveniensbeslutet — INTE `'supplier_value'`).
...
**§6a.7.5 Rättat P1 (granskning `2026-09-09-010`) — vald `Tm_m > 0`-mekanism.** v16 lovade
resultatet (`invalid_policy_fields`/`'min'`) men sköt upp VALET av mekanism till
implementationstillfället, trots att dagens `minVarde` är inkluderande. V17 väljer: nytt
generiskt `KravPost`-fält `minvarde_exklusiv: bool = False` (Python)/`minExklusiv?: boolean`
(TS), FÅR bara sättas tillsammans med `minvarde`/`minVarde`
(`__post_init__`/`skapaKravPost` kastar annars — samma valideringsmönster §6a.1 redan
använder för `maxvarde`). `harled_resultatstatus`/`harledResultatstatus` OCH
`forkontrolleraPolicyIndata`s fältnära kontroll (§6a.2) byter kontrollen
`post.varde < f.minvarde` mot `(f.minvarde_exklusiv and not post.varde > f.minvarde) or
(not f.minvarde_exklusiv and post.varde < f.minvarde)` — samma orsak `'min'` i båda fallen,
skillnaden är bara `<` vs `<=` som gräns. `Tm_m` sätts till `minVarde: 0, minExklusiv:
true`; `Q_m` förblir `minVarde: 0` (inkluderande, `minExklusiv` osatt/`false`). Testat
isolerat (`npx tsc --noEmit --strict --skipLibCheck --target es2020`, `/tmp`-fil borttagen
efter körning): negativt element → `'min'`; exakt `0` med `minExklusiv: true` → `'min'`;
`0` med `minExklusiv` osatt → OK; ett litet positivt tal → OK i båda fallen. Kontrollerat
FÖRE divisionen i `signedMonthlyFlowAdjustment`/`_signed_monthly_flow_adjustment` (§6a.7.2)
kan nås.

**§6a.7.6 Rättat P2 (granskning `2026-09-09-010`) — Tm-proveniensen var självmotsägande.**
v16 sa att `Tm_m` INTE fick skickas in fritt av kunden (`kallaTyp: 'supplier_value'`), men
Batch 5d visade samtidigt tre likvärdiga serieinmatningar i UI, och `byggIndataFranPolicy`
(§6a.2) märker VARJE formulärvärde `kallaTyp: 'supplier_value'` oavsett fält — ett fritt
inmatat tal hade alltså automatiskt påstått sig vara ett verifierat leverantörsvärde. V17
väljer ETT spårbart kontrakt: `Tm_m` matas in i SAMMA UI-serieformulär som `Q_m`/`T_m` (det
finns ingen separat betrodd runtimekälla att hämta det ifrån — leverantörssvaret ger bara
METODEN, inte de tolv talen, se nedan), men `byggIndataFranPolicy` märker `Tm_m` specifikt
`kallaTyp: 'snapshot'` i stället för `'supplier_value'` — ett NYTT, tredje värde på samma
diskriminator (mirror i Python), som betyder "transkriberat av användaren från en angiven
källa (faktura eller direkt leverantörsbesked), inte ett fritt gissat tal". UI-fältet för
`Tm_m` får obligatorisk källhjälptext ("Ange nätets månadsmedelavkylning enligt
Lidköpings faktura eller skriftliga besked — inte ett eget uppskattat värde") och
`Resultatstatus.noggrannhet` sätts till `'uppskattat'` (§6a.7.7) så länge underlaget vilar på
en `'snapshot'`-post. `Q_m`/`T_m` behåller `kallaTyp: 'supplier_value'` — de är kundens EGNA
fakturavärden.
...
därför ett RUNTIMEKRAV som matas in per kundfall (`kallaTyp: 'snapshot'`, §6a.7.6), inte ett
statiskt katalogvärde i `adjustments`. Saknas det för ett givet kundfall ska HELA Lidköpings
fullständiga tariff blockeras för det fallet — inte räknas med ett gissat eller senast kända
`Tm`. Resultatet visas märkt `uppskattat` i UI:t tills ett konkret kundfall har
fakturaverifierats (samma `uppskattat`/`snapshot`-språk `Resultatstatus.noggrannhet` redan
använder på andra ställen i §6a).

**§6a.7.7 Rättat P1 (granskning `2026-09-09-010`) — Lidköpings "aktuell årskostnad" var
onåbar, och besparingsvägen saknade en före/efter-regel.** Se §6a.5 nedan för det explicita
förmågekontraktet (`stodjerAktuellArskostnad`) som nu gör Lidköpings estimerade årskostnad
nåbar via `beraknaArsprodukt`, och det typade produktbegränsningsfelet som blockerar
...
12. **Nytt:** Lidköpings "aktuell årskostnad" (§6a.5/§6a.7.7) = ett fasadanrop via
    `beraknaArsprodukt`, samma mönster som Stockholm; Lidköpings besparingsväg =
    `KontraktBlockerat`/motsvarande med ett typat produktbegränsningsfel, aldrig ett
    syntetiskt oförändrat före/efter-par.
...
**Oförändrat i v17 (granskning `2026-09-09-010`):** V17 rättar Lidköpings
implementationsPLAN (§6a.7, §6a.4/§6a.5) och gör den körbar — INGEN dispositionsflytt.
Räkningen förblir **7 + 57 + 28 = 92** (bas 7/47/24, variant 0/10/4), oförändrad sedan v16.
Lidköpings två bastariffer är fortsatt `ready_to_implement`.
