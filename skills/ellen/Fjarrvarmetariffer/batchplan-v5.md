# Batchplan v5.0 — implementationsordning för `ready_to_implement`

Upprättad 2026-09-08 av Claude. Ersätter `batchplan-v4.md` i sin helhet (v4 ändras INTE i
...
(supersedes `2026-09-08-003`). Bygger på dispositionerna i
[`tariffinventering-v5.md`](tariffinventering-v5.md) §4.1 (45 bastariffer) och §5 (9
`ready_to_implement`-varianter), samt §6:s körda grindpreflight och §7:s livscykel för
informationsförfrågningar. Ingen batch är påbörjad — detta är ett förslag till Codex
granskning och Roberts prioritering.

**Vad som är nytt i v5** (se granskning `2026-09-08-004` för fullständig motivering):

1. Batch 2 (Sundsvall Indal): informationsförfrågan R14 preciserad som en DELNING, inte en
   borttagning — den täcker även två fortsatt blockerade Sundsvall-tariffer hos samma
   medlem.
2. Batch 4 (Umeå): kompletterad med kapacitetsfaktorn `B` som ett fjärde obligatoriskt
   fält och en ny `post_multiplier`-medveten kapacitetsmotor — v4 nämnde bara
   flödesjusteringen och missade att grinden avvisar posten oavsett.
3. Batch 5a/5b: explicit livscykel för C4:s, Kils, TEMAB:s, Borlänges och Falu ytterorters
   `issues`-texter, som v5:s egen körda grindkontroll visade var en ANDRA, dold blockerare
   bakom den katalogrättelse v4 redan beskrev.
4. Batch 5c: "Visas för användaren"-raden rättad så Mälarenergi 2–4 lägenheters undantag
   (ingen effekt/band) syns i själva resultatpresentationen, inte bara i indatalistan.
5. Batch 6: `katalog.py`/`.ts`s `grind()`-utökning för de nya kapacitetsformerna namngiven
   explicit som en berörd fil.
6. Batch 7 (Stockholm Exergi): omskriven för en NAMNGIVEN ADAPTER i stället för "motorn
   finns redan" — katalogpostens energiform är inte allmänt godkänd, plus en ytterligare
   `issues`-rad upptäckt av v5:s grindkörning.
7. Fyra variantkällor med "samma URL som ovan" skrivna ut direkt; Finspångs, Södertörns och
   Borås variantkällor rättade till aktuella 2026-dokument.
...
- **Filer:** katalog-JSON, `godkanda()`-testet i `test_katalog.py` (räknaren 7→8), `remaining_information_requests` (R14 delas — se inventeringens §7 — så `sundsvall-energi-sundsvall-normal-2026`/`-matfors-och-kvissleby-normal-2026` förblir medlemsblockerade medan Indal/Liden/Lucksta öppnas).
...
- **Modell:** TVÅ separata nya justeringstyper i `justeringar.py`/`adjustments.ts` —
  ...
  multiplikator`); ändringen gör att en tariff GODKÄNNS när `B` är deklarerat som ett
  obligatoriskt `Tariffpolicy`-fält, aldrig genom att kalkylatorn själv räknar ut
  `U = normalårskorrigerad_energi_dec_jan_feb / energi_sep_apr` (bolagets normalårskorrigering
  är inte publicerad). Ingen väntar på leverantörsbesked.
...
- **Teststrategi:** golden-värde per tariff, gränstest för respektive formels
  ...
  BLOCKERADE varianter av samma typnamn, samt Umeås tre `U`-intervall (0,300/0,500/0,800)
  som bevisar `B` alltid kommer från leverantörsvärdet, aldrig en egen beräkning.
...
- **Obligatorisk indata:** debiterbar effekt/band (fakturan) — automatisk gruppindelning
  BLOCKERAS för C4 (500 kW-gränsen). KATALOGRÄTTELSE (utökat i v5, granskning 2026-09-08-004,
  P1): C4 (`c4-energi-kristianstad-2026`), Kils (`kils-energi-kil-2026`, dessutom `fixed:0`
  på alla fyra band) och TEMAB (`temab-fjarrvarme-tierp-karlholmsbruk-och-orbyhus-2026`)
  har var sin `issues`-text som INTE matchar grindens godkännandelista — normaliseras till
  den redan kända typen "Metod för debiterbar effekt/kapacitet är inte fullständigt
  mappad; använd leverantörens fakturavärde" (se inventeringens §6). Informationsförfrågningarna
...
- **Obligatorisk indata:** debiterbar effekt/band + prissatt flöde i m³ (hela året,
  fakturan/avtalet) för samtliga sex. KATALOGRÄTTELSE (utökat i v5, granskning 2026-09-08-004,
  P1): Borlänge och Falu ytterorter har var sin `issues`-text som inte matchar grindens
  godkännandelista — normaliseras (se inventeringens §6); Falu ytterorters normalisering
  gör dessutom explicit att beräkning ovanför 500 kW blockeras (fail-closed), inte
  extrapoleras. Informationsförfrågningarna R04 (borlange-energi), R15 (falu-energi-vatten,
  delad — se inventeringens §7) TAS BORT/delas.
...
- **Filer:** ny kapacitetsformelkod (`heterogeneous_bands`, `piecewise_polynomial`) i
  `faktura.py`/`fjarrvarme.ts`, `katalog.py`/`.ts` (`grind()` utökad med de nya
  kapacitetsformerna i sin godkännandelista), två nya justeringstyper
  (`optional_environmental_addon`, `conditional_flow`) i `justeringar.py`/`.ts`,
  katalograder, policyer.
...
- **Modell (rättat i v5, granskning 2026-09-08-004, P1: v4 saknade en körbar väg genom
  grinden — katalogpostens energiform `monthly_with_peak_volume_replacement` är inte en
  allmänt godkänd energiform):** NAMNGIVEN ADAPTER, inte en generell motorutökning —
  energiformen är unik för Stockholm Exergi i hela katalogen. `katalog.py`/`.ts` får ett
  namngivet undantag för just `stockholm-exergi-stockholm-exergi-normal-2026` som binder
  `annual_forward`-fasaden till samma redan fakturavaliderade leverantörsspecifika
  beräkningsväg (`_stockholm_exergi_policy`-mönstret) i stället för den generella
  `selected_band_affine`-katalogmotorn. Bygg ett källverifierat årsreferensfall,
  motsvarande Sandvikens granskningskedja (`2026-09-06-004`→`2026-09-07-003`). Gör kall
  energi och returtemperatur till synlig, obligatorisk indata för årsvägen — MED samma
  period-/upplösningskontrakt som det redan godkända `monthly_invoice`-kontraktet: kall
  energi samtliga tolv månader, returtemperatur bara november–mars (5 vintermånader).
  Ytterligare katalogrättelse (upptäckt av v5:s egen grindverifiering): `issues`-texten
  "Förtydligande av prisvillkor 2026 finns..." TAS BORT — den medger själv att
  leverantörens effektvärde kan användas som kundindata, samma mönster adaptern bygger på.
...
- **Filer:** `resultatkontrakt.py`/`.ts` (ny anropspunkt), `policyregister.py` (utökad
  `_stockholm_exergi_policy`), `katalog.py`/`.ts` (namngivet undantag).
